# nix/nixosModules.nix — NixOS module for wenshu-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: wenshu runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $WENSHU_HOME/.env
# and read by wenshu at startup — no container recreation needed for env changes.
#
# Tool resolution: the wenshu wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.wenshu-agent;
    effectivePackage =
    ...
    wenshu-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-wenshu-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "wenshu-config-attrs";
      description = "Wenshu YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — wenshu
    ...
    generatedConfigFile = pkgs.writeText "wenshu-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # WENSHU_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "wenshu-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'WENSHU_DOC_EOF'\n${value}\nWENSHU_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "wenshu-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/wenshu";
    ...
    # wenshu user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "wenshu-container-entrypoint" ''
      set -eu

      WENSHU_UID="''${WENSHU_UID:?WENSHU_UID must be set}"
      WENSHU_GID="''${WENSHU_GID:?WENSHU_GID must be set}"

      # ── Group: ensure a group with GID=$WENSHU_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$WENSHU_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="wenshu"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$WENSHU_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$WENSHU_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$WENSHU_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$WENSHU_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="wenshu"
        TARGET_HOME="/home/wenshu"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$WENSHU_UID" -g "$WENSHU_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$WENSHU_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$WENSHU_UID:$WENSHU_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure WENSHU_HOME is owned by the target user.
      ...
      if [ -n "''${WENSHU_HOME:-}" ] && [ -d "$WENSHU_HOME" ]; then
        find "$WENSHU_HOME" \! -user "$WENSHU_UID" -exec chown "$WENSHU_UID:$WENSHU_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/wenshu-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/wenshu-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/wenshu ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/wenshu
        chmod 0440 /etc/sudoers.d/wenshu
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$WENSHU_UID" --regid="$WENSHU_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $WENSHU_HOME/.env.
    ...
    # Default: /var/lib/wenshu/workspace → /data/workspace.
  ...
  in {
    options.services.wenshu-agent = with lib; {
      enable = mkEnableOption "Wenshu Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = wenshu-agent;
        description = "The wenshu-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/wenshu";
        description = "State directory. Contains .wenshu-hermes/ subdir (WENSHU_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Wenshu config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $WENSHU_HOME/.env at activation time.
          Wenshu reads this file on every startup via load_wenshu_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $WENSHU_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $WENSHU_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `wenshu gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the wenshu user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the wenshu plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Wenshu discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          wenshu_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as wenshu (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in wenshu-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the wenshu CLI to environment.systemPackages and export
          WENSHU_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.wenshu symlink to the service
            stateDir. These users are automatically added to the wenshu group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.wenshu-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the wenshu CLI to system PATH and export WENSHU_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.wenshu-hermes/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.WENSHU_HOME = "${cfg.stateDir}/.wenshu-hermes";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.wenshu-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the wenshu user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.wenshu-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed wenshu
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure wenshu is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.wenshu        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.wenshu-hermes/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.wenshu-hermes/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.wenshu-hermes/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.wenshu-hermes/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.wenshu-hermes/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."wenshu-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.wenshu
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.wenshu ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.wenshu ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.wenshu -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.wenshu-hermes/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.wenshu-hermes/$_subdir"
            chmod 2770 "${cfg.stateDir}/.wenshu-hermes/$_subdir"
            find "${cfg.stateDir}/.wenshu-hermes/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # wenshu-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.wenshu-hermes/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.wenshu-hermes/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.wenshu-hermes/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.wenshu-hermes/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.wenshu-hermes/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.wenshu-hermes/.managed
          chmod 0644 ${cfg.stateDir}/.wenshu-hermes/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.wenshu-hermes/.container-mode <<'WENSHU_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    wenshu_bin=${containerDataDir}/current-package/bin/wenshu
    WENSHU_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.wenshu-hermes/.container-mode
            chmod 0644 ${cfg.stateDir}/.wenshu-hermes/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.wenshu-hermes/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.wenshu-hermes";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.wenshu-hermes" ]; then
                  rm -f "${symlinkPath}"
                  echo "wenshu-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.wenshu -> stateDir/.wenshu for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.wenshu-hermes";
                target = "${cfg.stateDir}/.wenshu-hermes";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "wenshu-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.wenshu-hermes/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.wenshu-hermes/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.wenshu-hermes/auth.json
              fi
            ''}
          ''}
          ...
          # Wenshu reads $WENSHU_HOME/.env at startup via load_wenshu_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.wenshu-hermes/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'WENSHU_NIX_ENV_EOF'
    ${envFileContent}
    WENSHU_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.wenshu-hermes/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.wenshu-hermes/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.wenshu-hermes/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.wenshu-agent = {
          description = "Wenshu Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            WENSHU_HOME = "${cfg.stateDir}/.wenshu-hermes";
            WENSHU_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $WENSHU_HOME/.env by the activation script. load_wenshu_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/wenshu"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the wenshu group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.wenshu-agent = {
          description = "Wenshu Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              WENSHU_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              WENSHU_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env WENSHU_UID="$WENSHU_UID" \
                --env WENSHU_GID="$WENSHU_GID" \
                --env WENSHU_HOME=${containerDataDir}/.wenshu \
                --env WENSHU_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/wenshu gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
