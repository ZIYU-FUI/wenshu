...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      wenshuNpmLib = self'.packages.default.passthru.wenshuNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. wenshu-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        packages =
          with pkgs;
          [
            (pkgs.runCommand "wenshu" { } ''
              mkdir -p $out/bin
              install -Dm755 ${../wenshu} $out/bin/wenshu
            '')
        ...
        shellHook = ''
          ${combinedNonNpm}
          ${wenshuNpmLib.mkNpmDevShellHook npmPackageJsonPaths}

          # for the devshell to pick up the src
          export WENSHU_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          echo "Wenshu Agent dev shell in $WENSHU_PYTHON_SRC_ROOT"
          echo "Ready. Run 'wenshu' or 'sandbox wenshu' to start."
        '';
      };
    };
}
