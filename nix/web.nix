# nix/web.nix — Wenshu Web Dashboard (Vite/React) frontend build
{ pkgs, wenshuNpmLib, ... }:
let
  # @wenshu/shared ships as a file: workspace dep of web, so its source
...
pkgs.buildNpmPackage (npm // {
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../wenshu_cli/web_dist for the
