...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      wenshu-agent = self'.packages.default;
      wenshuVenv = wenshu-agent.wenshuVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "wenshu-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${wenshuVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "wenshu-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "wenshu-build-package" { } ''
          echo "PASS: package built at ${wenshu-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "wenshu-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "wenshu-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${wenshu-agent}/bin/wenshu || (echo "FAIL: wenshu binary missing"; exit 1)
          test -x ${wenshu-agent}/bin/wenshu-agent || (echo "FAIL: wenshu-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${wenshu-agent}/bin/wenshu version 2>&1 | grep -qi "wenshu" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "wenshu-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in wenshu wenshu-agent wenshu-acp; do
            test -x ${wenshu-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "wenshu-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking wenshu --help ==="
          ${wenshu-agent}/bin/wenshu --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${wenshu-agent}/bin/wenshu --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "wenshu-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${wenshu-agent}/share/wenshu-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${wenshu-agent}/share/wenshu-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "WENSHU_BUNDLED_SKILLS" ${wenshu-agent}/bin/wenshu || \
            (echo "FAIL: WENSHU_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: WENSHU_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${wenshu-agent}/share/wenshu-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${wenshu-agent}/share/wenshu-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "WENSHU_OPTIONAL_SKILLS" ${wenshu-agent}/bin/wenshu || \
            (echo "FAIL: WENSHU_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, WENSHU_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "wenshu-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${wenshu-agent}/share/wenshu-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${wenshu-agent}/share/wenshu-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "WENSHU_BUNDLED_PLUGINS" ${wenshu-agent}/bin/wenshu || \
            (echo "FAIL: WENSHU_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: WENSHU_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "wenshu-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${wenshu-agent}/share/wenshu-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${wenshu-agent}/share/wenshu-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${wenshu-agent}/share/wenshu-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "WENSHU_BUNDLED_LOCALES" ${wenshu-agent}/bin/wenshu || \
            (echo "FAIL: WENSHU_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: WENSHU_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (WENSHU_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && WENSHU_BUNDLED_LOCALES=${wenshu-agent}/share/wenshu-agent/locales \
            ${wenshuVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with WENSHU_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${wenshuVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${wenshuVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without WENSHU_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "wenshu-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${wenshu-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${wenshu-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "WENSHU_TUI_DIR" ${wenshu-agent}/bin/wenshu || \
            (echo "FAIL: WENSHU_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: WENSHU_TUI_DIR set in wrapper"
        ...
        # Verify WENSHU_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        wenshu-node = pkgs.runCommand "wenshu-node-version" { } ''
          set -e
          echo "=== Checking WENSHU_NODE in wrapper ==="
          grep -q "WENSHU_NODE" ${wenshu-agent}/bin/wenshu || \
            (echo "FAIL: WENSHU_NODE not set in wrapper"; exit 1)
          echo "PASS: WENSHU_NODE present in wrapper"

          WENSHU_NODE=$(sed -n "s/^export WENSHU_NODE='\(.*\)'/\1/p" ${wenshu-agent}/bin/wenshu)
          test -x "$WENSHU_NODE" || (echo "FAIL: WENSHU_NODE=$WENSHU_NODE not executable"; exit 1)
          echo "PASS: WENSHU_NODE executable at $WENSHU_NODE"

          NODE_MAJOR=$("$WENSHU_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All WENSHU_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify WENSHU_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "wenshu-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(WENSHU_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking WENSHU_MANAGED guards ==="
          check_blocked "config set" ${wenshu-agent}/bin/wenshu config set model foo
          check_blocked "config edit" ${wenshu-agent}/bin/wenshu config edit
        ...
        extra-python-packages = let
          ...
          wenshuWithExtra = wenshu-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "wenshu-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${wenshuWithExtra}/bin/wenshu || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${wenshuWithExtra}/bin/wenshu || \
          ...
          if grep -q "PYTHONPATH" ${wenshu-agent}/bin/wenshu; then
        ...
        extra-dependency-groups = let
          wenshuWithGroups = wenshu-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "wenshu-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${wenshuWithGroups}"
          echo "venv: ${wenshuWithGroups.wenshuVenv}"
        ...
        messaging-variant = pkgs.runCommand "wenshu-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.wenshuVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "wenshu-config-roundtrip" {
          ...
          merge_and_load() {
            local wenshu_home="$1"
            export WENSHU_HOME="$wenshu_home"
            ${configMergeScript} ${nixSettings} "$wenshu_home/config.yaml"
            ${wenshuVenv}/bin/python3 -c '
import json, sys
