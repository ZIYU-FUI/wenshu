"""
Wenshu MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Wenshu-specific extra)

Usage:
    wenshu mcp serve
    wenshu mcp serve --verbose
...
logger = logging.getLogger("wenshu.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using WENSHU_HOME."""
    try:
        from wenshu_constants import get_wenshu_home
        return get_wenshu_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("WENSHU_HOME", Path.home() / ".wenshu-hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from wenshu_constants import get_wenshu_home
        directory_file = get_wenshu_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("WENSHU_HOME", Path.home() / ".wenshu-hermes")
...
class EventBridge:
    ...
    This is the Wenshu equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from wenshu_constants import get_wenshu_home
            db_file = get_wenshu_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("WENSHU_HOME", Path.home() / ".wenshu-hermes")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Wenshu MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "wenshu",
        instructions=(
            "Wenshu Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Wenshu MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
