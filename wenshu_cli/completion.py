"""Shell completion script generation for wenshu CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_wenshu_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# 文枢 bash completion
# Add to ~/.bashrc:
#   eval "$(wenshu completion bash)"

_wenshu_profiles() {{
    local profiles_dir="$HOME/.wenshu-hermes/profiles"
...
_wenshu_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_wenshu_profiles)" -- "$cur"))
        return
    fi
...
complete -F _wenshu_completion wenshu
"""
...
_wenshu_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.wenshu-hermes/profiles" ]]; then
        profiles+=($HOME/.wenshu-hermes/profiles/*(N/:t))
    fi
...
_wenshu() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_wenshu_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'wenshu command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# 文枢 fish completion",
        "# Add to your config:",
        "#   wenshu completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __wenshu_profiles",
        "    echo default",
        "    if test -d $HOME/.wenshu-hermes/profiles",
        "        for d in $HOME/.wenshu-hermes/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c wenshu -f -s p -l profile"
        " -d 'Profile name' -xa '(__wenshu_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__wenshu_profiles)' -d 'Profile name'"
                )
