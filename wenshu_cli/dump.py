"""
Dump command for wenshu CLI.

Outputs a compact, plain-text summary of the user's 文枢 setup
...
from wenshu_cli.config import get_wenshu_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.wenshu-hermes/.env.
    ...
    exports. ``wenshu debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.wenshu_build_sha`` by the Dockerfile's
    ``WENSHU_GIT_SHA`` build-arg (see ``wenshu_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``wenshu dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from wenshu_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(wenshu_home: Path) -> int:
...
def _cron_summary(wenshu_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = wenshu_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"wenshu_home:      {display_wenshu_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.wenshu-hermes/.env: a managed
        ...
        # A credential added via `wenshu auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `wenshu auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["wenshu-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(wenshu_home)}")
    lines.append(f"  skills:             {_count_skills(wenshu_home)}")
