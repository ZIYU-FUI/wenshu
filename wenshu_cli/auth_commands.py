...
def _get_custom_provider_names() -> list:
    ...
    try:
        from wenshu_cli.config import get_compatible_custom_providers, load_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_wenshu_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:wenshu_pkce",
    ...
    if provider == "nous":
        ...
        # <wenshu-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `wenshu --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``wenshu auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``wenshu auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`wenshu auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Wenshu reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `wenshu auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `wenshu auth` is called bare."""
