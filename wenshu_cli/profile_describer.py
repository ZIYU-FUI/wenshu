...
Used by ``wenshu profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``wenshu_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the 文枢 kanban board.
...
Rules:
  ...
  - Never write "文枢 profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_wenshu_home())
        else:
