"""On-demand supply-chain audit for 文枢 installs.

Scans three surfaces a 文枢 user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The 文枢 venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.wenshu-hermes/plugins``
...
def _discover_plugins(wenshu_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.wenshu-hermes/plugins``.
...
def run_audit(
    *,
    ...
    home = wenshu_home or Path(get_wenshu_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, wenshu_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(wenshu_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `wenshu security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, wenshu_home=home
    )
    if total == 0:
