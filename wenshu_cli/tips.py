...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or wenshu -c.",
    ...
    "Ctrl+Z suspends 文枢 to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "wenshu -c resumes your most recent CLI session. wenshu -c \"project name\" resumes by title.",
    "wenshu -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "wenshu -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "wenshu chat -t web,terminal enables only specific toolsets for a focused session.",
    "wenshu chat -s github-pr-workflow preloads a skill at launch.",
    "wenshu chat -q \"query\" runs a single non-interactive query and exits.",
    "wenshu chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "wenshu chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "wenshu --yolo bypasses all dangerous command approval prompts for the entire session.",
    "wenshu chat --source telegram tags the session for filtering in wenshu sessions list.",
    "wenshu -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "wenshu doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "wenshu dump outputs a compact setup summary — great for bug reports.",
    "wenshu config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "wenshu config edit opens config.yaml in your default editor.",
    "wenshu config check scans for missing or stale configuration options.",
    "wenshu sessions browse opens an interactive session picker with search.",
    "wenshu sessions stats shows session counts by platform and database size.",
    "wenshu sessions prune --older-than 30 cleans up old sessions.",
    "wenshu skills search react --source skills-sh searches the skills.sh public directory.",
    "wenshu skills check scans installed hub skills for upstream updates.",
    "wenshu skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "wenshu skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "wenshu mcp add github --command npx adds MCP servers from the command line.",
    "wenshu mcp serve runs 文枢 itself as an MCP server for other agents.",
    "wenshu auth add lets you add multiple API keys for credential pool rotation.",
    "wenshu completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "wenshu logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "wenshu backup creates a zip backup of your entire 文枢 home directory.",
    "wenshu profile create coder creates an isolated profile that becomes its own command.",
    "wenshu profile create work --clone copies your current config and keys to a new profile.",
    "wenshu update syncs new bundled skills to ALL profiles automatically.",
    "wenshu gateway install sets up 文枢 as a system service (systemd/launchd).",
    "wenshu memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "wenshu webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: wenshu tools disables unused tools, wenshu skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "wenshu models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as wenshu -w).",
    ...
    "WENSHU_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call 文枢 tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'wenshu profile create coder' creates the 'coder' command.",
    "wenshu profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, 文枢 prints a resume command with session ID and stats.",
    "wenshu sessions export backup.jsonl exports all sessions for backup or analysis.",
    "wenshu -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "wenshu skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .wenshu-hermes/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: wenshu cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "文枢 runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "wenshu gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to 文枢 via a local macOS server.",
    ...
    "Context files (.wenshu.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "文枢 loads project context from .wenshu.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "wenshu mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "wenshu mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "wenshu mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — wenshu tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.wenshu-hermes/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "wenshu chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "wenshu plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "wenshu acp runs 文枢 as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "WENSHU_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "wenshu auth add nous or wenshu auth add openai-codex sets up OAuth-based providers.",
    ...
    "wenshu status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.wenshu-hermes/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make 文枢 your own.",
    ...
    "Before a gateway session reset, 文枢 auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.wenshu-hermes/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope 文枢 state via WENSHU_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "WENSHU_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in WENSHU_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.wenshu-hermes/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'wenshu profile create ls' would be rejected.",
    "wenshu profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "wenshu claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "WENSHU_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via wenshu tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.wenshu-hermes/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.wenshu-hermes/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — 文枢 auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of 文枢 config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while 文枢 is working.',
    ...
    '/reload reloads ~/.wenshu-hermes/.env into the running session — pick up new API keys without restarting.',
    ...
    "WENSHU_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.wenshu-hermes/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.wenshu-hermes/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'wenshu curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "wenshu curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'wenshu curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'wenshu auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'WENSHU_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "WENSHU_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'wenshu dashboard launches a local web UI on an available 127.0.0.1 port — zero data leaves localhost.',
    'wenshu dashboard embeds the full 文枢 TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.wenshu-hermes/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.wenshu-hermes/dashboard-plugins/ — no npm build required.',
    ...
    'WENSHU_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'WENSHU_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'WENSHU_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'WENSHU_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.wenshu-hermes/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.wenshu-hermes/skills/ so drop-in skills appear without restarting the session.',
    ...
    'wenshu -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'wenshu chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'wenshu chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'wenshu chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "wenshu chat --source tool tags programmatic chats so they don't clutter wenshu sessions list.",
    'wenshu dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'wenshu sessions rename <ID> "new title" renames any past session; wenshu sessions delete <ID> removes one.',
    'wenshu import restores a full 文枢 backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'wenshu fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'wenshu pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'wenshu setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'wenshu status --deep runs the full health sweep across every component; plain wenshu status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'WENSHU_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'WENSHU_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.wenshu-hermes/plugins/ — trust-gated by design.',
    "WENSHU_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'WENSHU_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'WENSHU_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'WENSHU_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'WENSHU_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'WENSHU_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'WENSHU_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'WENSHU_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'WENSHU_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'WENSHU_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'WENSHU_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes 文枢 block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from wenshu sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.wenshu-hermes/dashboard-plugins/.',
]
