"""Slash command definitions and autocomplete for the 文枢 CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of 文枢 config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal 文枢 works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while 文枢 is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.wenshu-hermes/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update 文枢 to the latest version", "Info"),
    CommandDef("version", "Show 文枢 version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`wenshu_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/wenshu`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Wenshu ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full 文枢
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``wenshu skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``wenshu skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``wenshu skills list`` and
...
# unrelated command landed. These claim their slots right after /wenshu,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /wenshu <command>).
...
# native slot, the alias spelling stays reachable via /wenshu reset).
...
# ``/wenshu <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/wenshu" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /wenshu topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /wenshu moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /wenshu debug on Slack.
_SLACK_VIA_WENSHU_ONLY = frozenset({"topup", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/wenshu <verb>`` subcommand.
    ...
    can still reach them via ``/wenshu <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/wenshu`` is always reserved as the first entry so the
    legacy ``/wenshu <subcommand>`` form keeps working for anything that
    ...
    # Reserve /wenshu as the catch-all top-level command.
    entries.append(("wenshu", "Talk to 文枢 or run a subcommand", "[subcommand] [args]"))
    seen.add("wenshu")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_WENSHU_ONLY:
            # Intentionally Slack-via-/wenshu only (see _SLACK_VIA_WENSHU_ONLY).
            return
    ...
    # everything except /wenshu, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://wenshu-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /wenshu handler.

    Maps both canonical names and aliases so /wenshu bg do stuff works
    the same as /wenshu background do stuff.

    Plugin-registered slash commands are included so ``/wenshu <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from wenshu_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
