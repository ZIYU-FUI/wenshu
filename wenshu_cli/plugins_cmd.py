"""``wenshu plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.wenshu-hermes/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other 文枢 subprocess resolution: :func:`shutil.which` first,
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from wenshu_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_wenshu_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_wenshu_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_wenshu_home()}/.env later)[/dim]")
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update 文枢.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `wenshu plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid 文枢 plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `wenshu plugins enable {installed_name}` to activate.[/dim]",
        )
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Wenshu core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _resolve_tool_override_grant(
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`wenshu plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]"
        )
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    plugin directory under ``~/.wenshu-hermes/plugins``. Include package metadata here
    so ``wenshu plugins list`` can show and enable them.
    """
    from wenshu_cli.plugins import ENTRY_POINTS_GROUP
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] wenshu plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] wenshu plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] wenshu plugins")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as 文枢
    ...
    try:
        from wenshu_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``wenshu plugins enable web/firecrawl`` cleared
