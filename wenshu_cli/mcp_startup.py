...
def _resolve_discovery_timeout(explicit: "float | None") -> float:
    ...
    if explicit is not None:
        return explicit
    try:
        from wenshu_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    The desktop app, dashboard WebSocket sidecar, and ``wenshu dashboard``
    all populate THIS module's ``_mcp_discovery_thread``, so the late-refresh
    scheduler consults this state to decide whether a slow server's tools are
