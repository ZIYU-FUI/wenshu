"""Unified provider-credential lifecycle across every store 文枢 reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.wenshu-hermes/.env``                     — the canonical secret store
    2. ``~/.wenshu-hermes/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.wenshu-hermes/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `wenshu auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``wenshu auth add``.
    """
    from wenshu_cli.config import load_env, save_env_value
