...
Used by ``wenshu kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``wenshu_cli/goals.py`` — same aux
...
WENSHU_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("WENSHU_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the 文枢 board.
...
def _profile_author() -> str:
    """Mirror of ``wenshu_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("WENSHU_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=WENSHU_KANBAN_SPECIFY_MAX_TOKENS,
