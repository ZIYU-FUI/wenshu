"""Migrate 文枢' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads 文枢' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  文枢 mcp_servers.<n>.command/args/env  → codex stdio transport
  文枢 mcp_servers.<n>.url/headers       → codex streamable_http transport
  文枢 mcp_servers.<n>.timeout           → codex tool_timeout_sec
  文枢 mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Wenshu-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. 文枢' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in 文枢 config.")
...
# Wenshu keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Wenshu' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, wenshu_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one 文枢 MCP server config to the codex inline-table dict
    ...
    if not isinstance(wenshu_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(wenshu_cfg.get("command"))
    has_url = bool(wenshu_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (WENSHU_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by 文枢)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert 文枢' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before 文枢' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior 文枢
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="wenshu-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a WENSHU_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed wenshu-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.wenshu`` symlink that happens to live under ``/private/var/folders``
...
def _build_wenshu_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches 文枢' own
    ...
    so a wenshu installed under /opt/, /usr/local/, or a venv all work.
    WENSHU_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # WENSHU_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_wenshu_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever WENSHU_HOME its launcher sets
    ...
    # — that would override the launcher's WENSHU_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("WENSHU_HOME", tmp_path) would
    ...
    wenshu_home = os.environ.get("WENSHU_HOME") or ""
    if wenshu_home and _looks_like_test_tempdir(wenshu_home):
        wenshu_home = ""
    if wenshu_home:
        env["WENSHU_HOME"] = wenshu_home
    # PYTHONPATH passes through so a worktree-launched wenshu finds the
    ...
    env["WENSHU_QUIET"] = "1"
    env["WENSHU_REDACT_SECRETS"] = env.get("WENSHU_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_wenshu_tools: bool = True,
) -> MigrationReport:
    """Translate 文枢 mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        wenshu_config: full ~/.wenshu-hermes/config.yaml dict
        ...
        expose_wenshu_tools: when True (default), register 文枢' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into 文枢 for tools
    ...
    wenshu_servers = (wenshu_config or {}).get("mcp_servers") or {}
    if not isinstance(wenshu_servers, dict):
        report.errors.append(
            "mcp_servers in 文枢 config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in wenshu_servers.items():
    ...
    # Inject Wenshu' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Wenshu for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/wenshu_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_wenshu_tools:
        translated["wenshu-tools"] = _build_wenshu_tools_mcp_entry()
        if "wenshu-tools" not in report.migrated:
            report.migrated.append("wenshu-tools")
