"""
Single source of truth for provider identity in 文枢.
...
2. **文枢 overlays** — transport type, auth patterns, aggregator flags,
...
# -- Wenshu overlay ----------------------------------------------------------
# Wenshu-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class WenshuOverlay:
    """Wenshu-specific provider metadata layered on top of models.dev."""
...
WENSHU_OVERLAYS: Dict[str, WenshuOverlay] = {
    "moa": WenshuOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": WenshuOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": WenshuOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": WenshuOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": WenshuOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": WenshuOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="WENSHU_QWEN_BASE_URL",
    ),
    "lmstudio": WenshuOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": WenshuOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": WenshuOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": WenshuOverlay(
    ...
    "kimi-for-coding": WenshuOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": WenshuOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": WenshuOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": WenshuOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": WenshuOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": WenshuOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": WenshuOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": WenshuOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": WenshuOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": WenshuOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": WenshuOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": WenshuOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": WenshuOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": WenshuOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": WenshuOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": WenshuOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": WenshuOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "wenshu", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. 文枢 overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + 文枢 overlay
    ...
    overlay = WENSHU_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + wenshu extra
    ...
    if overlay is not None:
        # Wenshu-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in WENSHU_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Wenshu provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from wenshu_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
