...
The provider list shown by ``wenshu model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`wenshu_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``wenshu model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`wenshu_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``wenshu model`` shows.
"""
...
# wenshu_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `wenshu model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``wenshu model`` universe.
    ...
    # Wenshu overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = WENSHU_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Wenshu overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
