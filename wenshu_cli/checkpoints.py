"""`wenshu checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.wenshu-hermes/checkpoints/``.  Actions:

    wenshu checkpoints               # same as `status`
    wenshu checkpoints status        # total size, project count, breakdown
    wenshu checkpoints list          # per-project checkpoint counts + workdir
    wenshu checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    wenshu checkpoints clear [-f]    # nuke the entire base (asks first)
    wenshu checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    wenshu checkpoints
    wenshu checkpoints prune --retention-days 3 --max-size-mb 200
    wenshu checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: wenshu checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``wenshu checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `wenshu checkpoints` → status
