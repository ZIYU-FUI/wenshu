...
def _add_server_runtime_args(parser) -> None:
    ...
    """
    parser.add_argument(
        "--port", type=int, default=0, help="Port (default 0: auto-assign an available OS port)"
    )
    parser.add_argument(
...
def build_dashboard_parser(
    ...
    # browser. It exists so the Wenshu Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the 文枢 backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the 文枢 backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # Nested subparser so bare `wenshu dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "WENSHU_DASHBOARD_OAUTH_CLIENT_ID into ~/.wenshu-hermes/.env, and prints "
            "how to engage the login gate. Requires being logged in (wenshu setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://wenshu.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via WENSHU_DASHBOARD_PORTAL_URL. Mainly for "
