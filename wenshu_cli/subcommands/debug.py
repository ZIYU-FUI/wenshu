"""``wenshu debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for 文枢. Use 'wenshu debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    wenshu debug share              Upload debug report (asks for confirmation)
    wenshu debug share --yes        Skip confirmation (for scripts/CI)
    wenshu debug share --lines 500  Include more log lines
    wenshu debug share --expire 30  Keep paste for 30 days
    wenshu debug share --local      Print report locally (no upload)
    wenshu debug share --no-redact  Disable upload-time secret redaction
    wenshu debug share --nous       Upload to Nous-internal storage (private)
    wenshu debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'wenshu debug share'",
    )
