"""
Status command for wenshu CLI.

Shows the status of all 文枢 components.
"""
...
from wenshu_cli.auth import AuthError, resolve_provider
from wenshu_cli.colors import Colors, color
from wenshu_cli.config import get_env_path, get_env_value, get_wenshu_home, load_config
from wenshu_cli.models import provider_label
from wenshu_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from wenshu_cli.nous_subscription import get_nous_subscription_features
from wenshu_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``wenshu config``'s
...
def show_status(args):
    """Show status of all 文枢 components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ 文枢 Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: wenshu portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: wenshu model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: wenshu auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: wenshu auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: wenshu model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from wenshu_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from wenshu_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   wenshu gateway")
    ...
    jobs_file = get_wenshu_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_wenshu_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'wenshu doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'wenshu setup' to configure", Colors.DIM))
    print()
