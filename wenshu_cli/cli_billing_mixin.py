...
``cli.py``'s ``WenshuCLI`` class. ``WenshuCLI`` inherits
...
Import discipline mirrors ``wenshu_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded; honors WENSHU_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _show_subscription(self):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                print("  Run `wenshu portal` to log in, then /subscription.")
            return
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from wenshu_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from wenshu_cli.nous_billing import (
            BillingError,
            BillingTransient,
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `wenshu portal` and enable terminal billing, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from wenshu_cli.auth import step_up_nous_billing_scope
        ...
        try:
            from wenshu_cli import nous_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `wenshu portal` to log in, then /topup.")
            return
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `wenshu portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `wenshu portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `wenshu portal` and enable terminal billing, then retry.")
            return
        confirm_choices = [
            ("yes", "Enable terminal billing", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        from wenshu_cli.nous_billing import BillingError, post_charge
