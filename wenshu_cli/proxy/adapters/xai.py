...
# xAI's public API is OpenAI-compatible for the endpoints Wenshu commonly
# uses. The Responses endpoint is included because Wenshu' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Wenshu-managed OAuth credentials."""

    auth_hint = "wenshu auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`wenshu auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`wenshu auth reset xai-oauth` or re-authenticate with "
                    "`wenshu auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `wenshu auth add xai-oauth --type oauth`."
            )
