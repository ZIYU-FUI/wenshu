"""CLI handlers for the ``wenshu proxy`` subcommand."""
...
from wenshu_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "wenshu proxy requires aiohttp. Install one of:\n"
        "  pip install 'wenshu-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"wenshu auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting 文枢 proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("文枢 proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: wenshu proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``wenshu proxy <subcommand>``."""
    ...
    print(
        "wenshu proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  wenshu proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
