"""
文枢 Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``wenshu desktop`` / ``wenshu gui``)
     Built inside the agent checkout under ``$WENSHU_HOME/wenshu-agent/``:
       ...
       - ``$WENSHU_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/文枢.app`` or ``~/Applications/文枢.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\文枢`` (NSIS per-user)
...
the app name ("文枢"), separate from ``$WENSHU_HOME``:
  - macOS:   ``~/Library/Application Support/文枢``
  - Windows: ``%APPDATA%\\文枢``
  - Linux:   ``$XDG_CONFIG_HOME/文枢`` (default ``~/.config/文枢``)
...
``wenshu uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(wenshu_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return wenshu_home / "wenshu-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "文枢"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "文枢"
...
def source_built_gui_artifacts(wenshu_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``wenshu desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``wenshu-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(wenshu_home)
    ...
    return [
        ...
        wenshu_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "文枢" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/文枢.app"),
            home / "Applications" / "文枢.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Wenshu).
            local_base / "Programs" / "文枢",
            # Older / alternate layout some builds used.
            local_base / "wenshu-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "文枢")
    else:
        ...
        paths += [
            data_base / "applications" / "wenshu.desktop",
            data_base / "applications" / "文枢.desktop",
        ]
    return paths


def agent_is_installed(wenshu_home: Path) -> bool:
    """Return True when a usable Python agent install exists under WENSHU_HOME.
    ...
    if (agent_root / "wenshu_cli").is_dir():
        return True
...
def gui_is_installed(wenshu_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(wenshu_home):
        if p.exists():
            return True
...
def gui_install_summary(wenshu_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = wenshu_home if wenshu_home is not None else get_wenshu_home()
...
def uninstall_gui(wenshu_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``wenshu-agent/wenshu_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$WENSHU_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove wenshu' or "
            "'sudo dnf remove wenshu'). AppImage builds are a single file you "
