"""
Top-level argparse construction for the wenshu CLI.
...
# argparse runs (it sets ``WENSHU_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``wenshu_cli.relaunch`` should carry over when
...
Examples:
    ...
    wenshu setup                  Run setup wizard
    wenshu logout                 Clear stored authentication
    wenshu auth add <provider>    Add a pooled credential
    wenshu auth list              List pooled credentials
    wenshu auth remove <p> <t>    Remove pooled credential by index, id, or label
    wenshu auth reset <provider>  Clear exhaustion status for a provider
    wenshu model                  Select default model
    wenshu fallback [list]        Show fallback provider chain
    wenshu fallback add           Add a fallback provider (same picker as `wenshu model`)
    wenshu fallback remove        Remove a fallback provider from the chain
    wenshu config                 View configuration
    wenshu config edit            Edit config in $EDITOR
    wenshu config set model gpt-4 Set a config value
    wenshu gateway                Run messaging gateway
    wenshu -s wenshu-agent-dev,github-auth
    wenshu -w                     Start in isolated git worktree
    wenshu gateway install        Install gateway background service
    wenshu sessions list          List past sessions
    wenshu sessions browse        Interactive session picker
    wenshu sessions rename ID T   Rename/title a session
    wenshu logs                   View agent.log (last 50 lines)
    wenshu logs -f                Follow agent.log in real time
    wenshu logs errors            View errors.log
    wenshu logs --since 1h        Lines from the last hour
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="wenshu",
        description="文枢 - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `wenshu chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to WENSHU_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.wenshu-hermes/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with 文枢",
    )
    ...
    # top-level parser: when the user writes `wenshu -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/wenshu_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also WENSHU_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.wenshu-hermes/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from 文枢 itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
