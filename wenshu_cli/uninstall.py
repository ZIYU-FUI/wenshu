...
- Keep data: Remove code but keep ~/.wenshu-hermes/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove 文枢 PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing wenshu-agent or wenshu PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Wenshu Agent" comment and following line
                if '# 文枢' in line or '# wenshu-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('wenshu' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing wenshu
                if 'wenshu' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the wenshu wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "wenshu",
        Path("/usr/local/bin/wenshu"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains wenshu_cli reference)
                content = wrapper.read_text()
                if 'wenshu_cli' in content or 'wenshu-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(wenshu_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``wenshu`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this 文枢 home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (wenshu_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``wenshu gateway run`` processes
    ...
    try:
        from wenshu_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from wenshu_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``WENSHU_HOME`` and ``WENSHU_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\wenshu\git\cmd``, ``%LOCALAPPDATA%\wenshu\git\bin``,
#      ``%LOCALAPPDATA%\wenshu\git\usr\bin``, ``%LOCALAPPDATA%\wenshu\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\wenshu\git\`` and Node to
#      ``%LOCALAPPDATA%\wenshu\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``wenshu dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\wenshu\gateway-service\`` and sometimes
...
def _wenshu_path_markers(wenshu_home: Path) -> list[str]:
    """Path-entry substrings that identify Wenshu-owned User-PATH entries."""
    root = str(wenshu_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare wenshu-agent install dir.
    markers = [root + "\\wenshu-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if WENSHU_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "wenshu".  We don't want to catch
    # unrelated entries like "cwenshu-foo" or "ephermeral", so we look for
    # backslash-wenshu as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(wenshu_home: Path) -> list[str]:
    """Strip Wenshu-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _wenshu_path_markers(wenshu_home)
            kept: list[str] = []
...
def remove_wenshu_env_vars_windows() -> list[str]:
    """Delete WENSHU_HOME and WENSHU_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("WENSHU_HOME", "WENSHU_GIT_BASH_PATH"):
                try:
...
def _is_default_wenshu_home(wenshu_home: Path) -> bool:
    """Return True when ``wenshu_home`` points at the default (non-profile) root."""
    try:
        from wenshu_constants import get_default_wenshu_root
        return wenshu_home.resolve() == get_default_wenshu_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its WENSHU_HOME directory.

    We shell out to ``wenshu -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current WENSHU_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m wenshu_cli.main` so we don't depend on a `wenshu`
    #    wrapper that may be half-removed mid-uninstall.
    wenshu_invocation = [_sys.executable, "-m", "wenshu_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                wenshu_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's WENSHU_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``wenshu uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$WENSHU_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from wenshu_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(wenshu_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ 文枢 Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No 文枢 Chat GUI installation was found.")
        print(f"  Checked: {wenshu_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The 文枢 agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(wenshu_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The 文枢 agent at {wenshu_home / 'wenshu-agent'}")
        print(f"  • Your config, sessions, and secrets under {wenshu_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The 文枢 agent is still installed. Run 'wenshu' to use the CLI,")
    print("or 'wenshu uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.wenshu-hermes/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.wenshu-hermes/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie WENSHU_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_wenshu_home(wenshu_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.wenshu data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ 文枢 Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {wenshu_home / 'config.yaml'}")
    print(f"  Secrets: {wenshu_home / '.env'}")
    print(f"  Data:    {wenshu_home / 'cron/'}, {wenshu_home / 'sessions/'}, {wenshu_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their WENSHU_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL 文枢 data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the 文枢 code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, wenshu_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • 文枢 PATH entries from shell configs / Windows User PATH")
    print("  • 文枢 wrapper scripts and 文枢-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • 文枢 config/data: {wenshu_home}")
        if _is_default_wenshu_home(wenshu_home):
        ...
        print(f"  • Keep 文枢 config/data: {wenshu_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``wenshu`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$WENSHU_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in wenshu_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\wenshu\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(wenshu_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No 文枢-owned PATH entries in User environment")

        log_info("Removing WENSHU_HOME / WENSHU_GIT_BASH_PATH User env vars...")
        removed_env = remove_wenshu_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No 文枢-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing wenshu command...")
    ...
    #     (only when they still point into this Wenshu home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing 文枢-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(wenshu_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No 文枢-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(wenshu_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE WENSHU_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from wenshu_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(wenshu_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.wenshu-hermes/, just remove the wenshu-agent subdir
            if wenshu_home in project_root.parents or project_root.parent == wenshu_home:
    ...
    #     under WENSHU_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(wenshu_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(wenshu_home)
    ...
    # 5. Optionally remove ~/.wenshu-hermes/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile WENSHU_HOME dirs live under
        ...
        try:
            if wenshu_home.exists():
                shutil.rmtree(wenshu_home)
                log_success(f"Removed {wenshu_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {wenshu_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {wenshu_home}")
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m wenshu_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import wenshu_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``wenshu_constants`` + ``wenshu_cli.colors``
    (and lazily ``wenshu_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m wenshu_cli.uninstall")
    parser.add_argument(
        "--mode",
