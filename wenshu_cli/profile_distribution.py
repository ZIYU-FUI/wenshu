"""Profile distributions — shareable, packaged 文枢 profiles via git.

A distribution is a 文枢 profile published as a git repository (or
...
* ``wenshu profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``wenshu skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``wenshu profile``, not a parallel tree):

    wenshu profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "wenshu-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            wenshu_requires=str(data.get("wenshu_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.wenshu_requires:
            out["wenshu_requires"] = self.wenshu_requires
        if self.author:
...
def check_wenshu_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires 文枢 {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this 文枢 distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``wenshu profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a 文枢 profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a 文枢 distribution."
        )

    # Version check up-front so we fail fast
    check_wenshu_requires(manifest.wenshu_requires, wenshu_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.wenshu).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="wenshu_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `wenshu profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`wenshu profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="wenshu_dist_update_") as tmp:
        plan = plan_install(
