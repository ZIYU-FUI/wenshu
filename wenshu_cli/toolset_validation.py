...
the tool registry or launching 文枢 (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``wenshu-cli`` to the non-existent ``wenshu``. ``resolve_toolset('wenshu')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``wenshu-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``wenshu`` instead of ``wenshu-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `wenshu tools` to reconfigure."
        )
    return warnings
