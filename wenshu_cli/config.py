"""
Configuration management for 文枢.

Config files are stored in ~/.wenshu-hermes/ for easy access:
- ~/.wenshu-hermes/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.wenshu-hermes/.env         - API keys and secrets

This module provides:
- wenshu config          - Show current configuration
- wenshu config edit     - Open config in editor
- wenshu config get      - Print a resolved configuration value
- wenshu config set      - Set a specific value
- wenshu config unset    - Remove a user configuration value
- wenshu config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``wenshu config set`` (which rewrites
    ...
    ``config.yaml`` in place: wenshu never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.wenshu-hermes/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``wenshu logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  wenshu config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Wenshu makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Wenshu itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``wenshu update``, the TUI build.
...
#   on every plugin install / ``wenshu update``.
...
# * ``WENSHU_HOME`` / ``WENSHU_PROFILE`` / ``WENSHU_CONFIG`` /
#   ``WENSHU_ENV`` — Wenshu runtime location flags. Writing these into
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or 文枢 runtime location "
            "(WENSHU_HOME, WENSHU_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.wenshu-hermes/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "WENSHU_TOOL_PROGRESS", "WENSHU_TOOL_PROGRESS_MODE",
...
from wenshu_cli.colors import Colors, color
from wenshu_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("WENSHU_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_wenshu_home() / ".managed"
...
def is_managed() -> bool:
    """Check if 文枢 is running in package-manager-managed mode.

    Two signals: the WENSHU_MANAGED env var (set by the systemd service),
    or a .managed marker file in WENSHU_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade wenshu-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``wenshu_cli/`` — i.e. the git checkout for source
    installs, ``/opt/wenshu`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$WENSHU_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how 文枢 was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$WENSHU_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. WENSHU_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.wenshu``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$WENSHU_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.wenshu:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$WENSHU_HOME``, a
    ...
    ``docker`` and ``wenshu update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/wenshu-agent`` image bakes a ``docker``
        stamp into ``/opt/wenshu`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $WENSHU_HOME.
    try:
    ...
    #    host install whose shared $WENSHU_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``wenshu update``.
    try:
        method = (
            (get_wenshu_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``wenshu_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$WENSHU_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/wenshu`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* 文枢 lives in a ``uv tool`` layout.

    ``uv tool install wenshu-agent`` places the install at
    ``.../uv/tools/wenshu-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``wenshu-agent`` happens to be uv-tool-installed on the machine while
    the *active* 文枢 is a regular pip/venv install, causing
    ``wenshu update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/wenshu-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade wenshu-agent"
    if method == "docker":
        return "docker pull nousresearch/wenshu-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade wenshu-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade wenshu-agent"
        return "pip install --upgrade wenshu-agent"
    return "wenshu update"
...
# banner, the TUI/desktop session info panel, and ``wenshu update``. NixOS
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``wenshu update`` / ``wenshu update --check`` so the wording — and the
...
# Long-form text for ``wenshu update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# wenshu_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Wenshu, it doesn't update
...
✗ ``wenshu update`` doesn't apply inside the Docker container.

文枢 runs as a published image (nousresearch/wenshu-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/wenshu-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate wenshu-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/wenshu-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/wenshu-agent/tags
  • Your config and session history live under ``$WENSHU_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``wenshu update`` inside Docker.
...
def format_managed_message(action: str = "modify this 文枢 installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("WENSHU_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this 文枢 installation is managed by NixOS "
            f"(WENSHU_MANAGED={env_hint}).\n"
            "Edit services.wenshu-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this 文枢 installation is managed by Homebrew "
            f"(WENSHU_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade wenshu-agent"
        )

    return (
        f"Cannot {action}: this 文枢 installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall 文枢."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from WENSHU_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, wenshu_bin
    or None if container mode is not active, we're already inside the
    container, or WENSHU_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("WENSHU_DEV") == "1":
        return None
    ...
    container_mode_file = get_wenshu_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "wenshu-agent")
    exec_user = info.get("exec_user", "wenshu")
    wenshu_bin = info.get("wenshu_bin", "/data/current-package/bin/wenshu")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "wenshu_bin": wenshu_bin,
    }
...
# Re-export from wenshu_constants — canonical definition lives there.
from wenshu_constants import get_wenshu_home, get_process_wenshu_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_wenshu_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_wenshu_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_wenshu_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the WENSHU_UID / WENSHU_GID env vars set by Docker deployments.

    Docker containers running 文枢 commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level WENSHU_HOME once, but
    subdirectories created at runtime by ``ensure_wenshu_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("WENSHU_UID", "").strip()
    gid_str = os.environ.get("WENSHU_GID", "").strip()
    try:
...
def _chown_to_wenshu_uid(path) -> None:
    """Chown ``path`` to ``WENSHU_UID:WENSHU_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_wenshu_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_wenshu_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the wenshu group can
    share state with the gateway service.

    The mode can be overridden via the WENSHU_HOME_MODE environment variable
    (e.g. WENSHU_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse WENSHU_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``WENSHU_UID``/``WENSHU_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("WENSHU_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_wenshu_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When 文枢 runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("WENSHU_CONTAINER") or os.environ.get("WENSHU_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set WENSHU_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into WENSHU_HOME, upgrading legacy empty templates.
...
# ensure_wenshu_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_WENSHU_HOME_ENSURED: set = set()


def ensure_wenshu_home():
    """Ensure ~/.wenshu directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``WENSHU_HOME`` we only re-run
    ...
    ``get_wenshu_home()`` and therefore re-run for the new path.
    """
    home = get_wenshu_home()
    key = str(home)

    if key in _WENSHU_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``wenshu profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old WENSHU_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_wenshu_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _WENSHU_HOME_ENSURED.add(key)


def _ensure_wenshu_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"WENSHU_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Wenshu-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Wenshu
        ...
        # identity slot (SOUL.md). Empty by default. The WENSHU_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Wenshu adds a coding operating brief
        ...
        # ``WENSHU_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             WENSHU_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force WENSHU_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Wenshu auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Wenshu sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Wenshu
        ...
        # bundled Wenshu image, which drops to the `wenshu` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Wenshu sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Wenshu should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``wenshu chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.wenshu-hermes/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: wenshu sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .wenshu.md, or .cursorrules before Wenshu applies
    ...
    # single read_file page exceeds these limits, Wenshu truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Wenshu' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); wenshu =
                                      # Wenshu' compression threshold triggers
    ...
    # See tools/kanban_tools.py and wenshu_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``wenshu dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``wenshu kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``wenshu kanban decompose`` and the kanban
        ...
        # ``wenshu profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `wenshu model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # can set `path`/`paths` to WENSHU_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/wenshu`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `wenshu pets`. Disabled until
        ...
        "pet": {
            ...
            # get_wenshu_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # raw config loader owns explicit defaults for these settings.
        ...
        # ``WENSHU_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``WENSHU_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # optional path prefix (e.g. ``https://example.com/wenshu``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Wenshu appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.wenshu-hermes/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.wenshu-hermes/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.wenshu-hermes/cache/delegation/
    ...
    # response. If not, Wenshu feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Wenshu auto-pauses the goal and
    ...
    "moa": {
        ...
        # <wenshu_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.wenshu-hermes/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${WENSHU_SKILL_DIR} and ${WENSHU_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `wenshu curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `wenshu curator run --consolidate` overrides
        ...
        # normally restored on every `wenshu update`, so pruning them only
        ...
        # skipped), snapshot ~/.wenshu-hermes/skills/ into
        # ~/.wenshu-hermes/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `wenshu curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for wenshu-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Wenshu Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (WENSHU_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Wenshu' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.wenshu-hermes/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or WENSHU_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `wenshu doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``wenshu_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Wenshu to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $WENSHU_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via WENSHU_CRON_MAX_PARALLEL env var.
        ...
        # WENSHU_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `wenshu -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `wenshu` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `wenshu kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     wenshu-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Wenshu tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.wenshu-hermes/logs/.
    ...
    # update model picker lists without shipping a wenshu-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `wenshu model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal WENSHU_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.wenshu-hermes/sessions/sessions.json entirely.
        ...
        # the WENSHU_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.wenshu.gateway` that defenses 1-2 don't
        ...
        # ``WENSHU_GATEWAY_MAX_STARTS`` / ``WENSHU_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.wenshu-hermes/.env,
        ...
        # behavior: files must live under the Wenshu cache, under
        ...
        # user. Bridged to WENSHU_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Wenshu
        # cache (~/.wenshu-hermes/cache/{documents,images,audio,video,screenshots})
        ...
        # to WENSHU_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # WENSHU_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to WENSHU_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.wenshu-hermes/state.db.
    ...
    "sessions": {
        ...
        # default of ``wenshu sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.wenshu-hermes/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        ...
        #     DBs) into <WENSHU_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``wenshu backup``-style zip
        #     of WENSHU_HOME into <WENSHU_HOME>/backups/, restorable with
        #     ``wenshu import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `wenshu update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<WENSHU_HOME>/lsp/bin/`` on first use.
    ...
    # `wenshu tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.wenshu-hermes/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.wenshu-hermes/.env (or your shell) and never in config.yaml.
            ...
            # ~/.wenshu-hermes/bin/ on first use.  When False you must install
            ...
            # `wenshu secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.wenshu-hermes/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Wenshu disables it for our users unless they
        # explicitly opt in here. When false (default), Wenshu sets
    ...
    # Wenshu Desktop (Electron app) launch options. These only affect
    # `wenshu desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the WENSHU_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials 文枢 mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'wenshu model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the 文枢 auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use 文枢, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @wenshu:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. WENSHU_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #wenshu)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: wenshu-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'wenshu-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote 文枢 API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote 文枢 API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote 文枢 API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # WENSHU_TOOL_PROGRESS and WENSHU_TOOL_PROGRESS_MODE are deprecated —
    ...
    "WENSHU_EPHEMERAL_SYSTEM_PROMPT": {
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `wenshu config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.wenshu.config`` entries, then checks
    ...
    try:
        ...
        # should never break `wenshu update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Wenshu' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Wenshu versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``wenshu_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (wenshu_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (wenshu_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (wenshu_cli/setup.py)
    "known_plugin_toolsets", # written/read by wenshu_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'wenshu setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — 文枢 won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, wenshu send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'wenshu doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("WENSHU_HOME", "~/.wenshu-hermes")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "wenshu update / wenshu -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("WENSHU_TOOL_PROGRESS")
            old_mode = get_env_value("WENSHU_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from WENSHU_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from WENSHU_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("WENSHU_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from WENSHU_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$WENSHU_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_wenshu_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `wenshu plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `wenshu curator status` has no
    ...
    #   3. Creates `~/.wenshu-hermes/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_wenshu_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_wenshu_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `wenshu config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `wenshu config set`"
                    )
    ...
    # behavior set it to true explicitly via `wenshu config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from wenshu_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from wenshu_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: wenshu config set <key> <value>")
    ...
    # `wenshu update`, but only the version bump is persisted.
    ...
    # metadata.wenshu.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_wenshu_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``wenshu config set``
    ...
    backends) — while display paths (``wenshu status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _wenshu_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.wenshu-hermes/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.wenshu-hermes/config.yaml.
    ...
    (which change ``WENSHU_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/wenshu/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_wenshu_dotenv()
...
#   openai-codex (OAuth — wenshu auth) — OpenAI Codex
#   nous         (OAuth — wenshu auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.wenshu-hermes/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset 文枢 writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.wenshu-hermes/.env.
    ...
    interactive menu render (`wenshu tools`, `wenshu setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `wenshu tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real 文枢 env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.wenshu-hermes/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.wenshu-hermes/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.wenshu-hermes/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.wenshu-hermes/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to 文枢 — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Wenshu vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.wenshu-hermes/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.wenshu-hermes/.env`` over ``os.environ``.

    Used for Wenshu-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ 文枢 Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from wenshu_cli.auth import get_anthropic_key
    ...
    # Warn on stale WENSHU_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("WENSHU_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale WENSHU_MAX_ITERATIONS={_env_ghost} "
                f"(run 'wenshu doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  wenshu config edit     # Edit config file", Colors.DIM))
    print(color("  wenshu config set <key> <value>", Colors.DIM))
    print(color("  wenshu setup           # Run setup wizard", Colors.DIM))
    print()
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``wenshu config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``wenshu config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``wenshu config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `wenshu config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `wenshu config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `wenshu config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: wenshu config get <key> [--json]")
            print()
            print("Examples:")
            print("  wenshu config get model")
            print("  wenshu config get terminal.backend")
            print("  wenshu config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: wenshu config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  wenshu config set model anthropic/claude-sonnet-4")
            print("  wenshu config set terminal.backend docker")
            print("  wenshu config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: wenshu config unset <key>")
            print()
            print("Examples:")
            print("  wenshu config unset model")
            print("  wenshu config unset terminal.backend")
            print("  wenshu config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'wenshu config migrate' to add them")
        ...
        print("  wenshu config           Show current configuration")
        print("  wenshu config edit      Open config in editor")
        print("  wenshu config get <key>          Print a resolved config value")
        print("  wenshu config set <key> <value>   Set a config value")
        print("  wenshu config unset <key>        Remove a config value")
        print("  wenshu config check     Check for missing/outdated config")
        print("  wenshu config migrate   Update config with new options")
        print("  wenshu config path      Show config file path")
        print("  wenshu config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``wenshu config`` UI so users
