...
Entry points (``cli.py`` ``main``, ``wenshu_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``WENSHU_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("WENSHU_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Wenshu-managed Git install directories so
    ...
    # already-running shells don't see the change — which means wenshu
    ...
    # install without Wenshu-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Wenshu's interactive
...
def _default_windows_editor() -> str:
    ...
    ``wenshu config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Wenshu-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\wenshu\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``wenshu`` from right after install) retain their old PATH.
    ...
    Patch-up strategy: add the known Wenshu-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "wenshu", "git", "cmd"),
        os.path.join(local_appdata, "wenshu", "git", "bin"),
        os.path.join(local_appdata, "wenshu", "git", "usr", "bin"),
        # Wenshu venv Scripts directory — host of the wenshu.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes wenshu, but harmless to include.
        os.path.join(local_appdata, "wenshu", "wenshu-agent", "venv", "Scripts"),
