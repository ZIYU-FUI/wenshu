"""Windows ConPTY bridge for the `wenshu dashboard` chat tab.

Drop-in counterpart to ``wenshu_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``wenshu_cli.web_server`` consumes: ``spawn``, ``read``,
