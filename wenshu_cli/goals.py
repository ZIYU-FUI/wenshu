"""Persistent session goals — the Ralph loop for 文枢.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, 文枢 feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.WenshuCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current WENSHU_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``wenshu_home`` path so profile switches still pick up the right DB.
        ...
        if state.consecutive_transport_failures >= DEFAULT_MAX_CONSECUTIVE_TRANSPORT_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "Check the goal_judge provider/key in ~/.wenshu-hermes/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: deepseek\n"
        ...
        if state.consecutive_parse_failures >= DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES:
            ...
            return {
                ...
                "message": (
                    ...
                    "model in ~/.wenshu-hermes/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: openrouter\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``wenshu -p <profile> chat -q "work kanban task <id>"``). The worker's
