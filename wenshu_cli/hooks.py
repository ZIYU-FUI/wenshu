"""wenshu hooks — inspect and manage shell-script hooks.

Usage::

    wenshu hooks list
    wenshu hooks test <event> [--for-tool X] [--payload-file F]
    wenshu hooks revoke <command>
    wenshu hooks doctor

Consent records live under ``~/.wenshu-hermes/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.wenshu-hermes/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``wenshu hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: wenshu hooks {list|test|revoke|doctor}")
        print("Run 'wenshu hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.wenshu-hermes/config.yaml.")
        print("See `wenshu hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `wenshu hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `wenshu hooks test` and `wenshu hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (文枢 wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `wenshu hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `wenshu hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `wenshu hooks doctor`.")
