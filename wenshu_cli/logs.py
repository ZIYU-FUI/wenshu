"""``wenshu logs`` — view and filter 文枢 log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.wenshu-hermes/logs/``.

Usage examples::

    wenshu logs                    # last 50 lines of agent.log
    wenshu logs -f                 # follow agent.log in real time
    wenshu logs errors             # last 50 lines of errors.log
    wenshu logs gateway -n 100    # last 100 lines of gateway.log
    wenshu logs gui -f            # follow gui.log (dashboard/pty/ws)
    wenshu logs desktop -f        # follow desktop.log (Electron app boot/backend)
    wenshu logs --level WARNING    # only WARNING+ lines
    wenshu logs --session abc123   # filter by session ID substring
    wenshu logs --component tools  # only tool-related lines
    wenshu logs --since 1h         # lines from the last hour
    wenshu logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_wenshu_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when 文枢 runs — try 'wenshu chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_wenshu_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_wenshu_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_wenshu_home()}/logs/")
        return

    print(f"Log files in {display_wenshu_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'wenshu chat' to generate logs)")
