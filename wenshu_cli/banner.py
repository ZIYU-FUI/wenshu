...
Pure display functions with no WenshuCLI state dependency.
"""
import json
import logging
import os
...
from wenshu_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built wenshu — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/hermes-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/wenshu-agent"
...
def _fetch_pypi_latest(package: str = "wenshu-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a 文枢 update is available.

    Two paths: if ``WENSHU_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = wenshu_home / ".update_check"
    embedded_rev = os.environ.get("WENSHU_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # WENSHU_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `wenshu update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/wenshu/update/check`
    ...
    try:
        from wenshu_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $WENSHU_HOME/wenshu-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = wenshu_home / "wenshu-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active 文枢 git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$WENSHU_HOME/wenshu-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``wenshu_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from wenshu_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    文枢 checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/wenshu-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"文枢 v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else WENSHU_CADUCEUS
    except Exception:
        _bskin = None
        _hero = WENSHU_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from wenshu_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from wenshu_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built wenshu; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else WENSHU_AGENT_LOGO
        console.print(_logo)
        console.print()
