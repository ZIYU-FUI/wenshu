"""MCP picker — interactive `wenshu mcp picker` (also the default `wenshu mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``wenshu mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `wenshu plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from wenshu_cli.cli_output import prompt_yes_no
from wenshu_cli.curses_ui import curses_single_select
from wenshu_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new 文枢 session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from wenshu_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `wenshu mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: wenshu mcp install <name>    Picker: wenshu mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Wenshu is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer 文枢 — run `wenshu update` "
...
def show_catalog() -> None:
    """`wenshu mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`wenshu mcp picker` (and default `wenshu mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`wenshu mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `wenshu mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
