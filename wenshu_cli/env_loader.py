"""Helpers for loading 文枢 .env files consistently across entrypoints."""
...
# load_wenshu_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_wenshu_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_wenshu_dotenv().
# Used by setup / `wenshu model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# WENSHU_HOME paths we've already pulled external secrets for during this
# process.  ``load_wenshu_dotenv()`` is called at module-import time from
# several hot modules (cli.py, wenshu_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_wenshu_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which WENSHU_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `wenshu setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``wenshu_cli.config._sanitize_env_lines`` which
    already knows all valid 文枢 env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from wenshu_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_wenshu_dotenv(
    *,
    wenshu_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load 文枢 environment files with user config taking precedence.

    Behavior:
    - `~/.wenshu-hermes/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(wenshu_home or os.getenv("WENSHU_HOME", Path.home() / ".wenshu-hermes"))
    ...
    #   EnvironmentFile=-/path/to/.wenshu-hermes/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of WENSHU_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of 文枢 reads
    ...
    owns the once-per-WENSHU_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_wenshu_dotenv()`` runs at import
    time from several hot modules (cli.py, wenshu_cli/main.py,
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `wenshu model`
