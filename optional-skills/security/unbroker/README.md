...
The design is **Wenshu-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
wenshu skills install official/security/unbroker
```

Then start a new Wenshu session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Wenshu session:
...
- **PII handling.** Dossiers live under the Wenshu home directory (`0600`, optionally
