...
All per-subject data lives under PDD_DATA_DIR (default: $WENSHU_HOME/unbroker),
which is the same trust boundary Wenshu uses for .env and OAuth tokens.
"""
...
def wenshu_home() -> Path:
    return Path(os.environ.get("WENSHU_HOME") or (Path.home() / ".wenshu-hermes"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else wenshu_home() / "unbroker"
