...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("WENSHU_HOME", os.path.expanduser("~/.wenshu-hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("WENSHU_HOME", Path.home() / ".wenshu-hermes")) / "skills" / "red-teaming" / "godmode"
...
WENSHU_HOME = Path(os.getenv("WENSHU_HOME", Path.home() / ".wenshu-hermes"))
CONFIG_PATH = WENSHU_HOME / "config.yaml"
PREFILL_PATH = WENSHU_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Wenshu models — already uncensored, just needs clean prompt
    "wenshu": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "wenshu" in model_lower or "nous" in model_lower:
        return "wenshu"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Wenshu config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.wenshu-hermes/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Wenshu for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Wenshu for changes to take effect.")
