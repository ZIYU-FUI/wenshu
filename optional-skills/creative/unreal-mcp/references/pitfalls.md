...
### 1. Start order: editor first, Wenshu second

Wenshu probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Wenshu session.
...
restart the editor, restart the Wenshu session.
...
server fails to bind (Output Log) or Wenshu' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.wenshu-hermes/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Wenshu

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Wenshu' connection
lives in `~/.wenshu-hermes/config.yaml` via `wenshu mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Wenshu — just don't
mistake it for the Wenshu setup step.
...
explicitly warns against overlapping calls. Wenshu executes same-turn tool
...
### 9. Timeouts: Wenshu gives up before Unreal does

Wenshu' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Wenshu runs elsewhere (SSH backend,
