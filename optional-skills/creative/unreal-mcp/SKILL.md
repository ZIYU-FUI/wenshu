...
Companion skill for the `unreal-engine` entry in the Wenshu MCP catalog. The
...
Two halves, in this order: the editor side must be up before Wenshu connects.
...
### One-time, Wenshu side

    wenshu mcp install unreal-engine
...
`~/.wenshu-hermes/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Wenshu — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Wenshu connects
...
2. Start the Wenshu session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Wenshu
they appear as:
...
then happens at `wenshu mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Wenshu runs batched calls
...
- **Start order matters.** Editor + server up first, then the Wenshu
...
- **Timeouts on long operations.** Wenshu' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.wenshu-hermes/config.yaml` for
