...
wenshu kanban watch --tenant <project-slug>

# Snapshot of the board
wenshu kanban list --tenant <project-slug>
wenshu kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
wenshu kanban stats --tenant <project-slug>

# Visual dashboard (browser)
wenshu dashboard

# Inspect a specific task (includes comments + events)
wenshu kanban show <task-id>

# Follow a single task's event stream
wenshu kanban tail <task-id>
```

Verify available subcommands with `wenshu kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `wenshu kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `wenshu kanban block <id>` then `wenshu kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `wenshu kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `wenshu kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `wenshu profile list` (profile exists?), `wenshu status` (gateway/dispatcher up?), and verify tenant. |
...
wenshu kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
wenshu kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(wenshu kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
wenshu kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
wenshu kanban block <task-id>
wenshu kanban archive <task-id>

# Diagnose what happened
wenshu kanban show <task-id>      # task body, comments, recent events
wenshu kanban tail <task-id>      # follow the live event stream
wenshu kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `wenshu kanban create` for the director
...
while true; do
    clear
    wenshu kanban list --tenant <slug>
    echo "---"
    wenshu kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `wenshu kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `wenshu profile list` and start
