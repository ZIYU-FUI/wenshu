...
Maps each role archetype to the Wenshu skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public wenshu-agent
...
## Wenshu skills relevant to video production

### Visual / rendering skills (`wenshu-agent/skills/creative/`)
...
### Design / pre-production skills (`wenshu-agent/skills/creative/`)
...
### Audio / media skills (`wenshu-agent/skills/creative/` + `skills/media/`)
...
These are **not** Wenshu skills but external CLIs / APIs that profiles invoke.
...
## Built-in Wenshu tools for media review

These are native Wenshu tools — not invoked via terminal but through their own
...
key is present in `${WENSHU_HOME:-~/.wenshu}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Wenshu profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${WENSHU_HOME:-~/.wenshu}/.env` → environment variable.
...
When a new Wenshu-public video skill ships:
