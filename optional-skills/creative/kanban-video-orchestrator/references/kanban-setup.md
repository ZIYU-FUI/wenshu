...
the actual `setup.sh` that creates the project workspace, configures Wenshu
...
2. **Create profiles** — `wenshu profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.wenshu-hermes/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `wenshu kanban create` assigned to the director
...
```bash
wenshu profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.wenshu-hermes/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.wenshu-hermes/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Wenshu
...
Each profile gets a `SOUL.md` at `~/.wenshu-hermes/profiles/<name>/SOUL.md` that
...
```bash
wenshu kanban create "Direct production of <video title>" \
    --assignee director \
...
the Wenshu `.env` (`${WENSHU_HOME:-$HOME/.wenshu}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _wenshu_env="${WENSHU_HOME:-$HOME/.wenshu}/.env"
    if grep -q "^${var}=" "$_wenshu_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_wenshu_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_wenshu_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY wenshu ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY wenshu OPENROUTER_API_KEY || exit 1
# ...
```
