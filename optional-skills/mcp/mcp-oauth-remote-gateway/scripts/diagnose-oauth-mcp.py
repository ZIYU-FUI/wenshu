...
Usage:
  ...
  <server_name> matches the files in $WENSHU_HOME/mcp-tokens/<server>.json etc.
...
def _wenshu_home():
    # Prefer Wenshu' own resolver (profile-safe); fall back to env then ~/.wenshu.
    try:
        from wenshu_constants import get_wenshu_home
        return str(get_wenshu_home())
    except Exception:
        return os.environ.get("WENSHU_HOME") or os.path.expanduser("~/.wenshu-hermes")


def _tokens_dir():
    return os.path.join(_wenshu_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "wenshu-diag", "version": "1.0"}},
        },
        headers={
