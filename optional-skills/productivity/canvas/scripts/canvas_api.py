#!/usr/bin/env python3
"""Canvas LMS API CLI for Wenshu Agent.
...
def _check_config():
    ...
    if missing:
        wenshu_env = os.path.join(
            os.environ.get("WENSHU_HOME", os.path.expanduser("~/.wenshu-hermes")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {wenshu_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Wenshu Agent"
    )
