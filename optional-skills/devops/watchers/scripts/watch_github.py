...
Usage (via cron with --no-agent):
    ...
    wenshu cron create wenshu-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$WENSHU_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name wenshu-issues --repo NousResearch/wenshu-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Wenshu .env file
(``${WENSHU_HOME:-~/.wenshu}/.env``) to avoid the 60 req/hr
