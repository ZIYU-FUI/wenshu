...
Usage (via cron with --no-agent):
    ...
    wenshu cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$WENSHU_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Wenshu-Watcher/1.0"})
