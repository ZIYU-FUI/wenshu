...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to WENSHU_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/wenshu), this function
    mirrors the bridge source to a writable WENSHU_HOME location and returns that
    ...
    # Try WENSHU_HOME location first
    wenshu_home = get_wenshu_home()
    wenshu_home_bridge = wenshu_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to WENSHU_HOME if needed
    if wenshu_home_bridge.exists():
        return wenshu_home_bridge

    # Mirror the bridge source to WENSHU_HOME
    try:
        wenshu_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            wenshu_home_bridge,
            dirs_exist_ok=False,
