...
There are two ways to add a platform to the Wenshu gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.wenshu-hermes/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Wenshu code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `wenshu gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `wenshu_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Wenshu core.
...
And add it to the `wenshu-gateway` composite:

```python
"wenshu-gateway": {
    "includes": [..., "wenshu-your-platform"]
}
```
...
## 12. Status Display (`wenshu_cli/status.py`)
...
## 13. Gateway Setup Wizard (`wenshu_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ wenshu_cli/ toolsets.py \
  --include="*.py" -l | sort -u
