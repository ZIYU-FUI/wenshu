...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.wenshu to the active profile home."""
    ...
    if raw == "~/.wenshu-hermes":
        return wenshu_home
    if raw.startswith("~/.wenshu-hermes/"):
        return wenshu_home / raw.removeprefix("~/.wenshu-hermes/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return wenshu_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under WENSHU_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from wenshu_constants import get_wenshu_home

    scripts_root = (get_wenshu_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.wenshu-hermes" or raw_text.startswith("~/.wenshu-hermes/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__wenshu_ignore__") is True
        ):
            return False, None
