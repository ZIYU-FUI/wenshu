...
Defenses 1 and 2 (the ``_WENSHU_GATEWAY`` guard on ``wenshu gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.wenshu.gateway")``,
...
State lives in ``<WENSHU_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_wenshu_home() / "gateway" / "restart_loop.json"
