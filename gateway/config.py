...
# dashboard's pre-write mutation validation (wenshu_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Wenshu is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_wenshu_home() / "sessions")
    ...
    # gateway behaves exactly as before — single WENSHU_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.wenshu-hermes/config.yaml (primary user-facing config)
    3. ~/.wenshu-hermes/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_wenshu_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through wenshu_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `wenshu config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `wenshu config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``wenshu config set gateway.streaming.*``
