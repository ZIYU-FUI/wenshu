...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``wenshu setup`` display).
    ...
    # built-ins).  Used by ``wenshu gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `wenshu`
        # invocation -- including plain `wenshu chat`, which never touches any
        ...
        # `wenshu setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``wenshu setup``/``gateway status``, channel
