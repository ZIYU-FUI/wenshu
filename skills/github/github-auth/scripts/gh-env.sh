#!/usr/bin/env bash
# GitHub environment detection helper for Wenshu Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _wenshu_env="${WENSHU_HOME:-$HOME/.wenshu}/.env"; [ -f "$_wenshu_env" ] && grep -q "^GITHUB_TOKEN=" "$_wenshu_env" 2>/dev/null; then
