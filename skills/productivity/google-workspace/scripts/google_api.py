#!/usr/bin/env python3
"""Google Workspace API CLI for Wenshu Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Wenshu-facing JSON contract and falls back to the Python client
...
from _wenshu_home import get_wenshu_home
...
def _gws_binary() -> str | None:
    override = os.getenv("WENSHU_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Wenshu Agent")
