...
## 5. Wenshu Configuration

### Config location

`$WENSHU_HOME/config.yaml` (defaults to `~/.wenshu-hermes/config.yaml` when `WENSHU_HOME` is unset)
...
Restart the Wenshu session for changes to take effect. The MCP connection is
