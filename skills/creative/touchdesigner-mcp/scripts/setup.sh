...
WENSHU_HOME_DIR="${WENSHU_HOME:-$HOME/.wenshu}"
WENSHU_CFG="${WENSHU_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Wenshu config has twozero_td MCP entry ──
if [[ ! -f "$WENSHU_CFG" ]]; then
    echo -e " ${FAIL} Wenshu config not found at ${WENSHU_CFG}"
    manual_steps+=("Create ${WENSHU_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$WENSHU_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Wenshu config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Wenshu config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${WENSHU_CFG} manually"); }
    manual_steps+=("Restart Wenshu session to pick up config change")
fi
