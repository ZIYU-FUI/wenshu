...
Write plugins for the Wenshu desktop app: statusbar items, layout panes,
...
system (`~/.wenshu-hermes/plugins/`) is otherwise documented separately.
...
- The Wenshu desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$WENSHU_HOME/desktop-plugins/` (usually
  `~/.wenshu-hermes/desktop-plugins/`).

## How to Run

1. Create `$WENSHU_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.wenshu-hermes/...` by default, or `~/.wenshu-hermes/profiles/<profile>/...` under a
...
The ONLY import surface is `@wenshu/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.wenshu-hermes/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@wenshu/plugin-sdk`, `react`, and
