...
| ⚠️ partial | 7 | 16% | Swift file exists with a doc-comment citing the hermes module, but the implementation is a stub / minimum-surface / wire-up-not-yet-done. Z-contract golden tests on most of these would fail. Modules: conversation_loop, auxiliary_client, system_prompt, context_engine. (10 of the original 18 ⚠ partial entries have been closed by H1-H8 + P1-P11 follow-up tickets: anthropic_adapter + tool_executor + conversation_compression + model_metadata + skill_preprocessing + skill_commands + credential_sources + tool_result_classification + agent_init + message_content. = 4 of the remaining 7 are stale per the 2026-09-19 audit re-check (= turn_context + turn_finalizer + message_sanitization + context_references all already have their hermes surface per TICKET-HERMES-PARTIAL-006/007/008/009; = per-module table below shows the per-module closure status). |
...
Honest tally: **29/43 modules fully done** (= direct port + wenshu-side wins = 7 + 11 = 18 baseline + 11 closed by H1-H8 + P1-P12 follow-up = 29) and **14/43 still partial** (= 7 ⚠️ partial with documented gaps + 4 ⚠️ partial stale per re-check + 3 ❌ missing closed but stale in the audit row above). Per the 2026-09-19 audit re-check v3 (= see per-module table below), the work-tree gap is now 14/43 modules (= down from 25/43 at 2026-09-04).
...
| agent_init | ✅ closed | P9 (`5eef58dc5`) |
| message_content | ✅ closed | P11 (`e1f2fbc82`) |
| context_references | ✅ stale (= already closed per TICKET-HERMES-PARTIAL-006; = `ContextReference` struct + `ContextReferences` actor + `add/remove/reference/for/persist/clear` + `ContextReferencesBuilder` + `ContextReferenceParser` all present with JSON-on-disk persistence per audit 2026-09-19 re-check) | (no P-ticket needed) |
| turn_context | ✅ stale (= already closed per TICKET-HERMES-PARTIAL-007; = `shouldRunPreflightEstimate` + `compressionMadeProgress` + `TurnContext` struct + `TurnContextBuilder.buildTurnContext` all present per audit 2026-09-19 re-check) | (no P-ticket needed) |
...
| turn_finalizer | ✅ stale (= already closed per TICKET-HERMES-PARTIAL-008; = `finalize` + `coalesceAdjacentText` + `finalizeTurn` + `PostTurnHooks` + `FinalizedTurn` + `closeInterruptedToolSequence` + `dropTrailingEmptyResponseScaffolding` all present per audit 2026-09-19 re-check) | (no P-ticket needed) |
| message_sanitization | ✅ stale (= already closed per TICKET-HERMES-PARTIAL-009; = `sanitizeText` + `sanitize` + `sanitizeSurrogates` + `escapeInvalidCharsInJSONStrings` + `repairToolCallArguments` + `closeInterruptedToolSequence` + `stripNonASCII` + `dropTrailingEmptyResponseScaffolding` + `sanitizeAll` all present per audit 2026-09-19 re-check) | (no P-ticket needed) |
