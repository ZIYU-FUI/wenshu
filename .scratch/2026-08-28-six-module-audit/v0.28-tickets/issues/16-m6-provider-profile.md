# Issue 16 — M6/M6 hermes verbatim-port — provider profile
...
Provider profile + models.dev catalog + model metadata adapter (= 11 curated providers 已经是现有, 本次扩展 profile + dynamic metadata)

## From hermes

```
providers/base.py + agent/models_dev.py + agent/model_metadata.py
```

## To wenshu

```
Sources/WenshuApp/Core/Provider/{ProviderProfile,ModelsDev,ModelMetadata}.swift + extend ProviderFetcher.swift + WenshuVerifier.swift
```
...
可独立 (= ProviderFetcher.swift 已存在, 本次只扩展)
