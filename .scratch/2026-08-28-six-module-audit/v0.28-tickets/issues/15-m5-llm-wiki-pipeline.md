# Issue 15 — M5/M6 hermes verbatim-port — llm wiki pipeline
...
LLM Wiki 4-layer ingest pipeline (= raw / entities / abstracts / indexes) — 接受 EPUBKit imports (issue 03) 作为触发

## From hermes

```
skills/research/llm-wiki/SKILL.md v2.1.0 (= Karpathy LLM Wiki 4-layer pattern)
```

## To wenshu

```
Sources/WenshuApp/Domain/{LLMWikiSchema,LLMWikiIngestor,LLMWikiLinter}.swift
```
...
M5 LLM Wiki pipeline 落地时 + 任何 reference-library ingest (= EPUBKit / .ws import / manual paste)

## Dependencies

issue 03 EPUBKit + adapter; issue 12 entity-extraction; issue 13 smart-query-rewriter (= pipeline 入口 接收这些 component 的输出)
