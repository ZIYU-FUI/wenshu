...
| | |
|---|---|
| 来源 | 可选 — 通过 `wenshu skills install official/blockchain/hyperliquid` 安装 |
| 路径 | `optional-skills/blockchain/hyperliquid` |
| 版本 | `0.1.0` |
| 作者 | Hugo Sequier (Hugo-SEQUIER), Wenshu Agent |
| 许可证 | MIT |
...
脚本从 `~/.wenshu-hermes/.env` 读取两个可选默认值：
...
辅助脚本：`~/.wenshu-hermes/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
对于 `state`、`spot-balances`、`fills`、`orders` 和 `review`，当 `~/.wenshu-hermes/.env` 中设置了 `HYPERLIQUID_USER_ADDRESS` 时，地址参数为可选。
