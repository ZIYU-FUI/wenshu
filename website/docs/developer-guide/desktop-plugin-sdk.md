...
title: "Desktop Plugin SDK (@wenshu/plugin-sdk)"
description: "Extend the native Wenshu Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Wenshu Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `WenshuPlugin`.
It imports one module — `@wenshu/plugin-sdk` — and gets everything: the app's
...
`$WENSHU_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Wenshu. This page is the **native
desktop app** (`wenshu desktop`) SDK — the `@wenshu/plugin-sdk` module and
`$WENSHU_HOME/desktop-plugins/`. The **web dashboard** (`wenshu dashboard`) has
its own, unrelated plugin system on `window.__WENSHU_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Wenshu Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$WENSHU_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `WenshuPlugin` contract, appear in **Settings → Plugins**, and
...
[`wenshu-example-plugins`](https://github.com/NousResearch/wenshu-example-plugins)
repo.

## Quick start — your first plugin

Create `$WENSHU_HOME/desktop-plugins/hello/plugin.js` (that's `~/.wenshu-hermes/...`
by default, or `~/.wenshu-hermes/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.wenshu-hermes/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@wenshu/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Wenshu' }),
      jsx('div', {
...
The only importable specifiers are `@wenshu/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `WenshuPlugin`:

```ts
interface WenshuPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `wenshu.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@wenshu/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@wenshu/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@wenshu/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@wenshu/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@wenshu/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@wenshu/plugin-sdk'
...
```javascript
import { queryClient } from '@wenshu/plugin-sdk'
...
`dashboard/` subfolder of a regular Wenshu plugin and declare it in a
`manifest.json`:

```
~/.wenshu-hermes/plugins/<id>/
└── dashboard/
...
wenshu-agent codebase directly (`wenshu_state`, `wenshu_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.wenshu-hermes/`)
...
(`wenshu.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `WenshuPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@wenshu/plugin-sdk` alias.
2. It's still lint-fenced to `@wenshu/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@wenshu/plugin-sdk`, `react`,
...
| Plugin contract | `WenshuPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `wenshu-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`wenshu-desktop-plugins`** skill — it carries the same contract as this page in
...
`$WENSHU_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `wenshu logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@wenshu/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.wenshu-hermes/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.wenshu-hermes/logs/errors.log` for
