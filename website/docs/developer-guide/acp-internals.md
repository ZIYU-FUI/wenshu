...
The ACP adapter wraps Wenshu' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
wenshu acp / wenshu-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.wenshu-hermes/.env
  -> configure stderr logging
  -> construct WenshuACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'wenshu-agent[acp]==<version>' wenshu-acp`, pointed at the `wenshu-agent` PyPI release.
...
- `allow_once` -> Wenshu `once`
- `allow_always` -> Wenshu `always`
- reject options -> Wenshu `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Wenshu tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["wenshu-acp"])
...
Instead it reuses Wenshu' runtime resolver:

- `acp_adapter/auth.py`
- `wenshu_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Wenshu provider/credentials. It also always advertises a terminal setup auth method (`wenshu-setup`, args `--setup`) so first-run registry clients can open Wenshu' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.wenshu-hermes/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `wenshu-acp` toolset definition
- `wenshu_cli/main.py` — `wenshu acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `wenshu-acp` script
