...
Wenshu Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `wenshu plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Wenshu has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|wenshu|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `wenshu`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Wenshu raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$WENSHU_HOME`
...
```bash
wenshu config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
wenshu config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Wenshu' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Wenshu
  records the resulting compaction events (compression counters, session
  events). Set `wenshu` to let Wenshu' compression threshold initiate
  app-server compaction, or `off` to disable Wenshu-initiated automatic
  compaction entirely (codex may still compact natively).

Wenshu' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Wenshu' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Wenshu
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Wenshu can avoid; user-facing docs for `/model`, fallback providers, and
