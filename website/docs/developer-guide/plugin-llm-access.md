...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Wenshu already has applies. The plugin never sees a
...
[`wenshu-example-plugins`](https://github.com/NousResearch/wenshu-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
wenshu-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/wenshu-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Wenshu provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Wenshu provider
        # (must be one Wenshu already knows about — same names as
        # `wenshu model` and config.yaml model.provider).
        ...
        # plugin sends — Wenshu does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.wenshu-hermes/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Wenshu' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Wenshu subsystem:
...
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/wenshu-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/wenshu-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
