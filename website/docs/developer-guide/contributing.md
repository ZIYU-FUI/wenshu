...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Wenshu Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Wenshu Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Wenshu core? Start with [Build a Wenshu Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Wenshu itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Wenshu instance (throwaway WENSHU_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m wenshu_cli.main
scripts/dev-sandbox.sh --persistent python -m wenshu_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.wenshu-hermes/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.wenshu` — use `get_wenshu_home()` from `wenshu_constants` for code paths and `display_wenshu_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
