...
sidebar_position: 1
title: "Architecture"
description: "Wenshu Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Wenshu Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ wenshu_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # WenshuCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── wenshu_state.py           # SQLite session/state database with FTS5
├── wenshu_constants.py       # WENSHU_HOME, profile-aware paths
...
│
├── wenshu_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `wenshu` subcommands (large file)
...
│   ├── skills_config.py      # wenshu skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # wenshu tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # wenshu gateway start/stop
│
...
```text
User input → WenshuCLI.process_input()
...
Three discovery sources: `~/.wenshu-hermes/plugins/` (user), `.wenshu-hermes/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `wenshu plugins` or `config.yaml`.
...
Exposes Wenshu as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`wenshu -p <name>`) gets its own WENSHU_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
