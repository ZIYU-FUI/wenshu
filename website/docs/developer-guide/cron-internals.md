...
description: "How Wenshu stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `wenshu_cli/cron.py` | CLI `wenshu cron` subcommands |
...
Jobs are stored in `~/.wenshu-hermes/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `wenshu cron` commands are run or during active CLI sessions.
...
```python
# ~/.wenshu-hermes/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.wenshu-hermes/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `wenshu cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `wenshu cron` CLI provides direct job management:

```bash
wenshu cron list                    # Show all jobs
wenshu cron create                  # Interactive job creation (alias: add)
wenshu cron edit <job_id>           # Edit job configuration
wenshu cron pause <job_id>          # Pause a running job
wenshu cron resume <job_id>         # Resume a paused job
wenshu cron run <job_id>            # Trigger immediate execution
wenshu cron remove <job_id>         # Delete a job
```
