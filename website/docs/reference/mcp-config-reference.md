...
description: "Reference for Wenshu Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Wenshu may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Wenshu only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Wenshu does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Wenshu uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.wenshu-hermes/mcp-tokens/<server>.json` and reused across sessions
