...
This page covers all commands related to [Wenshu profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `wenshu profile` without a subcommand shows help.
...
```bash
wenshu profile use <name>
```

Sets `<name>` as the active profile. All subsequent `wenshu` commands (without `-p`) will use this profile.
...
wenshu profile use default
```

## `wenshu profile create`

```bash
wenshu profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `wenshu profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `wenshu update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.wenshu`), use `wenshu skills opt-out` / `wenshu skills opt-in`. |
...
wenshu profile create work2 --clone-from work

# Clone everything from a specific profile
wenshu profile create work2-backup --clone-from work --clone-all
```

## `wenshu profile describe`

```bash
wenshu profile describe [<name>] [options]
```
...
wenshu profile describe researcher

# Set it explicitly
wenshu profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
wenshu profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
wenshu profile describe --all --auto
```

## `wenshu profile delete`

```bash
wenshu profile delete <name> [options]
```
...
```bash
wenshu profile delete mybot
wenshu profile delete mybot --yes
```
...
```bash
wenshu profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Wenshu home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ wenshu profile show work
Profile: work
Path:    ~/.wenshu-hermes/profiles/work
...
## `wenshu profile alias`

```bash
wenshu profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Wenshu installation.
...
```bash
wenshu profile alias work
# Creates/updates ~/.local/bin/work

wenshu profile alias work --name mywork
# Creates ~/.local/bin/mywork

wenshu profile alias work --remove
# Removes the wrapper script
```

## `wenshu profile rename`

```bash
wenshu profile rename <old-name> <new-name>
```
...
```bash
wenshu profile rename mybot assistant
# ~/.wenshu-hermes/profiles/mybot → ~/.wenshu-hermes/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `wenshu profile export`

```bash
wenshu profile export <name> [options]
```
...
```bash
wenshu profile export work
# Creates work.tar.gz in the current directory

wenshu profile export work -o ./work-2026-03-29.tar.gz
```

## `wenshu profile import`

```bash
wenshu profile import <archive> [options]
```
...
```bash
wenshu profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

wenshu profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`wenshu profile export` / `import` are still the right commands for
...
### `wenshu profile install`

```bash
wenshu profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `wenshu -p telemetry`). |
...
wenshu profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
wenshu profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
wenshu profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
wenshu profile install ./telemetry/
```

### `wenshu profile update`

```bash
wenshu profile update <name> [--force-config] [--yes]
```
...
```bash
wenshu profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Wenshu version, author, env var requirements, the source URL/path, and
...
`wenshu profile list` also shows the distribution name and version in a
`Distribution` column, and `wenshu profile show <name>` / `delete <name>`
...
wenshu profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
wenshu profile install https://github.com/your-org/internal-assistant.git
```
...
`wenshu_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Wenshu version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Wenshu can clone from.
3. Tell recipients to run `wenshu profile install <your-repo-url>`.
...
## `wenshu -p` / `wenshu --profile`

```bash
wenshu -p <name> <command> [options]
wenshu --profile <name> <command> [options]
```

Global flag to run any Wenshu command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
wenshu -p work chat -q "Check the server status"
wenshu --profile dev gateway start
wenshu -p personal skills list
wenshu -p work config edit
```

## `wenshu completion`

```bash
wenshu completion <shell>
```
...
- `wenshu profile <TAB>` — subcommands (list, use, create, etc.)
- `wenshu profile use <TAB>` — profile names
- `wenshu -p <TAB>` — profile names
