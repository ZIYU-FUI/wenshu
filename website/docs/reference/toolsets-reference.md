...
description: "Reference for Wenshu core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `wenshu-cli` is the default for interactive CLI sessions)
...
```bash
wenshu chat --toolsets web,file,terminal
wenshu chat --toolsets debugging        # composite — expands to file + terminal + web
wenshu chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - wenshu-cli          # default for CLI
  # - wenshu-telegram   # override for Telegram gateway
```

### Interactive management

```bash
wenshu tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Wenshu tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `wenshu-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `wenshu-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `wenshu-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `wenshu tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `wenshu-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `wenshu-cli`:

| Toolset | Differences from `wenshu-cli` |
|---------|-------------------------------|
| `wenshu-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, and clarify, plus the `safe` (read-only) bundle. |
| `wenshu-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `wenshu-api-server` | Drops `clarify` and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `wenshu-cron` | Same as `wenshu-cli`. |
| `wenshu-telegram` | Same as `wenshu-cli`. |
| `wenshu-discord` | Adds `discord` and `discord_admin` on top of `wenshu-cli`. |
| `wenshu-slack` | Same as `wenshu-cli`. |
| `wenshu-whatsapp` | Same as `wenshu-cli`. |
| `wenshu-signal` | Same as `wenshu-cli`. |
| `wenshu-matrix` | Same as `wenshu-cli`. |
| `wenshu-mattermost` | Same as `wenshu-cli`. |
| `wenshu-email` | Same as `wenshu-cli`. |
| `wenshu-sms` | Same as `wenshu-cli`. |
| `wenshu-bluebubbles` | Same as `wenshu-cli`. |
| `wenshu-dingtalk` | Same as `wenshu-cli`. |
| `wenshu-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `wenshu-qqbot` | Same as `wenshu-cli`. |
| `wenshu-wecom` | Same as `wenshu-cli`. |
| `wenshu-wecom-callback` | Same as `wenshu-cli`. |
| `wenshu-weixin` | Same as `wenshu-cli`. |
| `wenshu-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `wenshu-cli`. |
| `wenshu-homeassistant` | Same as `wenshu-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `wenshu-webhook` | Same as `wenshu-cli`. |
| `wenshu-gateway` | Internal gateway orchestrator toolset — union of every `wenshu-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `WENSHU_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `wenshu tools`

The `wenshu tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
