...
description: "Configure and use Honcho memory with Wenshu -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Wenshu -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `wenshu skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Wenshu

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Wenshu profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/wenshu#running-honcho-locally-with-wenshu

### Verify

```bash
wenshu honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Wenshu profile's AI peer
...
Honcho models conversations as interactions between **peers**. Wenshu creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Wenshu instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Wenshu run |
| `global` | Single session across all directories |

Manual override: `wenshu honcho map my-project-name`
...
Each Wenshu profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block wenshu.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `wenshu.coder` host block in `honcho.json`
...
```bash
wenshu honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Wenshu when Honcho memory is active.
...
Config file: `$WENSHU_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `wenshu honcho setup`. Ensure `memory.provider: honcho` is in `~/.wenshu-hermes/config.yaml`.

### Memory not persisting across sessions
Check `wenshu honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `wenshu profile create <name> --clone`. For existing profiles: `wenshu honcho sync`.
...
| Command | Description |
|---------|-------------|
| `wenshu honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `wenshu honcho status` | Show resolved config, connection test, peer info for active profile |
| `wenshu honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `wenshu honcho disable` | Disable Honcho for the active profile |
| `wenshu honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `wenshu honcho peers` | Show peer identities across all profiles |
| `wenshu honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `wenshu honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `wenshu honcho sessions` | List known directory-to-session-name mappings |
| `wenshu honcho map <name>` | Map current working directory to a Honcho session name |
| `wenshu honcho identity` | Seed AI peer identity or show both peer representations |
| `wenshu honcho sync` | Create host blocks for all Wenshu profiles that don't have one yet |
| `wenshu honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Wenshu + Honcho |
| `wenshu memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `wenshu memory status` | Show active memory provider and config |
| `wenshu memory off` | Disable external memory provider |
