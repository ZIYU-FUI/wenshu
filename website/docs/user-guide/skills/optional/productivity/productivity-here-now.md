...
| | |
|---|---|
| Source | Optional — install with `wenshu skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${WENSHU_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${WENSHU_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${WENSHU_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client wenshu
```
...
```bash
PUBLISH="${WENSHU_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client wenshu
```
...
```bash
DRIVE="${WENSHU_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `wenshu`)    |
