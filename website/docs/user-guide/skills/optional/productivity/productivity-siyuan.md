...
| | |
|---|---|
| Source | Optional — install with `wenshu skills install official/productivity/siyuan` |
...
3. Store it in `${WENSHU_HOME:-~/.wenshu}/.env`:
   ```
...
```yaml
# In ~/.wenshu-hermes/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
