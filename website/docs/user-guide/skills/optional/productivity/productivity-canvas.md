...
| | |
|---|---|
| Source | Optional — install with `wenshu skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Wenshu Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${WENSHU_HOME:-~/.wenshu}/.env`:
...
```bash
CANVAS="python $WENSHU_HOME/skills/productivity/canvas/scripts/canvas_api.py"
