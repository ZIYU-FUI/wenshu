...
| | |
|---|---|
| Source | Optional — install with `wenshu skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Wenshu or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Wenshu. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.wenshu-hermes/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.wenshu-hermes/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Wenshu integration, either:

- configure the server in `~/.wenshu-hermes/config.yaml` using the `native-mcp` skill, or
...
### Wenshu cannot see the deployed server

The server-building part may be correct while the Wenshu config is not. Load the `native-mcp` skill and configure the server in `~/.wenshu-hermes/config.yaml`, then restart Wenshu.
