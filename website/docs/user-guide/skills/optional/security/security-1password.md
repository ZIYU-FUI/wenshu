...
| | |
|---|---|
| Source | Optional — install with `wenshu skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Wenshu Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Wenshu terminal calls (desktop app flow only)
...
### Service Account (recommended for Wenshu)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${WENSHU_HOME:-~/.wenshu}/.env` (the skill will prompt for this on first load).
...
## Wenshu Execution Pattern (desktop app flow)

Wenshu terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/wenshu-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/wenshu-op.sock"
