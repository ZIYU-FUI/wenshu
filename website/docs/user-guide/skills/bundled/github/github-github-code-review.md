...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _wenshu_env="${WENSHU_HOME:-$HOME/.wenshu}/.env"; [ -f "$_wenshu_env" ] && grep -q "^GITHUB_TOKEN=" "$_wenshu_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_wenshu_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Wenshu Agent\",
    \"comments\": [
...
```bash
source "${WENSHU_HOME:-$HOME/.wenshu}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Wenshu Agent. Code looks clean — good test coverage, no security concerns."
