...
- Give it a name like "wenshu-agent"
- Select scopes:
...
- Give it a title like "wenshu-agent-&lt;machine-name>"
...
elif _wenshu_env="${WENSHU_HOME:-$HOME/.wenshu}/.env"; [ -f "$_wenshu_env" ] && grep -q "^GITHUB_TOKEN=" "$_wenshu_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_wenshu_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
