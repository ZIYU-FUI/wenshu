...
`~/.wenshu-hermes/config.yaml`, `~/.wenshu-hermes/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/wenshu`:

```text
/etc/wenshu/
├── config.yaml     # managed config layer (wins over ~/.wenshu-hermes/config.yaml)
└── .env            # managed env layer (wins over ~/.wenshu-hermes/.env + shell)
```
...
The location can be relocated with the `WENSHU_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `WENSHU_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Wenshu.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export WENSHU_MANAGED_DIR=/opt/org/wenshu-policy
```

:::warning
A user who can set `WENSHU_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `wenshu doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/wenshu/config.yaml` (managed) | `/etc/wenshu/.env` (managed) |
| 2 | `~/.wenshu-hermes/config.yaml` (user) | `~/.wenshu-hermes/.env` (user) |
...
```bash
wenshu config        # shows a header naming the managed source + the pinned keys
wenshu doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Wenshu refuses and names the source:

```bash
$ wenshu config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/wenshu/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `wenshu config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/wenshu

# Pin some config values for every user on this machine
sudo tee /etc/wenshu/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/wenshu/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/wenshu
sudo chmod 0644 /etc/wenshu/config.yaml /etc/wenshu/.env
```

Changes take effect on the next Wenshu start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`wenshu doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Wenshu as `root`), managed scope is advisory.
