...
If you only run one Wenshu agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Wenshu agents that should all
...
(`ai.wenshu.gateway-<name>.plist`) or systemd user service
(`wenshu-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
wenshu profile create coder
wenshu profile create personal-bot
wenshu profile create research
...
```bash
wenshu config set gateway.multiplex_profiles true
wenshu gateway restart
```

Equivalently, in the default profile's `~/.wenshu-hermes/config.yaml`:
...
You do **not** run `wenshu gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `wenshu gateway start` / `run` is a
...
home). `wenshu status` reports the multiplexer and the profiles it serves;
`wenshu status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/wenshu-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: wenshu-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    wenshu gateway "$action"
  else
    wenshu -p "$profile" gateway "$action"
  fi
}
...
```bash
wenshu-gateways start      # start every configured profile
wenshu-gateways stop       # stop every configured profile
wenshu-gateways restart    # restart all
wenshu-gateways status     # status across all
wenshu-gateways list       # delegates to `wenshu gateway list`
```

:::tip
The `default` profile is targeted with `wenshu gateway <action>` (no `-p`),
not `wenshu -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `wenshu -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.wenshu.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/wenshu-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.wenshu.gateway.plist` /
`wenshu-gateway.service`.
...
```bash
# Default profile
tail -f ~/.wenshu-hermes/logs/gateway.log
tail -f ~/.wenshu-hermes/logs/gateway.error.log

# Named profile
tail -f ~/.wenshu-hermes/profiles/<name>/logs/gateway.log
tail -f ~/.wenshu-hermes/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.wenshu-hermes/logs/gateway.log ~/.wenshu-hermes/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
wenshu logs -f                  # follow default profile
wenshu -p coder logs -f         # follow one profile
wenshu logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
wenshu profile list             # profiles + model + gateway state
wenshu-gateways status          # full status across every profile
launchctl list | grep wenshu    # macOS — PIDs and labels
systemctl --user list-units 'wenshu-gateway-*'   # Linux — units
```
...
```
~/.wenshu-hermes/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.wenshu-hermes/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
wenshu config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.wenshu-hermes/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=wenshu --why="gateways running" \
  sleep infinity &
...
`wenshu-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.wenshu-hermes/.env ~/.wenshu-hermes/profiles/*/.env
```

## Updating the code

`wenshu update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `wenshu gateway start` after a previous `wenshu gateway stop`. The
...
```bash
ps -ef | grep "wenshu_cli.*-p <profile>"
cat ~/.wenshu-hermes/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.wenshu.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.wenshu.gateway-<profile>.plist

# Linux
systemctl --user restart wenshu-gateway-<profile>.service
```

### Health check

```bash
wenshu doctor                  # default profile
wenshu -p <profile> doctor     # one profile
```
