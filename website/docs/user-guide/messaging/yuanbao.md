...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Wenshu Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Wenshu to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.wenshu-hermes/.env`:
...
All standard Wenshu commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.wenshu-hermes/logs/gateway.log`
...
4. Consider enabling verbose logging: `WENSHU_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Wenshu automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
WENSHU_LOG_LEVEL=debug wenshu gateway
```
...
```bash
wenshu chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
