...
Connect Wenshu to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Wenshu supports two WeCom integration modes:
...
> Run `wenshu gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Wenshu decrypts the message, queues it for the agent
...
(Use `wenshu gateway start` only after `wenshu gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Wenshu and the token the
...
in `~/.wenshu-hermes/.env` values around `=` will also break signature checks. After
fixing, restart `wenshu gateway run`.
...
Check `wenshu gateway run` logs for the bound host/port. If the adapter bound to
