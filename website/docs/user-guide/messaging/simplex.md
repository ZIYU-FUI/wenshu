...
> Run `wenshu gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `WENSHU_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `wenshu pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `wenshu send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`wenshu send` CLI](/guides/pipe-script-output):

```bash
wenshu send simplex:<contact-id> "Done!"
```

## Privacy notes

- SimpleX never reveals phone numbers or email addresses — contacts use opaque IDs
- The connection between Wenshu and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
