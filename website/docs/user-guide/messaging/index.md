...
description: "Chat with Wenshu from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Wenshu from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Wenshu](/guides/use-voice-mode-with-wenshu).
...
```mermaid
flowchart TB
    subgraph Gateway["Wenshu Gateway"]
...
For group chats, hooks, and automation flows, Wenshu supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Wenshu keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Wenshu does not hide failures just because the text resembles a silence token.
...
```bash
wenshu gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
wenshu gateway              # Run in foreground
wenshu gateway setup        # Configure messaging platforms interactively
wenshu gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo wenshu gateway install --system   # Linux only: install a boot-time system service
wenshu gateway start        # Start the default service
wenshu gateway stop         # Stop the default service
wenshu gateway status       # Check default service status
wenshu gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.wenshu-hermes/config.yaml"
gateway:
...
```bash
wenshu gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Wenshu sends heartbeats
...
| `/update` | Update Wenshu Agent to the latest version |
...
with the `session_reset` section in `~/.wenshu-hermes/config.yaml`:
...
Configure per-platform overrides in `~/.wenshu-hermes/gateway.json`:
...
wenshu pairing approve telegram XKGH5N7P

# Other pairing commands:
wenshu pairing list          # View pending + approved users
wenshu pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Wenshu appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.wenshu-hermes/config.yaml`:
...
Off by default. When enabled, Wenshu prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.wenshu-hermes/config.yaml`:
...
```bash
WENSHU_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
wenshu gateway install               # Install as user service
wenshu gateway start                 # Start the service
wenshu gateway stop                  # Stop the service
wenshu gateway status                # Check status
journalctl --user -u wenshu-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo wenshu gateway install --system
sudo wenshu gateway start --system
sudo wenshu gateway status --system
journalctl -u wenshu-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Wenshu installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit wenshu-gateway` (or `sudo systemctl edit wenshu-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `wenshu update`. When `wenshu update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart wenshu-gateway` command (it never blocks on an interactive password prompt).
...
```bash
wenshu gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `wenshu update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo wenshu update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/wenshu-gateway`:

```
wenshu ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed wenshu-gateway*, /usr/bin/systemctl --no-ask-password start wenshu-gateway*, /usr/bin/systemctl --no-ask-password restart wenshu-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Wenshu will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Wenshu installations on the same machine (with different `WENSHU_HOME` directories), each gets its own systemd service name. The default `~/.wenshu` uses `wenshu-gateway`; other installations use `wenshu-gateway-<hash>`. The `wenshu gateway` commands automatically target the correct service for your current `WENSHU_HOME`.
:::

### macOS (launchd)

```bash
wenshu gateway install               # Install as launchd agent
wenshu gateway start                 # Start the service
wenshu gateway stop                  # Stop the service
wenshu gateway status                # Check status
tail -f ~/.wenshu-hermes/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.wenshu.gateway.plist`. It includes three environment variables:
...
- **WENSHU_HOME** — scopes the gateway to your Wenshu installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `wenshu gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `WENSHU_HOME` directory gets its own launchd label. The default `~/.wenshu` uses `ai.wenshu.gateway`; other installations use `ai.wenshu.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `wenshu-cli` | Full access |
| Telegram | `wenshu-telegram` | Full tools including terminal |
| Discord | `wenshu-discord` | Full tools including terminal |
| WhatsApp | `wenshu-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `wenshu-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `wenshu-slack` | Full tools including terminal |
| Google Chat | `wenshu-google_chat` | Full tools including terminal |
| Signal | `wenshu-signal` | Full tools including terminal |
| SMS | `wenshu-sms` | Full tools including terminal |
| Email | `wenshu-email` | Full tools including terminal |
| Home Assistant | `wenshu-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `wenshu-mattermost` | Full tools including terminal |
| Matrix | `wenshu-matrix` | Full tools including terminal |
| DingTalk | `wenshu-dingtalk` | Full tools including terminal |
| Feishu/Lark | `wenshu-feishu` | Full tools including terminal |
| WeCom | `wenshu-wecom` | Full tools including terminal |
| WeCom Callback | `wenshu-wecom-callback` | Full tools including terminal |
| Weixin | `wenshu-weixin` | Full tools including terminal |
| BlueBubbles | `wenshu-bluebubbles` | Full tools including terminal |
| QQBot | `wenshu-qqbot` | Full tools including terminal |
| Yuanbao | `wenshu-yuanbao` | Full tools including terminal |
| Microsoft Teams | `wenshu-teams` | Full tools including terminal |
| API Server | `wenshu-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `wenshu-webhook` | Full tools including terminal |
| Raft | `wenshu-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.wenshu-hermes/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
