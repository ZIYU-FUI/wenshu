...
Connect Wenshu to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Wenshu — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Wenshu runs it in a
...
wenshu photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Wenshu Agent` project on your account.
...
Runtime credentials are written to `~/.wenshu-hermes/.env`
...
(device token, dashboard project id) lives in `~/.wenshu-hermes/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Wenshu
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Wenshu replies with a pairing code. Approve it with:

```bash
wenshu pairing approve photon <CODE>
```

Use `wenshu pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.wenshu-hermes/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.wenshu-hermes/.env`):
...
By default Wenshu responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Wenshu` and
`@Wenshu agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Wenshu will reply.

## Status & troubleshooting

```bash
wenshu photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Wenshu uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run wenshu photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `wenshu photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `wenshu photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `wenshu photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Wenshu sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Wenshu wake words  | JSON list / comma / newline regex patterns for group mentions |
