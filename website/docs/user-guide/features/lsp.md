...
Wenshu runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Wenshu
...
1. Wenshu captures a baseline of current diagnostics for the file.
...
…). Wenshu auto-detects the binary on PATH or in
`<WENSHU_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Wenshu at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<WENSHU_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`wenshu lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Wenshu installs both packages together when you
run `wenshu lsp install typescript` or auto-install fires on first
use.

## CLI

```
wenshu lsp status          # service state + per-server install status
wenshu lsp list            # registry, optionally --installed-only
wenshu lsp install <id>    # eagerly install one server
wenshu lsp install-all     # try every server with a known recipe
wenshu lsp restart         # tear down running clients
wenshu lsp which <id>      # print resolved binary path
```

`wenshu lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <WENSHU_HOME>/lsp/bin
...
When `install_strategy: auto`, Wenshu installs binaries into
`<WENSHU_HOME>/lsp/bin/`. NPM packages land in
`<WENSHU_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Wenshu-owned and is
...
Servers are kept alive for the life of the Wenshu process. There's
...
**`wenshu lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<WENSHU_HOME>/lsp/bin/`. Run
`wenshu lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `wenshu lsp status`**
...
When `wenshu lsp status` shows a `Backend warnings` section, install
...
`~/.wenshu-hermes/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.wenshu-hermes/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `wenshu lsp restart` to clear the set;
