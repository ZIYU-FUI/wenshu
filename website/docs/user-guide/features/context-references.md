...
Type `@` followed by a reference to inject content directly into your message. Wenshu expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Wenshu env: `$WENSHU_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$WENSHU_HOME/skills/.hub/`
