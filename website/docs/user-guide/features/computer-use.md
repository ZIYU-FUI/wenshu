...
Wenshu Agent can drive your desktop — clicking, typing, scrolling,
...
```
wenshu computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `wenshu computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `wenshu tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Wenshu app). `wenshu computer-use doctor` will tell you which permission is missing. |
...
```
wenshu -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.wenshu-hermes/config.yaml`.

## `wenshu computer-use doctor` — your first triage stop

`wenshu computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ wenshu computer-use doctor
...
agent is acting here." Each Wenshu run declares its own cua-driver
**session id** (something like `wenshu-3a7b9c14d2e8`); the cursor's
...
Wenshu intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Wenshu-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Wenshu skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Wenshu
...
Wenshu applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.wenshu-hermes/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Wenshu applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Wenshu agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Wenshu agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Wenshu warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Wenshu from inside the RDP / console
...
```
WENSHU_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
WENSHU_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Wenshu disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Wenshu sets
...
When it's on, `wenshu computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Wenshu at a binary you built from source instead
of the published release. Wenshu resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`WENSHU_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Wenshu testing (see notes).
...
### Option B — point Wenshu straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`WENSHU_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
WENSHU_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
WENSHU_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Wenshu is using your build

- `wenshu computer-use status` prints the resolved binary path and
  version.
- `wenshu computer-use doctor` confirms the binary is reachable and
...
- **Wenshu spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Wenshu
  change needed either way.
- **Local builds skip the version check.** Wenshu warns when the
...
**First action when anything's off: run `wenshu computer-use doctor`.**
...
Run `wenshu computer-use install` to fetch the cua-driver binary, or
run `wenshu tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `wenshu computer-use doctor` will
...
- **Wenshu-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Wenshu `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Wenshu (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Wenshu among them)
