...
Wenshu can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<WENSHU_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Wenshu renders the sprite at full fidelity when your
...
wenshu pets list
wenshu pets list cat

# Install a pet and make it active in one step
wenshu pets install boba --select
...
## `wenshu pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `wenshu pets list [query] [--limit N]` |
| List installed pets | `wenshu pets list --installed` |
| Install a pet | `wenshu pets install <slug> [--select] [--force]` |
| Set the active pet | `wenshu pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `wenshu pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `wenshu pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `wenshu pets off` |
| Remove an installed pet | `wenshu pets remove <slug>` |
| Diagnose setup | `wenshu pets doctor` |

`wenshu pets show` flags:
...
Beyond installing pre-made pets from the gallery, Wenshu can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `wenshu pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `wenshu tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `WENSHU_PET_IMAGE_PROVIDER` env var (e.g. `WENSHU_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Wenshu is
...
Run `wenshu pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Wenshu uses its own
  profile-scoped `<WENSHU_HOME>/pets/` instead — install through `wenshu pets`.
