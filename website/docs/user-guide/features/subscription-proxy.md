...
chat completions — use your Wenshu-managed provider subscription as their
...
| Use case | "Use Wenshu as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Wenshu stores
the refresh token in `~/.wenshu-hermes/auth.json` — the same place all Wenshu
...
Model:      Wenshu-4-70B    # or Wenshu-4.3-36B, Wenshu-4-405B
```
...
interface in `wenshu_cli/proxy/adapters/`.
...
If you see `not logged in`, run `wenshu portal`. If you see
...
`wenshu_cli/proxy/adapters/<provider>.py` and registering it in
