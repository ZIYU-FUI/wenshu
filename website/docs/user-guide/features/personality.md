...
description: "Customize Wenshu Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Wenshu Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `WENSHU_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Wenshu is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Wenshu now seeds a default `SOUL.md` automatically in:

```text
~/.wenshu-hermes/SOUL.md
```

More precisely, it uses the current instance's `WENSHU_HOME`, so if you run Wenshu with a custom home directory, it will use:
...
- Wenshu creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Wenshu loads `SOUL.md` only from `WENSHU_HOME`
- Wenshu does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Wenshu falls back to a built-in default identity
...
If Wenshu loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `WENSHU_HOME`, the personality belongs to the Wenshu instance itself.

That also makes it easier to teach users:
- "Edit `~/.wenshu-hermes/SOUL.md` to change Wenshu' default personality."
...
- how Wenshu should handle uncertainty, disagreement, or ambiguity
...
## What Wenshu injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Wenshu falls back to a built-in default identity ("You are Wenshu Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Wenshu ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Wenshu, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Wenshu its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.wenshu-hermes/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.wenshu-hermes/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-wenshu)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Wenshu speaks
- `display.skin` and `/skin` affect how Wenshu looks in the terminal
