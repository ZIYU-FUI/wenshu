...
Schedule tasks to run automatically with natural language or cron expressions. Wenshu exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Wenshu itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
At creation, an unpinned job (one you don't give an explicit `provider`/`model`) follows the global default selected by `wenshu model` — and Wenshu **snapshots** that provider and model on the job. If the global default later changes, the job **fails closed**: it skips the run, makes no inference call, and sends an alert telling you to pin the provider/model explicitly (`cronjob action=update job_id=… provider=… model=…`) to proceed. This prevents an unattended job from silently inheriting a switch to a paid provider/model and spending money you didn't intend (#44585). To make a job deliberately track your global default, pin it to the new values after changing them. `wenshu setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Wenshu disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
wenshu cron create "every 2h" "Check server status"
wenshu cron create "every 1h" "Summarize new feed items" --skill blogwatcher
wenshu cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Wenshu will use the unified `cronjob` tool internally.
...
wenshu cron create "every 1d at 09:00" \
...
```bash
wenshu cron edit <job_id> --schedule "every 4h"
wenshu cron edit <job_id> --prompt "Use the revised task"
wenshu cron edit <job_id> --skill blogwatcher --skill maps
wenshu cron edit <job_id> --add-skill maps
wenshu cron edit <job_id> --remove-skill blogwatcher
wenshu cron edit <job_id> --clear-skills
```
...
```bash
wenshu cron list
wenshu cron pause <job_id_or_name>
wenshu cron resume <job_id_or_name>
wenshu cron run <job_id_or_name>
wenshu cron remove <job_id_or_name>
wenshu cron edit <job_id_or_name> [...flags]
wenshu cron status
wenshu cron tick
```
...
```bash
wenshu gateway install     # Install as a user service
sudo wenshu gateway install --system   # Linux: boot-time system service for servers
wenshu gateway             # Or run in foreground
...
1. loads jobs from `~/.wenshu-hermes/cron/jobs.json`
...
A file lock at `~/.wenshu-hermes/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Wenshu records each claimed cron attempt in the profile-local
`~/.wenshu-hermes/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Wenshu marks an abandoned
...
Inspect recent attempts with `wenshu cron runs [job-id] --limit 20` (alias:
...
| `"local"` | Save to local files only (`~/.wenshu-hermes/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.wenshu-hermes/cron/output/`), but no message is sent to the delivery target.
...
Or set the `WENSHU_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.wenshu-hermes/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Wenshu directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Wenshu will write the check script to `~/.wenshu-hermes/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.wenshu-hermes/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.wenshu-hermes/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.wenshu-hermes/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.wenshu-hermes/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Wenshu reads Job 1's most recent output from `~/.wenshu-hermes/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `wenshu tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `wenshu tools` cron-platform config wins; otherwise Wenshu falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Wenshu should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.wenshu-hermes/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.wenshu-hermes/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.wenshu-hermes/scripts/flag-ready.sh
...
```python
#!/usr/bin/env python
# ~/.wenshu-hermes/scripts/new-rows.py
import json, sqlite3
...
:::tip
Wenshu's own `~/.wenshu-hermes/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::
...
Jobs are stored in `~/.wenshu-hermes/cron/jobs.json`. Output from job runs is saved to `~/.wenshu-hermes/cron/output/{job_id}/{timestamp}.md`.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `wenshu cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `WENSHU_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Wenshu resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
