...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Wenshu's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Wenshu instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
wenshu memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `wenshu` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Wenshu syncs them back at session init.
...
The `wenshu honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `wenshu memory setup honcho` (or run `wenshu memory setup` and pick it from the list); the `wenshu honcho` subcommand then appears on the next invocation.

```bash
wenshu memory setup honcho    # Configure Honcho directly (works before activation)
wenshu honcho status          # Connection status, config, and key settings
wenshu honcho setup           # Redirects to `wenshu memory setup` (post-activation alias)
wenshu honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
wenshu honcho peer            # Show or update peer names + dialectic reasoning level
wenshu honcho mode            # Show or set recall mode (hybrid/context/tools)
wenshu honcho tokens          # Show or set token budget for context and dialectic
wenshu honcho identity        # Seed or show the AI peer's Honcho identity
wenshu honcho sync            # Sync Honcho config to all existing profiles
wenshu honcho peers           # Show peer identities across all profiles
wenshu honcho sessions        # List known Honcho session mappings
wenshu honcho map             # Map current directory to a Honcho session name
wenshu honcho enable          # Enable Honcho for the active profile
wenshu honcho disable         # Disable Honcho for the active profile
wenshu honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `wenshu honcho`

If you previously used the standalone `wenshu honcho setup`:
...
No re-login or re-setup needed. Run `wenshu memory setup` and select "honcho" — the wizard detects your existing config.
