...
When Wenshu Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.wenshu-hermes/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.wenshu-hermes/config.yaml` (switchable per session
via `/personality`).
...
If you use Wenshu' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.wenshu-hermes/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Wenshu Agent's deliverable
