...
description: "Connect Wenshu Agent to external tool servers via MCP — and control exactly which MCP tools Wenshu loads"
---

# MCP (Model Context Protocol)

MCP lets Wenshu Agent connect to external tool servers so the agent can use tools that live outside Wenshu itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Wenshu to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Wenshu tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Wenshu to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.wenshu-hermes/config.yaml`:
...
4. Ask Wenshu to use the MCP-backed capability.
...
Wenshu will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Wenshu ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
wenshu mcp                # interactive picker (default)
wenshu mcp catalog        # plain-text list, scriptable
wenshu mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Wenshu
...
`optional-mcps/` in the wenshu-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Wenshu prompts at install time and writes the value to
  `~/.wenshu-hermes/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Wenshu points you at
  `wenshu auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Wenshu probes the MCP server to list every
...
written (if not). Re-run `wenshu mcp configure <name>` once the server is
reachable to refine.
...
the wenshu-agent repo, so Nous has reviewed each entry before it shipped —
...
PR adds an entry with a newer `manifest_version` than your installed Wenshu
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Wenshu`) for that entry instead of silently hiding it. Run `wenshu update`
to install the latest Wenshu when you see that.
...
from environment variables (which include everything in `~/.wenshu-hermes/.env`).
...
MCPs are never auto-updated. Re-run `wenshu mcp install <name>` to refresh
after a Wenshu update if a manifest version changed.
...
HTTP MCP servers are remote endpoints Wenshu connects to directly.
...
- you do not want Wenshu spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Wenshu handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Wenshu prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.wenshu-hermes/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Wenshu runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Wenshu prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Wenshu on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Wenshu expects them.
...
Then run `wenshu mcp login googledrive` — with the pre-registered client, Wenshu skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.wenshu-hermes/config.yaml` from inside a running Wenshu session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `wenshu mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Wenshu passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
Wenshu reads MCP config from `~/.wenshu-hermes/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Wenshu skips the server entirely |
...
For well-known MCP servers, `wenshu mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
wenshu mcp add codex --preset codex
```
...
You can pick any local name (`wenshu mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Wenshu registers MCP tools

Wenshu prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Wenshu sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Wenshu also registers utility tools around MCP resources and prompts:
...
- Wenshu only registers resource utilities if the MCP session actually supports resource operations
- Wenshu only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Wenshu, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Wenshu skips the server completely and does not even attempt a connection.
...
You can also separately disable Wenshu-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Wenshu does not create an empty runtime MCP toolset for that server.
...
Wenshu discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Wenshu when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Wenshu receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Wenshu does not blindly pass your full shell environment.
...
Then verify your config and restart Wenshu.
...
Because Wenshu now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Wenshu may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Wenshu via the `sampling/createMessage` protocol. This allows an MCP server to ask Wenshu to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Wenshu as an MCP server

In addition to connecting **to** MCP servers, Wenshu can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Wenshu's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Wenshu
- You want a single MCP server that bridges to all of Wenshu's connected messaging platforms at once
...
Add Wenshu to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Wenshu in a specific location:

```json
{
  "mcpServers": {
    "wenshu": {
      "command": "/home/user/.wenshu-hermes/wenshu-agent/venv/bin/wenshu",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Wenshu-specific channel browser:
...
The MCP server includes a live event bridge that polls Wenshu's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
wenshu mcp serve              # Normal mode
wenshu mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Wenshu's session store (`~/.wenshu-hermes/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `wenshu send` CLI.
...
- The embedded `wenshu mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Wenshu, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
