...
Wenshu Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.wenshu-hermes/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Wenshu stores session history so it can resume conversations, but it does not
...
Wenshu explicitly injects for that turn.
...
For example, if a user sends an image and asks Wenshu to make a meme from it,
Wenshu may inspect that image once with vision and run an image-processing
...
`wenshu sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`wenshu` or `wenshu chat`) |
...
wenshu -c "my project"   # → resumes "my project #3"
```
...
wenshu --resume 20250305_091523_a1b2c3d4
wenshu -r 20250305_091523_a1b2c3d4

# Resume by title
wenshu --resume "refactoring auth"

# Or with the chat subcommand
wenshu chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `wenshu sessions list`.

### Conversation Recap on Resume

When you resume a session, Wenshu displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Wenshu session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.wenshu-hermes/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `wenshu -r "<title>"` from the shell) and pick up where the platform left off.
...
Wenshu automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `wenshu sessions list` or `wenshu sessions browse`.
...
```bash
wenshu sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Wenshu creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`wenshu -c "my project"`), it automatically picks the most recent session in the lineage.
...
Wenshu provides a full set of session management commands via `wenshu sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
wenshu sessions list

# Filter by platform
wenshu sessions list --source telegram

# Show more sessions
wenshu sessions list --limit 50
```
...
`wenshu sessions export` is one surface for every export format, selected with `--format`:
...
wenshu sessions export backup.jsonl

# Export sessions from a specific platform
wenshu sessions export telegram-history.jsonl --source telegram

# Export a single session
wenshu sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
wenshu sessions export backup.jsonl --redact
```
...
wenshu sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
wenshu sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
wenshu sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
wenshu sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `wenshu-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
wenshu sessions export --format trace

# One session to a local trace file
wenshu sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
wenshu sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.wenshu-hermes/session-exports`).

```bash
# Export one session to Markdown
wenshu sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
wenshu sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
wenshu sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
wenshu sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
wenshu sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
wenshu sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
wenshu sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
wenshu sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
wenshu sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
wenshu sessions prune

# Custom age threshold — bare numbers are days
wenshu sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
wenshu sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
wenshu sessions prune --newer-than 5h

# Explicit window with absolute timestamps
wenshu sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
wenshu sessions prune --source telegram
wenshu sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
wenshu sessions prune --newer-than 5h --title "smoke test"   # title substring
wenshu sessions prune --older-than 30 --max-messages 3        # tiny sessions
wenshu sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
wenshu sessions prune --model gpt-5 --older-than 1w           # by model (substring)
wenshu sessions prune --provider openrouter --older-than 60   # by billing provider
wenshu sessions prune --branch feature/old-experiment         # by git branch
wenshu sessions prune --user 12345678 --chat-type group       # by messaging origin
wenshu sessions prune --max-tokens 500 --older-than 7         # by token usage
wenshu sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
wenshu sessions prune --newer-than 5h --dry-run

# Skip confirmation
wenshu sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `wenshu sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `wenshu sessions prune` keeps the 90-day cutoff. Every
...
`wenshu sessions archive` takes the same filters as `prune` but soft-hides
...
wenshu sessions archive --newer-than 5h

# Archive by title substring, preview first
wenshu sessions archive --title "dry run" --dry-run
wenshu sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `wenshu sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`wenshu sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
wenshu sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`wenshu insights`](/reference/cli-commands#wenshu-insights).
...
When Wenshu cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Wenshu uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Wenshu in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.wenshu-hermes/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.wenshu-hermes/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.wenshu-hermes/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.wenshu-hermes/sessions/sessions.json` is the **gateway routing index** — it maps
...
`wenshu sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.wenshu-hermes/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `wenshu sessions list`, the cause is
`state.db` not receiving them — run `wenshu sessions repair` and watch for a
...
`*.jsonl` files in `~/.wenshu-hermes/sessions/`. They are no longer written or
read by Wenshu. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Wenshu process in the same `WENSHU_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.wenshu-hermes/config.yaml`:
...
wenshu sessions delete <session_id>
