...
title: "Git Worktrees"
description: "Run multiple Wenshu agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Wenshu Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Wenshu so each session has a clean, isolated working directory.

## Why Use Worktrees with Wenshu?

Wenshu treats the **current working directory** as the project root:

- CLI: the directory where you run `wenshu` or `wenshu chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.wenshu-hermes/config.yaml`
...
git worktree add ../repo-feature feature/wenshu-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/wenshu-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Wenshu there:

```bash
cd ../repo-feature

# Start Wenshu in the worktree
wenshu
```
...
git worktree add ../repo-experiment-a feature/wenshu-a
git worktree add ../repo-experiment-b feature/wenshu-b
```
...
- Works on its own branch (`feature/wenshu-a` vs `feature/wenshu-b`).
...
- Wenshu checkpoint data under `~/.wenshu-hermes/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Wenshu experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/wenshu-checkpoints-docs`, `feature/wenshu-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Wenshu from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `wenshu -w` (Automatic Worktree Mode)

Wenshu has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `wenshu/wenshu-<hash>`).
...
```bash
wenshu -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `wenshu -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Wenshu session its own clean checkout.
