...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Wenshu Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Wenshu setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Wenshu Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Wenshu Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Wenshu for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Wenshu working on my machine | `wenshu setup` | Run a real chat and verify it responds |
| I already know my provider | `wenshu model` | Save the config, then start chatting |
| I want a bot or always-on setup | `wenshu gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `wenshu model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `wenshu model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Wenshu cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Wenshu Agent
...
The single most important setup step. Use `wenshu model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `wenshu setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `wenshu update`. Re-enable anything later with `wenshu tools`, seed skills with `wenshu skills opt-in --sync`, or tune settings with `wenshu setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `wenshu model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `wenshu model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `wenshu model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `wenshu_cli/models.py` may change between releases) | `wenshu model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `wenshu model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `wenshu model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `wenshu model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `wenshu model` (requires `copilot` CLI + `copilot login`) |
...
Wenshu Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `wenshu model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Wenshu separates secrets from normal config:

- **Secrets and tokens** → `~/.wenshu-hermes/.env`
- **Non-secret settings** → `~/.wenshu-hermes/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
wenshu config set model anthropic/claude-opus-4.6
wenshu config set terminal.backend docker
wenshu config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
wenshu            # classic CLI
wenshu --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Wenshu ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `wenshu` vs `wenshu --tui`.
:::
...
```bash
wenshu --continue    # Resume the most recent session
wenshu -c            # Short form
```
...
```bash
wenshu gateway setup    # Interactive platform configuration
```
...
- `wenshu tools` — tune tool access per platform
- `wenshu skills` — browse and install reusable workflows
...
```bash
wenshu config set terminal.backend docker    # Docker isolation
wenshu config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Wenshu install directory (the curl installer placed it at
# ~/.wenshu-hermes/wenshu-agent on Linux/macOS or %LOCALAPPDATA%\wenshu\wenshu-agent on Windows):
cd ~/.wenshu-hermes/wenshu-agent
...
Skills are on-demand instruction documents that teach Wenshu how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Wenshu ships with a catalog of bundled skills already installed in `~/.wenshu-hermes/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
wenshu skills browse                      # list everything available
wenshu skills search kubernetes           # find skills by keyword
wenshu skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `wenshu skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Wenshu ask what you need
```
...
```yaml
# Add to ~/.wenshu-hermes/config.yaml
mcp_servers:
  github:
    command: npx
...
(If you installed without `[all]`, run `cd ~/.wenshu-hermes/wenshu-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Wenshu opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `wenshu model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `wenshu gateway setup` and check `wenshu gateway status` |
| `wenshu --continue` can't find old session | Switched profiles or session never saved | Check `wenshu sessions list` and confirm you're in the right profile |
...
| `wenshu doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `wenshu model`
3. `wenshu setup`
4. `wenshu sessions list`
5. `wenshu --continue`
6. `wenshu gateway status`
...
| Command | Description |
|---------|-------------|
| `wenshu` | Start chatting |
| `wenshu model` | Choose your LLM provider and model |
| `wenshu tools` | Configure which tools are enabled per platform |
| `wenshu setup` | Full setup wizard (configures everything at once) |
| `wenshu doctor` | Diagnose issues |
| `wenshu update` | Update to latest version |
| `wenshu gateway` | Start the messaging gateway |
| `wenshu --continue` | Resume last session |
