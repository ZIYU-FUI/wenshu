...
description: "Run Wenshu Agent directly on an Android phone with Termux"
---

# Wenshu on Android with Termux
...
Wenshu Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Wenshu from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Wenshu now ships a Termux-aware installer path:
...
- links `wenshu` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `wenshu` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/wenshu" "$PREFIX/bin/wenshu"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `wenshu` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.wenshu-hermes/.env`.
...
### `wenshu doctor` says ripgrep or Node is missing
