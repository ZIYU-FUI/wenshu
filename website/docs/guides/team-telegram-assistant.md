...
This tutorial walks you through setting up a Telegram bot powered by Wenshu Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Wenshu Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.wenshu-hermes/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Wenshu itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Wenshu Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_wenshu_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Wenshu Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.wenshu-hermes/.env`:
...
```
[Gateway] Starting Wenshu Gateway...
...
# View live logs
journalctl --user -u wenshu-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `wenshu gateway install` to update the plist.
:::
...
```bash
wenshu gateway stop && wenshu gateway start
```
...
wenshu pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.wenshu-hermes/.env`:
...
Control how much detail the bot shows when using tools. In `~/.wenshu-hermes/config.yaml`:
...
Customize how the bot communicates by editing `~/.wenshu-hermes/SOUL.md`:

For a full guide, see [Use SOUL.md with Wenshu](/guides/use-soul-with-wenshu).
...
```bash
# From the CLI
wenshu cron list          # View all scheduled jobs
wenshu cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u wenshu-gateway` (Linux) or `~/.wenshu-hermes/logs/gateway.log` (macOS) |
| Cron job output | `~/.wenshu-hermes/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.wenshu-hermes/cron/jobs.json` |
| Pairing data | `~/.wenshu-hermes/pairing/` |
| Session history | `~/.wenshu-hermes/sessions/` |
