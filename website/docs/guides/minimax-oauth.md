...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Wenshu Agent — no API key required"
---

# MiniMax OAuth

Wenshu Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Wenshu automatically refreshes your session.
...
# → Wenshu opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.wenshu-hermes/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.wenshu-hermes/.env
```
...
```bash
wenshu auth add minimax-oauth --no-browser
```

Wenshu will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Wenshu implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Wenshu generates a PKCE verifier / challenge pair and a random state value.
...
4. Wenshu polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.wenshu-hermes/auth.json` under the `minimax-oauth` key.
...
```bash
wenshu config set model.default MiniMax-M2.7
wenshu config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.wenshu-hermes/config.yaml` will contain entries similar to:
...
```bash
wenshu --provider minimax-oauth    # canonical
wenshu --provider minimax-portal   # alias
wenshu --provider minimax-global   # alias
wenshu --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `wenshu setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
wenshu --provider minimax-oauth
```
...
Wenshu refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Wenshu marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Wenshu marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `wenshu auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Wenshu raises a timeout error.

**Fix:** re-run `wenshu auth add minimax-oauth` (or `wenshu model`). The flow starts fresh.

### State mismatch (possible CSRF)

Wenshu detected that the `state` value returned by the authorization server does not match what it sent.
...
If `wenshu` cannot open a browser window, use `--no-browser`:

```bash
wenshu auth add minimax-oauth --no-browser
```

Wenshu prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `wenshu model` and select MiniMax (OAuth), or run `wenshu auth add minimax-oauth`.
...
```bash
wenshu auth logout minimax-oauth
```
...
- [wenshu doctor](../reference/cli-commands.md)
