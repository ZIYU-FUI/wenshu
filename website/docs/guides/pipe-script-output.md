...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `wenshu send`."
---

# Pipe Script Output to Messaging Platforms

`wenshu send` is a small, scriptable CLI that pushes a message to any
messaging platform Wenshu is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | wenshu send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `wenshu
...
wenshu send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
wenshu send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
wenshu send --list

# Filter by platform
wenshu send --list telegram
```
...
Any platform Wenshu ships adapters for works as a target:
...
`wenshu send` resolves the message body in this order:

1. **Positional argument** — `wenshu send --to telegram "hi"`
2. **`--file PATH`** — `wenshu send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | wenshu send --to telegram`

When stdin is a TTY (no pipe), Wenshu does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  wenshu send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `wenshu send` reuses your Wenshu config, the same script works on
any host where Wenshu is installed — no need to export bot tokens into
...
`wenshu send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  wenshu send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | wenshu send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.wenshu-hermes/bin/wenshu send \
...
```bash
./train.py --epochs 200 && \
  wenshu send --to telegram "training done" || \
  wenshu send --to telegram "training failed (exit $?)"
```
...
wenshu send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(wenshu send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `wenshu send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `wenshu send` calls
the platform's REST endpoint directly using credentials from
`~/.wenshu-hermes/.env` and `~/.wenshu-hermes/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `wenshu gateway start` and retry.
...
# Machine-readable
wenshu send --list --json
```

The listing is built from `~/.wenshu-hermes/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`wenshu gateway
...
| Approach | Multi-platform | Reuses Wenshu creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `wenshu send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`wenshu send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `wenshu send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `wenshu send` shares with cron delivery.
