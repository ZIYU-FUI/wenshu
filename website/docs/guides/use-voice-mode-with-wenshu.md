---
sidebar_position: 8
title: "Use Voice Mode with Wenshu"
description: "A practical guide to setting up and using Wenshu voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Wenshu
...
- you want Wenshu sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Wenshu.
...
## Step 1: make sure normal Wenshu works first
...
Wenshu supports both local and cloud speech stacks.
...
### If you use `wenshu setup`

If you choose NeuTTS in the setup wizard, Wenshu checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Wenshu transcribes and responds
...
- asking Wenshu to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Wenshu loop.

## Tuning CLI behavior

### Silence threshold

If Wenshu starts/stops too aggressively, tune:
...
Wenshu stays a normal chat bot, but can speak replies.
...
- you want Wenshu to function like a portable research or ops assistant
...
Wenshu joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Wenshu detects speech boundaries
- transcripts are posted in the associated text channel
- Wenshu responds in text and audio
...
1. get text Wenshu working
2. install `wenshu-agent[voice]`
