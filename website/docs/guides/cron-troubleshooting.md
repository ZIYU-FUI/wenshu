...
description: "Diagnose and fix common Wenshu cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`wenshu gateway` for foreground, or `wenshu gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `wenshu cron tick`.
...
```bash
date
wenshu cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.wenshu-hermes/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.wenshu-hermes/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.wenshu-hermes/.env` |
...
| `local` | Write access to `~/.wenshu-hermes/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `wenshu cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `wenshu skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `wenshu skills list`.
...
2. `~/.wenshu-hermes/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `wenshu cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Wenshu config directory). Verify:
```bash
ls ~/.wenshu-hermes/scripts/your-script.py   # Must exist
wenshu cron edit <job_id> --script ~/.wenshu-hermes/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `wenshu skills install <skill-name>`.
...
Jobs are stored in `~/.wenshu-hermes/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.wenshu-hermes/cron/jobs.json
chmod 600 ~/.wenshu-hermes/cron/jobs.json   # Your user should own it
```
...
```bash
wenshu cron list                    # Show all jobs, states, next_run times
wenshu cron run <job_id>            # Schedule for next tick (for testing)
wenshu cron edit <job_id>           # Fix configuration issues
wenshu logs                         # View recent Wenshu logs
wenshu skills list                  # Verify installed skills
```
...
1. Run the job with `wenshu cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.wenshu-hermes/logs/agent.log` for scheduler messages and `~/.wenshu-hermes/logs/errors.log` for warnings
