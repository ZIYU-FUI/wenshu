...
```bash
wenshu teams-pipeline validate
```
...
```bash
wenshu teams-pipeline token-health
wenshu teams-pipeline token-health --force-refresh
```
...
```bash
wenshu teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
wenshu teams-pipeline maintain-subscriptions
wenshu teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Wenshu cron (recommended if you already run the Wenshu gateway)

Wenshu ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.wenshu-hermes/scripts/`. First create the script:

```bash
mkdir -p ~/.wenshu-hermes/scripts
cat > ~/.wenshu-hermes/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec wenshu teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.wenshu-hermes/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
wenshu cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/wenshu-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Wenshu Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=wenshu
EnvironmentFile=/etc/wenshu/env
ExecStart=/usr/local/bin/wenshu teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/wenshu-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Wenshu Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now wenshu-teams-pipeline-maintain.timer
systemctl list-timers wenshu-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/wenshu teams-pipeline maintain-subscriptions >> /var/log/wenshu/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.wenshu-hermes/.env` at the top of a wrapper script that crontab calls.
...
```bash
wenshu teams-pipeline subscriptions   # should show expirationDateTime advanced
wenshu teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
wenshu teams-pipeline list
wenshu teams-pipeline list --status failed
wenshu teams-pipeline show <job-id>
```

### Replay a stored job

```bash
wenshu teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
wenshu teams-pipeline fetch --meeting-id <meeting-id>
wenshu teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `wenshu teams-pipeline maintain-subscriptions --dry-run`
- inspect `wenshu teams-pipeline list --status failed`
...
- run `wenshu teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `wenshu teams-pipeline run`
...
- [ ] `wenshu teams-pipeline validate` returns an OK snapshot
- [ ] `wenshu teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Wenshu cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
