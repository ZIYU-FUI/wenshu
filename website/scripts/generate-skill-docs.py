...
def render_skill_page(
    ...
    wenshu_meta = (fm.get("metadata") or {}).get("wenshu") or {}
    tags = wenshu_meta.get("tags") or []
    related = wenshu_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `wenshu skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Wenshu Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Wenshu ships with a large built-in skill library copied into `~/.wenshu-hermes/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Wenshu also syncs bundled skills on `wenshu update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.wenshu-hermes/skills/` tree, it is still shipped with Wenshu; restore it with `wenshu skills reset <name> --restore`.",
        "",
