#!/usr/bin/env python3
"""Live test harness for Wenshu Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any wenshu imports.
ORIGINAL_HOME = os.environ.get("WENSHU_HOME")
ORIGINAL_AUTH = Path.home() / ".wenshu-hermes" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.wenshu-hermes/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.wenshu-hermes/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="wenshu_ts_live_"))
    wenshu_home = home_dir / ".wenshu-hermes"
    wenshu_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, wenshu_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".wenshu-hermes" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, wenshu_home / ".env")
        ...
        load_wenshu_dotenv(wenshu_home=str(Path.home() / ".wenshu-hermes"))
    ...
    (wenshu_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return wenshu_home
...
def reset_module_state():
    """Drop cached modules so the new WENSHU_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "wenshu_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["WENSHU_HOME"] = str(home)
...
def main():
    ...
    # Restore original WENSHU_HOME
    if ORIGINAL_HOME is not None:
        os.environ["WENSHU_HOME"] = ORIGINAL_HOME
    else:
