#!/usr/bin/env bash
# Canonical test runner for wenshu-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.wenshu-hermes/...)
#
# Usage:
...
# (WENSHU_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.wenshu-hermes/wenshu-agent/venv"; do
...
elif [ -n "${WENSHU_PYTHON:-}" ] && [ -x "$WENSHU_PYTHON" ] \
    && "$WENSHU_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: WENSHU_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `wenshu` binary rather
  # than the devShell hook.
  PYTHON="$WENSHU_PYTHON"
  echo "▶ no local venv — using Nix dev venv via WENSHU_PYTHON: $PYTHON"
else
  echo "error: no virtualenv found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and WENSHU_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  exit 1
fi
...
if [ -f "$HOME/.wenshu-hermes/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.wenshu-hermes"
...
exec env -i \
  ...
  LC_ALL=C.UTF-8 \
  PYTHONHASHSEED=0 \
  ${WENSHU_RUN_SLOW_PET_TESTS:+WENSHU_RUN_SLOW_PET_TESTS="$WENSHU_RUN_SLOW_PET_TESTS"} \
