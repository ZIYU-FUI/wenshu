...
When Wenshu runs in TUI mode, the gateway child process communicates with
...
# ``get_wenshu_home() / "plugins"`` (user) and ``./.wenshu-hermes/plugins/`` (project,
# gated behind ``WENSHU_ENABLE_PROJECT_PLUGINS``) — see
# ``wenshu_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``wenshu_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import wenshu_constants (this script
    ...
    # plugins in get_wenshu_home()/plugins/ and project plugins in
    # ./.wenshu-hermes/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_wenshu_home() / "plugins"]
    if os.environ.get("WENSHU_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".wenshu-hermes" / "plugins")
