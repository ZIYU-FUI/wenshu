...
VERSION_FILE = REPO_ROOT / "wenshu_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@wenshu.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.wenshu-hermes": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "contato@siteup.com.br": "SiteupAgencia",  # PR #57435 salvage (desktop chat transport: back off notification poller when session is busy; #55578)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level WENSHU_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (wenshu journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "wenshu.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.wenshu-hermes/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows wenshu-shim resolution + prefer --update on recovery; #52378)
    ...
    "wenshu@marian.local": "Schrotti77",
    ...
    "274096618+wenshu-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "wenshu@nousresearch.com": "NousResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on wenshu mcp add)
    ...
    "wenshu@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyawenshu.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (WENSHU_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (WENSHU_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: wenshu-eval community link)
    ...
    "wenshuagent26@gmail.com": "wenshuagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+wenshuagent26@users.noreply.github.com": "wenshuagent26",
    ...
    "vigo@wenshu": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.wenshu@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Wenshu runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level WENSHU_HOME state files)
    ...
    "zhchl@wenshu-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Wenshu
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"wenshu-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "wenshu@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/hermes-agent",
    ...
    lines.append(f"# Wenshu Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Wenshu Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Wenshu Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Wenshu Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Wenshu Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Wenshu Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Wenshu Agent v{new_version} ({calver_date})' "
