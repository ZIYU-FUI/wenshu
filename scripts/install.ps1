# ============================================================================
# Wenshu Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$WenshuHome = $(if ($env:WENSHU_HOME) { $env:WENSHU_HOME } else { "$env:LOCALAPPDATA\wenshu-wenshu" }),
    [string]$InstallDir = $(if ($env:WENSHU_HOME) { "$env:WENSHU_HOME\wenshu-agent" } else { "$env:LOCALAPPDATA\wenshu-wenshu\wenshu-agent" }),
    ...
    # builds apps/desktop into a launchable Wenshu.exe.
    #
    # Why opt-in:
    #   * Wenshu-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Wenshu.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Wenshu.exe
    ...
    #     `wenshu desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "|             * Wenshu Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $WenshuHome)) {
        New-Item -ItemType Directory -Force -Path $WenshuHome | Out-Null
    }
    $envFile = Join-Path $WenshuHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $WenshuHome "node"
...
function Install-Uv {
    # Wenshu owns its own uv at $WenshuHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (wenshu_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `wenshu update` stay in sync.
    $managedUv = Join-Path $WenshuHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $WenshuHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $WenshuHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $WenshuHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $WenshuHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and WENSHU_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Wenshu-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Wenshu setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Wenshu uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\wenshu\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Wenshu needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Wenshu copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\wenshu\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``WENSHU_GIT_BASH_PATH`` (User scope) so Wenshu can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Wenshu-managed PortableGit install instead..."
    }

    # Download PortableGit into $WenshuHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $WenshuHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Wenshu features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Wenshu needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``WENSHU_GIT_BASH_PATH`` (User env scope) so Wenshu can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $WenshuHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $WenshuHome\git\usr\bin\bash.exe
    $candidates += "$WenshuHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$WenshuHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("WENSHU_GIT_BASH_PATH", $candidate, "User")
            $env:WENSHU_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set WENSHU_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Wenshu may not find Git Bash."
    Write-Info "If needed, set WENSHU_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$WenshuHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$WenshuHome\node;$env:Path"
        Write-Success "Node.js $version found (Wenshu-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Wenshu-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $WenshuHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $WenshuHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\wenshu-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$WenshuHome\node") { Remove-Item -Recurse -Force "$WenshuHome\node" }
                Move-Item $extractedDir.FullName "$WenshuHome\node"
                ...
                $version = & "$WenshuHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $WenshuHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\wenshu-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `wenshu
                    # update` path (#4735).
                    ...
                    $stashName = "wenshu-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``wenshu update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Wenshu behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running wenshu processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/ZIYU-FUI/wenshu/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/ZIYU-FUI/wenshu/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/ZIYU-FUI/wenshu/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\wenshu-$zipLabel.zip"
                $extractPath = "$env:TEMP\wenshu-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `wenshu update` (see the notes at the shared clone-path
    ...
    # next `wenshu update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Wenshu-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running wenshu process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running wenshu processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Wenshu_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (wenshu.exe) plus its child tree.
            & taskkill /F /T /IM wenshu.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM wenshu.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m wenshu_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not wenshu.exe.
            ...
            # user (see wenshu_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `wenshu update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `wenshu.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install wenshu-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and wenshu.exe broken with
    ...
    # here, not 30 seconds later when the user runs `wenshu`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register wenshu.exe in dist-info/RECORD but fail to
        ...
        # `wenshu` command while wenshu-agent.exe / wenshu-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m wenshu_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `wenshu dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\wenshu_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `wenshu dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `wenshu dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: wenshu_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up wenshu command..."
    
    if ($NoVenv) {
        $wenshuBin = "$InstallDir"
    } else {
        $wenshuBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so wenshu is globally available
    # On Windows, the wenshu.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$wenshuBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$wenshuBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $wenshuBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set WENSHU_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\wenshu instead
    # of the Unix default ~/.wenshu
    $currentWenshuHome = [Environment]::GetEnvironmentVariable("WENSHU_HOME", "User")
    if (-not $currentWenshuHome -or $currentWenshuHome -ne $WenshuHome) {
        [Environment]::SetEnvironmentVariable("WENSHU_HOME", $WenshuHome, "User")
        Write-Success "Set WENSHU_HOME=$WenshuHome"
    }
    $env:WENSHU_HOME = $WenshuHome
    
    # Update current session
    $env:Path = "$wenshuBin;$env:Path"
    
    Write-Success "wenshu command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.wenshu-bootstrap-complete which tells the Wenshu
    ...
    # Wenshu-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `wenshu update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".wenshu-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the WENSHU_HOME directory structure ($WenshuHome, default %LOCALAPPDATA%\wenshu)
    New-Item -ItemType Directory -Force -Path "$WenshuHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WenshuHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$WenshuHome\.env"
    ...
    # Create config.yaml
    $configPath = "$WenshuHome\config.yaml"
    if (-not (Test-Path $configPath)) {
    ...
    # (the default PS5 behaviour), and Wenshu's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$WenshuHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in wenshu_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $WenshuHome"
    
    # Seed bundled skills into $WenshuHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $WenshuHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $WenshuHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$WenshuHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $WenshuHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Wenshu-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'wenshu setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\wenshu-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\wenshu-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\wenshu-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Wenshu` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Wenshu.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_wenshu_desktop command
    # resolves apps/desktop/release/win-unpacked/Wenshu.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Wenshu-managed
    ...
    # The --dir mode produces an unpacked Wenshu.exe in
    ...
    # are NOT trying to work around). The Wenshu icon + product name are
    # stamped onto Wenshu.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\wenshu-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Wenshu.exe",
        "$desktopDir\release\win-arm64-unpacked\Wenshu.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Wenshu.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Wenshu icon + identity are stamped onto Wenshu.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, wenshu-agent#38216).
    ...
    #    Wenshu.exe. We deliberately do NOT point them at `wenshu desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Wenshu.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Wenshu.lnk')
        )
        ...
        # with the Wenshu icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.wenshu-hermes/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'wenshu setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m wenshu_cli.main setup
    } else {
        python -m wenshu_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $wenshuCmd = "$InstallDir\venv\Scripts\wenshu.exe"
    if (-not (Test-Path $wenshuCmd)) {
        $wenshuCmd = "wenshu"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$WenshuHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'wenshu whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: wenshu gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$WenshuHome\logs\gateway.log"
            Start-Process -FilePath $wenshuCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$WenshuHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: wenshu gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: wenshu gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$WenshuHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$WenshuHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$WenshuHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$WenshuHome\wenshu-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   wenshu              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   wenshu setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   wenshu config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   wenshu config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   wenshu gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   wenshu update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Wenshu repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Wenshu-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Wenshu to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
