...
it is automatically downloaded from GitHub releases to $WENSHU_HOME/bin/tirith.
...
def _get_wenshu_home() -> str:
    """Return the Wenshu home directory, respecting WENSHU_HOME env var."""
    return str(get_wenshu_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_wenshu_home(), ".tirith-install-failed")
...
def _wenshu_bin_dir() -> str:
    """Return $WENSHU_HOME/bin, creating it if needed."""
    d = os.path.join(_get_wenshu_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $WENSHU_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_wenshu_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $WENSHU_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $WENSHU_HOME/bin/tirith
    ...
    wenshu_bin = os.path.join(_wenshu_bin_dir(), "tirith")
    if os.path.isfile(wenshu_bin) and os.access(wenshu_bin, os.X_OK):
        _resolved_path = wenshu_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return wenshu_bin
