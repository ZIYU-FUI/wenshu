...
`mcp_tool.py`, and `wenshu_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# WenshuMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_wenshu_provider_class() -> Optional[type]:
    ...
    class WenshuMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._wenshu_server_name = server_name
            self._wenshu_home = ""
            ...
            self._wenshu_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`WenshuTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, WenshuTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._wenshu_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import WenshuTokenStorage
                        if isinstance(storage, WenshuTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._wenshu_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import WenshuTokenStorage
            if not isinstance(storage, WenshuTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``wenshu mcp reauth``.
            ...
            back to ``wenshu mcp reauth``.
            """
            try:
                if self._wenshu_preregistered:
                    return
    ...
    return WenshuMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_WENSHU_PROVIDER_CLS: Optional[type] = _make_wenshu_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._wenshu_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`WenshuMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _WENSHU_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `wenshu mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``wenshu mcp remove <name>`` and (indirectly) by
        ``wenshu mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, wenshu_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, wenshu_home=wenshu_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, wenshu_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, wenshu_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, wenshu_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(wenshu_home) / f"{_safe_filename(server_name)}.json"
            try:
