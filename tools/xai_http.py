...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``wenshu tools`` repaint, tool-registration scans,
    ...
    2. ``~/.wenshu-hermes/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``wenshu auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_wenshu_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.wenshu-hermes/.env`` first, then ``os.environ``.

    Wraps :func:`wenshu_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from wenshu_cli.config import get_env_value as _wenshu_get_env_value

        value = _wenshu_get_env_value(name)
...
def wenshu_xai_user_agent() -> str:
    """Return a stable Wenshu-specific User-Agent for xAI HTTP calls."""
    try:
        from wenshu_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Wenshu-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Wenshu config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Wenshu home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_wenshu_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Wenshu-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``wenshu_cli.config.get_env_value`` so keys
    stored in ``~/.wenshu-hermes/.env`` (the standard Wenshu location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("WENSHU_XAI_BASE_URL")
