#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Wenshu Skills Hub.
...
Used by wenshu_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _wenshu_home() -> Path:
    return get_wenshu_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _wenshu_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            wenshu_meta = metadata.get("wenshu", {})
            if isinstance(wenshu_meta, dict):
                tags = wenshu_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.wenshu-hermes/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/wenshu-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                wenshu_meta = meta_block.get("wenshu", {})
                if isinstance(wenshu_meta, dict):
                    tags = wenshu_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Wenshu centralized index source
# ---------------------------------------------------------------------------

WENSHU_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
WENSHU_INDEX_TTL = 6 * 3600  # 6 hours


def _wenshu_index_cache_file() -> Path:
    return _index_cache_dir() / "wenshu-index.json"


def _load_wenshu_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for WENSHU_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    wenshu_index_cache_file = _wenshu_index_cache_file()
    if wenshu_index_cache_file.exists():
        try:
            age = time.time() - wenshu_index_cache_file.stat().st_mtime
            if age < WENSHU_INDEX_TTL:
                return json.loads(wenshu_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Wenshu index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Wenshu index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Wenshu index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        wenshu_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        wenshu_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class WenshuIndexSource(SkillSource):
    """Skill source backed by the centralized Wenshu Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_wenshu_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "wenshu-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "wenshu-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        WenshuIndexSource(auth=auth), # Centralized index (search + resolved install paths)
