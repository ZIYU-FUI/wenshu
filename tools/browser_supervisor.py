...
One ``CDPSupervisor`` runs per Wenshu ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Wenshu logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "wenshu-dialog-bridge.invalid"
...
(() => {
  if (window.__wenshuDialogBridgeInstalled) return;
  window.__wenshuDialogBridgeInstalled = true;
  const ENDPOINT = "http://wenshu-dialog-bridge.invalid/";
