"""
`wenshu computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Wenshu side — the only
...
def _cua_child_env() -> Dict[str, str]:
    """cua-driver child env with the Wenshu telemetry policy applied.
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Wenshu provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `WENSHU_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver
    ...
    if driver_cmd is None:
        try:
            ...
            driver_cmd = os.environ.get("WENSHU_CUA_DRIVER_CMD") or "cua-driver"
