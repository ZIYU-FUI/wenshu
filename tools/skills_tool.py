...
# Cache validation (mirrors wenshu_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.wenshu-hermes/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set WENSHU_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped WENSHU_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from WENSHU_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set WENSHU_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (WENSHU_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("WENSHU_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("WENSHU_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_wenshu_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.wenshu-hermes/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``WENSHU_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``WENSHU_PLATFORM`` environment variable
    3. ``WENSHU_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from wenshu_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("WENSHU_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.wenshu-hermes/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``wenshu skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile WENSHU_HOME; the module-level
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_wenshu_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from wenshu_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: wenshu plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.wenshu-hermes/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `wenshu skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.wenshu.* first (agentskills.io convention), fall back to top-level
        wenshu_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            wenshu_meta = metadata.get("wenshu", {}) or {}

        tags = _parse_tags(wenshu_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            wenshu_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
