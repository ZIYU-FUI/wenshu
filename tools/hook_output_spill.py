...
* Never raises. Any I/O error (disk full, permission denied, missing
  WENSHU_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_wenshu_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: WENSHU_HOME env var, then ~/.wenshu
            home = os.environ.get("WENSHU_HOME") or os.path.expanduser("~/.wenshu-hermes")
