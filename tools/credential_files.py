...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.wenshu-hermes",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``WENSHU_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside WENSHU_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because WENSHU_HOME is exactly where
    ...
    """
    wenshu_home = _resolve_wenshu_home()

    # Reject absolute paths — they bypass the WENSHU_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to WENSHU_HOME)",
            relative_path,
        )
        return False

    host_path = wenshu_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape WENSHU_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, wenshu_home)
    ...
    # inside WENSHU_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.wenshu-hermes/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="wenshu-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching wenshu_constants.get_wenshu_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_wenshu_dir()`` for backward compatibility with old directory layouts.
    """
    from wenshu_constants import get_wenshu_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_wenshu_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.wenshu``,
    SSH ``<remote_home>/.wenshu``, etc.) and whether translation is wanted.
