...
# Must match wenshu_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``wenshu-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``wenshu-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. wenshu-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.wenshu-hermes/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_wenshu_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `wenshu -z` or a cron job, or a "
        ...
        try:
            ...
            _source = get_session_env("WENSHU_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("WENSHU_UI_SESSION_ID", "")
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and WENSHU_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'wenshu auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Wenshu config.

    Prefer the shared persistent loader because it follows the active
    WENSHU_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``WENSHU_IGNORE_USER_CONFIG=1`` (``wenshu chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("WENSHU_IGNORE_USER_CONFIG") == "1"
