...
Backend is selected during ``wenshu tools`` setup (web.backend in config.yaml).
When available, Wenshu can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Wenshu config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Wenshu' config/.env layer (``wenshu config set``, ``wenshu tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.wenshu-hermes/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``wenshu tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `wenshu tools` repaint.
        try:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `wenshu plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `wenshu tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`wenshu plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from wenshu_cli.config import get_env_value as _gev
