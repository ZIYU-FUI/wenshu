...
from wenshu_constants import agent_browser_runnable, get_wenshu_home
from utils import env_int, is_truthy_value
from wenshu_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Wenshu secret
...
def _build_browser_env() -> dict:
    ...
    Strips Wenshu-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``wenshu_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    """
    from tools.environments.local import wenshu_subprocess_env

    env = wenshu_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    wenshu_node_bin = str(wenshu_home / "node" / "bin")
    wenshu_node_root = str(wenshu_home / "node")
    wenshu_nm_bin = str(wenshu_home / "node_modules" / ".bin")
    return [wenshu_node_bin, wenshu_node_root, wenshu_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.wenshu-hermes/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.wenshu-hermes/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Wenshu silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Wenshu CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-wenshu_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed wenshu processes — this way every clean wenshu exit sweeps
    ...
    # Sweep orphans from other crashed wenshu processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live wenshu processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current wenshu PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live wenshu process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning wenshu process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced wenshu PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent wenshu instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-wenshu_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live wenshu process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Wenshu-managed
    ...
    # node_modules binary, which disappears on the next ``wenshu update`` and
    ...
    # Build an extended search PATH including Wenshu-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_wenshu_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_wenshu_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_wenshu_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this wenshu PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_wenshu_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = wenshu_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Wenshu
    ...
    from wenshu_constants import get_wenshu_dir
    screenshots_dir = get_wenshu_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/wenshu-agent:latest")
                else:
