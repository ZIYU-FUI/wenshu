...
<url>``) is spawned as a direct child of the Wenshu process. Wenshu's own
...
Wenshu process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Wenshu startup's opt-in
