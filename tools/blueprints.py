...
`wenshu skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.wenshu.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.wenshu.blueprint`` is a mapping containing
    ...
    wenshu = meta.get("wenshu") if isinstance(meta, dict) else None
    blueprint = wenshu.get("blueprint") if isinstance(wenshu, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.wenshu.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.wenshu.blueprint`` block) they can hand
    to ``wenshu skills publish`` to share. ``body`` is the plain-language
