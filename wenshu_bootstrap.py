"""Windows UTF-8 bootstrap for Wenshu entry points.
...
should be imported at the very top of every Wenshu entry point
(``wenshu``, ``wenshu-agent``, ``wenshu-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Wenshu modules.

    Wenshu ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Wenshu source root
    ...
    """
    root = src_root or os.environ.get("WENSHU_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``WENSHU_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("WENSHU_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import wenshu_bootstrap``
# (or ``from wenshu_bootstrap import apply_windows_utf8_bootstrap``) at
