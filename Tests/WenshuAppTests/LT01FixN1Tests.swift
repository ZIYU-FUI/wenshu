// LT01FixN1Tests.swift · 文枢 (Wenshu) · v0.02.0 LT-N1-cc + LT-N1-revise → LT-N1-merge
//
...
//   5. LayoutShellView.topLeft slot 挂的是 ProjectListView (降级为静态字符串检查)
//      LT-N1-merge (2026-08-11): 派单拍板 topLeft = ProjectListView (V0-fix-6
//      5 tab 容器真值, 5 tab Picker 在 LayoutShellView.topLeftHeaderBar 内),
//      不是 LT-N1 原案的 ProjectBrowserView (LT-N1 原案是 ProjectBrowserView
//      自挂 NavigationStack + 5 tab 让 push 只影响 topLeft, 沿 V0-fix-6 真值
//      NavigationStack 在 LayoutShellView 顶层, push 覆盖整 layout — 派单
//      LT-N1-merge 拍板)。
//
...
//
// LT-N1-merge: 静态字符串检查的 worktree 路径用 `#filePath` 动态计算 (本测试
// 文件路径 → 上 3 级 = 项目根), 不再硬编码 `t_4c608c99-lt01-n1-{cc,revise}`
// worktree 路径 — 那些 worktree 可能在 merge 后清理掉, 硬编码会失败。
//
...
final class LT01FixN1Tests: XCTestCase {
    ...
    /// LT-N1-merge: 项目根路径, 用 `#filePath` 动态计算 (本测试文件路径
    /// → 上 3 级 = 项目根)。 取代硬编码 `t_4c608c99-lt01-n1-{cc,revise}`
    /// worktree 路径 — 那些 worktree 可能在 merge 后清理掉, 硬编码会失败。
    /// 路径格式: `<project_root>/Tests/WenshuAppTests/LT01FixN1Tests.swift` →
    /// `<project_root>` = 上 3 级 (`LT01FixN1Tests.swift` → `LT01FixN1Tests/`
    /// → `WenshuAppTests/` → `Tests/` → `<project_root>`)。
    ...
    /// LT-N1-merge 拍板: LayoutShellView 的 topLeft slot 必须挂 ProjectListView
    /// (V0-fix-6 5 tab 容器, 5 tab Picker 在 LayoutShellView.topLeftHeaderBar
    /// 内, ProjectListView 接 @Binding activeTab 共享同一 state — 沿 V0-fix-6
    /// 真值)。 LT-N1 原案的 ProjectBrowserView (自挂 NavigationStack 让 push
    /// 只影响 topLeft) 在 LT-N1-merge 后改为 dead code (详见
    /// ProjectBrowserView.swift 头部 comment)。
    func testLayoutShellView_topLeft_isProjectListView() throws {
        let layoutShellPath = projectRoot.appendingPathComponent("Sources/WenshuApp/Views/Layout/LayoutShellView.swift").path
        let content = try String(contentsOfFile: layoutShellPath, encoding: .utf8)
        // LT-N1-merge: 验证 panel(.topLeft) 分支里 ProjectListView( 在
        // `id == .topLeft` 之后 (panel() 函数的 topLeft case 直接挂
        // ProjectListView, 而不是先挂 ProjectBrowserView)。 用顺序检查
        // 而非固定窗口 — 拍板 comment 行数会变, 窗口大小不可靠。
        XCTAssertTrue(content.contains("ProjectListView"),
                      "LayoutShellView 必须挂载 ProjectListView (派单 LT-N1-merge §3 集成拍板)")
        guard let topLeftSlotRange = content.range(of: "id == .topLeft") else {
            XCTFail("LayoutShellView 必须有 `id == .topLeft` 分支 (panel() 函数的 topLeft case)")
            return
        }
        // 找 `id == .topLeft` 之后最近的 `ProjectListView(`, 看是否出现
        // (panel() 函数里的 ProjectListView( 调用)。
        let afterTopLeft = content[topLeftSlotRange.upperBound...]
        XCTAssertTrue(afterTopLeft.contains("ProjectListView("),
                      "panel(.topLeft) 分支必须挂 ProjectListView(...) (派单 LT-N1-merge 拍板, 5 tab Picker 在 LayoutShellView.topLeftHeaderBar)")
    }
    ...
    func testProjectDetailView_usesPickerSegmented() throws {
        let detailPath = projectRoot.appendingPathComponent("Sources/WenshuApp/Views/Project/ProjectDetailView.swift").path
    ...
    func testChapterTreeView_requiresProjectId() throws {
        let chapterPath = projectRoot.appendingPathComponent("Sources/WenshuApp/Views/Project/ChapterTreeView.swift").path
