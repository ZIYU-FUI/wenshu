...
final class WorkspaceStateTests: XCTestCase {
    ...
    func testWorkspaceStateRoundTrip() throws {
        ...
        let decoded = try decoder.decode(LayoutTreeState.self, from: data)
    ...
    func testMigrationRetiresV1Wholesale() {
        // A v1 LayoutTreeState (= flat pane array) cannot be decoded
        ...
        let decoder = JSONDecoder()
        // v1 schema decode as v2 MUST fail (= the v2 LayoutTreeState
        // has a `root` field that v1 lacks).
        XCTAssertThrowsError(try decoder.decode(LayoutTreeState.self, from: data))
    }
}
