...
struct ChatViewAppendShortcutTests {
    ...
    @Test func hstack_invariant_preserved() throws {
        ...
        // layer; = the user-interactive controls). The hidden Button("Append last assistant to input") Button
        ...
        // input row). The invariant (= Button("Append last assistant to input") Button is OUTSIDE the
        // input row HStack) is now distributed across two files; = this
        // test verifies both halves exist (= .frame(minHeight: 44) is
        // only in ChatInputBarView; = Button("Append last assistant to input") is only in ChatView).
        ...
        // The Button("Append last assistant to input") keyboard shortcut Button lives ONLY in
        // ChatView (= the keyboard shortcut block is a sibling of
        // ScrollViewReader; = not inside the input row).
        #expect(chatViewSrc.contains("Button(\"Append last assistant to input\")"))
        #expect(!inputBarSrc.contains("Button(\"Append last assistant to input\")"))
    }
}