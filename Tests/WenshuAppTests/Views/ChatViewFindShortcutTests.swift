...
struct ChatViewFindShortcutTests {
    ...
    @Test func hstack_invariant_preserved() throws {
        ...
        // layer; = the user-interactive controls). The hidden Button("Find in conversation") Button
        ...
        // input row). The invariant (= Button("Find in conversation") Button is OUTSIDE the
        // input row HStack) is now distributed across two files; = this
        // test verifies both halves exist (= .frame(minHeight: 44) is
        // only in ChatInputBarView; = Button("Find in conversation") is only in ChatView).
        ...
        // The Button("Find in conversation") keyboard shortcut Button lives ONLY in
        // ChatView (= the keyboard shortcut block is a sibling of
        // ScrollViewReader; = not inside the input row).
        #expect(chatViewSrc.contains("Button(\"Find in conversation\")"))
        #expect(!inputBarSrc.contains("Button(\"Find in conversation\")"))
    }
}