...
struct ChatViewGroupShortcutTests {
    ...
    @Test func hstack_invariant_preserved() throws {
        ...
        // layer; = the user-interactive controls). The hidden Button("Group messages by date") Button
        ...
        // input row). The invariant (= Button("Group messages by date") Button is OUTSIDE the
        // input row HStack) is now distributed across two files; = this
        // test verifies both halves exist (= .frame(minHeight: 30) is
        // only in ChatInputBarView; = Button("Group messages by date") is only in ChatView).
        ...
        #expect(chatViewSrc.contains("Button(\"Group messages by date\")"))
        #expect(!inputBarSrc.contains("Button(\"Group messages by date\")"))
    }
}