...
struct ChatMessageViewElapsedTimeTests {
    ...
    @Test func formatElapsed_subsecond_one_decimal() {
        #expect(ChatMessageView.formatElapsed(0.3) == "0.3s")
        #expect(ChatMessageView.formatElapsed(0.99) == "1.0s")
    }

    @Test func formatElapsed_subminute_one_decimal() {
        #expect(ChatMessageView.formatElapsed(3.2) == "3.2s")
        #expect(ChatMessageView.formatElapsed(12.5) == "12.5s")
        #expect(ChatMessageView.formatElapsed(59.9) == "59.9s")
    }

    @Test func formatElapsed_minute_plus() {
        #expect(ChatMessageView.formatElapsed(60.0) == "1m 0s")
        #expect(ChatMessageView.formatElapsed(125.0) == "2m 5s")
        #expect(ChatMessageView.formatElapsed(3600.0) == "60m 0s")
    }

    @Test func formatElapsed_zero_returns_0() {
        #expect(ChatMessageView.formatElapsed(0.0) == "0.0s")
    }

    @Test func formatElapsed_negative_clamped_to_zero() {
        // Defensive: clock skew can produce negative intervals.
        #expect(ChatMessageView.formatElapsed(-1.5) == "0.0s")
    }
