...
struct ChatViewPrevChatShortcutTests {
    ...
    @Test func hstack_invariant_preserved() throws {
        ...
        // layer; = the user-interactive controls). The hidden Button("Previous chat") Button
        ...
        // input row). The invariant (= Button("Previous chat") Button is OUTSIDE the
        // input row HStack) is now distributed across two files; = this
        // test verifies both halves exist (= .frame(minHeight: 44) is
        // only in ChatInputBarView; = Button("Previous chat") is only in ChatView).
        ...
        // The Button("Previous chat") keyboard shortcut Button lives ONLY in
        // ChatView (= the keyboard shortcut block is a sibling of
        // ScrollViewReader; = not inside the input row).
        #expect(chatViewSrc.contains("Button(\"Previous chat\")"))
        #expect(!inputBarSrc.contains("Button(\"Previous chat\")"))
    }
}