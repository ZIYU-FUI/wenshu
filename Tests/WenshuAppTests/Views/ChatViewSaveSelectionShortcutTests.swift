...
struct ChatViewSaveSelectionShortcutTests {
    ...
    @Test func hstack_invariant_preserved() throws {
        ...
        // layer; = the user-interactive controls). The hidden Button("Save selection") Button
        ...
        // input row). The invariant (= Button("Save selection") Button is OUTSIDE the
        // input row HStack) is now distributed across two files; = this
        // test verifies both halves exist (= .frame(minHeight: 44) is
        // only in ChatInputBarView; = Button("Save selection") is only in ChatView).
        ...
        // The Button("Save selection") keyboard shortcut Button lives ONLY in
        // ChatView (= the keyboard shortcut block is a sibling of
        // ScrollViewReader; = not inside the input row).
        #expect(chatViewSrc.contains("Button(\"Save selection\")"))
        #expect(!inputBarSrc.contains("Button(\"Save selection\")"))
    }
}