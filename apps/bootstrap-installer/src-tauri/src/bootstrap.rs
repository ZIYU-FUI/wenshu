...
//! WENSHU_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for WENSHU_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub wenshu_home: Option<String>,
}
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_wenshu_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
...
pub(crate) fn resolve_wenshu_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_wenshu_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
...
pub(crate) fn wenshu_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".wenshu-bootstrap-complete").exists()
        && resolve_wenshu_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_wenshu_desktop_exe(install_root).ok_or_else(|| {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_wenshu_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::wenshu_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $WenshuHome which Stage-Repository clones
    // the repo INTO at $WenshuHome\wenshu-agent. Mirrors wenshu_constants.
    let wenshu_home = args
        .wenshu_home
        .clone()
        .unwrap_or_else(|| crate::paths::wenshu_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&wenshu_home).join("wenshu-agent");

    // Copy ourselves to WENSHU_HOME/wenshu-setup.exe so the desktop app can
...
async fn run_install_script(
    ...
    powershell::run_script(script_path, args, sink, wenshu_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    #[test]
    fn resolve_wenshu_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_wenshu_desktop_app(&root)
    ...
    #[test]
    fn resolve_wenshu_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_wenshu_desktop_app(&root).is_none(),
