...
export const $wenshuHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.wenshu-hermes/logs/bootstrap-installer.log')
    $wenshuHome.set('~/.wenshu-hermes')
  ...
  try {
    const [logPath, wenshuHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_wenshu_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $wenshuHome.set(wenshuHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Wenshu" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Wenshu-Setup.exe --update);
...
export async function launchWenshuDesktop(): Promise<void> {
  ...
  await invoke('launch_wenshu_desktop', { installRoot })
}
