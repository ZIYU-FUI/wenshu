 ...
 *     `X-Wenshu-Session-Token` header; WS uses `?token=`.
...
// wenshu_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - wenshu_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - wenshu_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / wenshu #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-wenshu_session_at', '__Secure-wenshu_session_at', 'wenshu_session_at']
const RT_COOKIE_VARIANTS = ['__Host-wenshu_session_rt', '__Secure-wenshu_session_rt', 'wenshu_session_rt']

// The Nous portal (NAS) does NOT use 文枢 gateway session cookies — it is a
...
// 文枢 Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://wenshu.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a wenshu session ACCESS-token cookie
 ...
 * the 文枢 gateway session cookies, so cloud sign-in / discovery liveness
