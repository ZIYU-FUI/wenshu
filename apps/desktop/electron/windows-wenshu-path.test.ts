// Unit tests for the pure Windows `wenshu` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvWenshuCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `wenshu` shim shadows
//      the real wenshu.cmd/wenshu.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the wenshu.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvWenshuCommand() — must probe the venv python via
//      canImportWenshuCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvWenshuCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportWenshuCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    wenshuHome: '/fake/wenshu-home',
...
test('resolveVenvWenshuCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvWenshuCommand('/root/venv/Scripts/wenshu.exe', [], deps), null)
})

test('resolveVenvWenshuCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvWenshuCommand('/root/venv/Scripts/wenshu.cmd', [], deps), null)
})

test('resolveVenvWenshuCommand: returns null when the basename is not wenshu/wenshu.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvWenshuCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvWenshuCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvWenshuCommand('/root/venv/bin/wenshu.exe', [], deps), null)
})

test('resolveVenvWenshuCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvWenshuCommand('/root/venv/Scripts/wenshu.exe', [], deps), null)
})

test('resolveVenvWenshuCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportWenshuCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvWenshuCommand('/root/venv/Scripts/wenshu.exe', ['serve'], deps)
...
test('resolveVenvWenshuCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvWenshuCommand('/root/venv/Scripts/wenshu.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'wenshu_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvWenshuCommand: is case-insensitive on wenshu.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvWenshuCommand('/root/venv/Scripts/WENSHU.EXE', [], deps))
  assert.ok(resolveVenvWenshuCommand('/root/venv/SCRIPTS/wenshu.exe', [], deps))
})
