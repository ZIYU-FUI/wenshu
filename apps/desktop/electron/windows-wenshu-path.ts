/**
 * windows-wenshu-path.ts
 *
 * Pure, dependency-injected pieces of Windows `wenshu` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvWenshuCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `wenshu` shim shadowed the real
 *      wenshu.cmd/wenshu.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\wenshu.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvWenshuCommand() — unwrapWindowsVenvWenshuCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `wenshu`) must
 * not shadow `wenshu.cmd`/`wenshu.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv wenshu shim, bootstrap-complete
 * marker) — gating on just the wenshu.exe console-script shim alone is the
...
export interface ResolveVenvWenshuCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportWenshuCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `wenshu`/`wenshu.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/wenshu(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m wenshu_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportWenshuCli(). A
 * venv whose update died mid-`pip install` still has python.exe + wenshu.exe
 ...
 *
 * Returns null when `command` is not a venv wenshu shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvWenshuCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvWenshuCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^wenshu(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv 文枢 at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing 文枢 Python at ${python}`,
    command: python,
