...
export function markerPath(wenshuHome) {
  return path.join(wenshuHome, '.wenshu-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(wenshuHome)
  let raw
 ...
 *
 * The Tauri-based wenshu-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(wenshuHome, pid, { now = Date.now } = {}) {
  const file = markerPath(wenshuHome)
