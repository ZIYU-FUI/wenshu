 ...
 * the three user-facing uninstall modes to the `wenshu uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m wenshu_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `wenshu_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'wenshu_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/文枢.app/Contents/MacOS/文枢  → …/文枢.app
 *   Windows: …\文枢\文枢.exe                 → …\文枢  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/文枢.app/Contents/MacOS/文枢 → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/文枢.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs 文枢.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]文枢$/i.test(dir) || /[\\/]wenshu-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/wenshu
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import wenshu_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, wenshuHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export WENSHU_HOME=${q(wenshuHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import wenshu_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but 文枢 install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "WENSHU_HOME=${String(wenshuHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
