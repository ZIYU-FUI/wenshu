 ...
 *      internals; WENSHU_HOME / WENSHU_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.wenshu-hermes/wenshu-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'wenshu')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Wenshu12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/wenshu/wenshu', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/文枢/wenshu', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.wenshu-hermes/wenshu-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'wenshu'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'wenshu')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'wenshu://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'wenshu://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves WENSHU_HOME + WENSHU_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    WENSHU_HOME: '/home/u/.wenshu-hermes',
    WENSHU_DESKTOP_REMOTE_URL: 'http://box:9119',
    WENSHU_DESKTOP_REMOTE_TOKEN: 'secret',
    WENSHU_DESKTOP_WENSHU_ROOT: '/home/u/dev/wenshu',
    WENSHU_DESKTOP_APP_NAME: 'WenshuSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.wenshu-hermes/wenshu-agent/apps/desktop/release/linux-unpacked/文枢',
    args: ['wenshu://open/agent/42', "--note=it's fine"],
    env: { WENSHU_HOME: '/home/u/.wenshu-hermes', WENSHU_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export WENSHU_HOME='\/home\/u\/\.wenshu-hermes'/)
  assert.match(script, /export WENSHU_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/文枢' 'wenshu:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `wenshu-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  const tmp = path.join(os.tmpdir(), `wenshu-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/文枢\/文枢'\n/)
})
