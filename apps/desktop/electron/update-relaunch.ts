 ...
 * Background
 * ----------
 * After `wenshu update` + `wenshu desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything WENSHU_DESKTOP_* is preserved
// plus WENSHU_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['WENSHU_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['WENSHU_DESKTOP_']
