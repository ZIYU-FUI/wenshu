...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'wenshu-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/wenshu-runner-test',
    sourceRepoRoot: null,
    wenshuHome: '/tmp/wenshu-runner-test',
    logRoot: '/tmp/wenshu-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'wenshu-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'wenshu-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/wenshu-agent',
      wenshuHome: '/tmp/wenshu'
    }),
    ['--dir', '/tmp/wenshu-agent', '--wenshu-home', '/tmp/wenshu', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/wenshu-agent', '--wenshu-home', '/tmp/wenshu', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/wenshu',
      wenshuHome: '/tmp/home'
    }),
    ['--dir', '/tmp/wenshu', '--wenshu-home', '/tmp/home', '--branch', 'main']
  )
})
