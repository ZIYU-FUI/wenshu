// Profile-delete routing logic for the `wenshu:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_wenshu_home() and
...
/**
 * Parse a `wenshu:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `wenshu:api` request away from the primary/window backend
 ...
 * ensure_wenshu_home() recreates the directory the delete just removed.
 */
