 ...
 * product name (`Wenshu`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Wenshu'
 *
 * This is a hard failure with no obvious cause for the user — `wenshu desktop`
 ...
 * (the mac-unpacked Wenshu.app bundle) and Windows (win-unpacked), so we
