#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Wenshu icon + version metadata onto the
// built Wenshu.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Wenshu.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `wenshu desktop`, the installer's
...
// path (which rebuilds via `wenshu desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Wenshu.exe>
//
...
// Stamp the Wenshu icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: '文枢',
      FileDescription: '文枢',
      CompanyName: '文枢 (Wenshu)',
      LegalCopyright: 'Copyright (c) 2026 安百强'
    }
  })

  console.log('[set-exe-identity] done — Wenshu icon + identity stamped')
}
