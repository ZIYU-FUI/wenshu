...
<p align="center">
  ...
  <a href="https://hermes-agent.nousresearch.com/docs/"><img src="https://img.shields.io/badge/Docs-wenshu--agent.nousresearch.com-FFD700?style=for-the-badge" alt="Documentation"></a>
  <a href="https://discord.gg/NousResearch"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord"></a>
  <a href="https://github.com/NousResearch/hermes-agent/blob/main/LICENSE"><img src="https://img.shields.io/badge/License-MIT-green?style=for-the-badge" alt="License: MIT"></a>
</p>

**The native desktop app for [Wenshu Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Wenshu surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Wenshu and hear it back.</td></tr>
...
### Install with Wenshu (recommended)

Already have the Wenshu CLI? Just run:

```bash
wenshu desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. On first launch Wenshu walks you through picking a provider and model; nothing else to configure.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Wenshu Desktop website.](https://hermes-agent.nousresearch.com/).
...
```bash
# throwaway WENSHU_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Wenshu Agent runtime into `WENSHU_HOME`
(`~/.wenshu`, or `%LOCALAPPDATA%\wenshu` on Windows), using the same layout as a
CLI install.
...
- **Wenshu Agent** runs as a headless `wenshu serve` process and exposes the
...
1. `WENSHU_DESKTOP_WENSHU_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `WENSHU_DESKTOP_WENSHU`, or `wenshu` on `PATH`
5. a system Python that can import the Wenshu runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Wenshu
...
Boot logs land in `WENSHU_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\wenshu\wenshu-agent\.wenshu-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\wenshu\wenshu-agent\venv"
```

> The default Wenshu home on Windows is `%LOCALAPPDATA%\wenshu`. Set the `WENSHU_HOME` env var if you've relocated it.
