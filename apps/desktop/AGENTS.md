...
How to build Wenshu Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Wenshu surface can also
...
- A **runtime home change** (switching the underlying `WENSHU_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Wenshu — do not assume
