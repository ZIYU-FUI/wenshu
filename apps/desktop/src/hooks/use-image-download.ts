...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'wenshu:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.wenshuDesktop?.saveImageFromUrl) {
        if (await window.wenshuDesktop.saveImageFromUrl(src)) {
