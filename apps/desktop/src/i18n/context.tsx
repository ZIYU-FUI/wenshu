...
import { getWenshuConfigRecord, type WenshuConfigRecord, saveWenshuConfig } from '@/wenshu'
...
export interface I18nConfigClient {
  getConfig: () => Promise<WenshuConfigRecord>
  saveConfig: (config: WenshuConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.wenshuDesktop?.api) {
      return Promise.resolve({})
    }

    return getWenshuConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.wenshuDesktop?.api) {
    ...
    return saveWenshuConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: WenshuConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: WenshuConfigRecord, locale: Locale): WenshuConfigRecord {
