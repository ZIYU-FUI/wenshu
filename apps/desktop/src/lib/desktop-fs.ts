import type {
  WenshuConnection,
  WenshuReadDirResult,
  WenshuReadFileTextResult,
  WenshuSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: WenshuConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<WenshuReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<WenshuReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<WenshuReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<WenshuReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: WenshuSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
