...
export function setZoomPercent(percent: number): void {
  window.wenshuDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.wenshuDesktop?.zoom) {
  void window.wenshuDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.wenshuDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
