...
function stubProbe(impl: (cwd: string) => Promise<WenshuRepoStatus | null>) {
  ;(window as unknown as { wenshuDesktop?: unknown }).wenshuDesktop = { git: { repoStatus: impl } }
}
...
describe('refreshRepoStatus', () => {
  ...
  it('runs one probe at a time and coalesces overlap into one trailing refresh', async () => {
    const resolvers: Array<(status: WenshuRepoStatus | null) => void> = []
    const calls: string[] = []
    let active = 0
    let maxActive = 0
