...
import type { SessionInfo, UsageStats } from '@/types/wenshu'
...
const COMPOSER_MODEL_KEY = 'wenshu.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'wenshu.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'wenshu.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'wenshu.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'wenshu.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'wenshu.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'wenshu.desktop.lastRoute'
...
function workspaceCwdKey(connection: WenshuConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.wenshuDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.wenshuDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<WenshuConnection | null>(null)
...
export const setConnection = (next: Updater<WenshuConnection | null>) => updateAtom($connection, next)
