...
import { getWenshuConfigRecord, saveWenshuConfig } from '@/wenshu'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getWenshuConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveWenshuConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
