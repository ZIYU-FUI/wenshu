...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/wenshu'
...
import type { ProfileInfo } from '@/types/wenshu'
...
// /api/profiles/active `current`). "default" is the root ~/.wenshu. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'wenshu.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'wenshu.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.wenshuDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new WENSHU_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.wenshuDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.wenshuDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'wenshu.desktop.showAllProfiles'
...
// Switch to the default (root ~/.wenshu) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.wenshuDesktop?.touchBackend?.(target).catch(() => undefined)
}
