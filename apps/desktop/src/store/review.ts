...
//
// Scope is always "uncommitted" — 文枢' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'wenshu.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'wenshu.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'wenshu.desktop.reviewTreeMode'
const SELECTED_KEY = 'wenshu.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<WenshuReviewFile[]>([])
...
export const $reviewShipInfo = atom<WenshuReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['wenshuDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: WenshuReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.wenshuDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.wenshuDesktop?.openExternal?.(url)
    }
