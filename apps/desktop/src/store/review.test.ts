...
import type { WenshuReviewFile, WenshuReviewShipInfo } from '@/global'
...
// reaches the gateway); everything else routes through window.wenshuDesktop.git,
...
function file(path: string, over: Partial<WenshuReviewFile> = {}): WenshuReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as WenshuReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.wenshuDesktop. Any op not supplied defaults
...
function stubReview(over: ReviewStub = {}) {
  ...
  ;(window as unknown as { wenshuDesktop?: unknown }).wenshuDesktop = {
    git: { review },
    openExternal: vi.fn()
  }
...
afterEach(() => {
  delete (window as unknown as { wenshuDesktop?: unknown }).wenshuDesktop
})
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as WenshuReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.wenshuDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: WenshuReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as WenshuReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { wenshuDesktop?: unknown }).wenshuDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as WenshuReviewShipInfo)
