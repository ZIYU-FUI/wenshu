...
const { getProfiles } = await import('@/wenshu')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/wenshu/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<WenshuConnection> = {}): WenshuConnection =>
  ({ baseUrl: 'https://wenshu-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as WenshuConnection

const localConn = (over: Partial<WenshuConnection> = {}): WenshuConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as WenshuConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<WenshuConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { wenshuDesktop: { getConnection } })
