 ...
 * app's bounds and stays visible while 文枢 is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'wenshu.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'wenshu.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.wenshuDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.wenshuDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.wenshuDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.wenshuDesktop?.petOverlay?.close()
}
