...
const isOpen = (gateway: WenshuGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<WenshuGateway | null>(null)
...
let primaryGateway: WenshuGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: WenshuGateway | null, profile = 'default'): void {
...
export function activeGateway(): WenshuGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<WenshuGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
