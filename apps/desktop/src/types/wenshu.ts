...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when 文枢 doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in wenshu_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `wenshu model` uses — instead of
...
export type WenshuConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `wenshu model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors wenshu_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with 文枢, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `wenshu tools` active-provider detection). */
  is_active: boolean
 ...
 *  not 文枢); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/wenshu/update/check` — the backend's own update state.
