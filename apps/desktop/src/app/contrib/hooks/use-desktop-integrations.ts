...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.wenshuDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.wenshuDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.wenshuDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.wenshuDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // wenshu:// deep links -> a reviewable /blueprint command in the composer.
  useEffect(() => {
    const unsubscribe = window.wenshuDesktop?.onDeepLink?.(payload => {
    ...
    void window.wenshuDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.wenshuDesktop?.onClosePreviewRequested?.(() => void closeActiveTab())
