...
import { getSessionMessages, triggerCronJob } from '@/wenshu'
...
import { useWenshuConfig } from '../session/hooks/use-wenshu-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshWenshuConfig, sttEnabled, voiceMaxRecordingSeconds } = useWenshuConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    ...
    window.addEventListener('wenshu:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('wenshu:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshWenshuConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshWenshuConfig])
