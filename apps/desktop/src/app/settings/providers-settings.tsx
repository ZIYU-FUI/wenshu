...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.wenshuDesktop?.terminal)
...
//      in wenshu_cli/provider_catalog.py) — the SAME provider identity
//      `wenshu model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // 文枢 can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider 文枢 can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // 文枢 never deletes creds another tool owns behind a silent API call.
