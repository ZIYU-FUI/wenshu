...
// 文枢' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = 文枢 default (medium), shown as Medium.
...
// backend now surfaces the full `wenshu model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in wenshu_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useWenshuConfigRecord()
  const setConfig = setWenshuConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `wenshu model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — 文枢 runs the flow for you.`}
          </p>
        )}
