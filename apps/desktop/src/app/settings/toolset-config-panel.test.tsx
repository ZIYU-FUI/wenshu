...
import type { ToolsetConfig } from '@/types/wenshu'
...
const getWenshuConfigSchema = vi.fn()
...
vi.mock('@/wenshu', () => ({
  ...
  getWenshuConfigSchema: () => getWenshuConfigSchema(),
...
beforeEach(() => {
  ...
  getWenshuConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getWenshuConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveWenshuConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveWenshuConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
