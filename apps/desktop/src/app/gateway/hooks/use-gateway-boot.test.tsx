...
// bug that drives the REAL useGatewayBoot hook + REAL WenshuGateway through a
...
function Harness({ refreshSessions }: { refreshSessions?: () => Promise<void> } = {}) {
  useGatewayBoot({
    ...
    refreshWenshuConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { wenshuDesktop?: unknown }).wenshuDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { wenshuDesktop?: unknown }).wenshuDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForWenshu) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startWenshu()'s remote branch awaits waitForWenshu() for 45s before it
    ...
    ;(window as { wenshuDesktop?: unknown }).wenshuDesktop = desktop
    ...
    // After ~45s waitForWenshu gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('文枢 backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "文枢 couldn't start" overlay — the socket is open
