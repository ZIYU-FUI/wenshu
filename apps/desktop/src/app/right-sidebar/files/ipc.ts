...
import type { WenshuReadDirEntry, WenshuReadDirResult } from '@/global'
...
export type ProjectTreeEntry = WenshuReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: WenshuReadDirEntry) {
...
async function filterIgnored(entries: WenshuReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<WenshuReadDirResult> {
  if (!window.wenshuDesktop) {
