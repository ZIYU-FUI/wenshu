        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.wenshuDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask 文枢 to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.wenshuDesktop?.watchPreviewFile ||
      !window.wenshuDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.wenshuDesktop.onPreviewFileChanged(payload => {
    ...
    void window.wenshuDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.wenshuDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.wenshuDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:wenshu-preview')
