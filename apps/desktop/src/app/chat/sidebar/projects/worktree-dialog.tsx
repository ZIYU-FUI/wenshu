...
import type { WenshuGitBranch } from '@/global'
...
const branchActionLabel = (branch: WenshuGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<WenshuGitBranch[]>([])
  ...
  const convert = async (branch: WenshuGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
