...
interface WenshuConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useWenshuConfig({ activeSessionIdRef, refreshProjectBranch }: WenshuConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshWenshuConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getWenshuConfig(), getWenshuConfigDefaults().catch(() => ({}))])
  ...
  return { refreshWenshuConfig, sttEnabled, voiceMaxRecordingSeconds }
}
