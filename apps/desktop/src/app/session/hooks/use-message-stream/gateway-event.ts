...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshWenshuConfig — two REST calls (config + defaults) per event, per
    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (apply) {
          ...
          // everything refreshWenshuConfig applies is either active-session
        ...
        const errorMessage = payload?.message || '文枢 reported an error'
