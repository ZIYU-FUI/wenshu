...
vi.mock('@/wenshu', () => ({
  getWenshuConfig: vi.fn(),
  getWenshuConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'wenshu.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getWenshuConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getWenshuConfig>>)

describe('useWenshuConfig refreshWenshuConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshWenshuConfig()
    })
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getWenshuConfig>>>()
    vi.mocked(getWenshuConfig).mockReturnValueOnce(profileConfig.promise)
    ...
    act(() => {
      pendingRefresh = result.current.refreshWenshuConfig(true)
    })
    expect(getWenshuConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getWenshuConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getWenshuConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getWenshuConfig>>>()
    vi.mocked(getWenshuConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshWenshuConfig(true)
      refreshC = result.current.refreshWenshuConfig(true)
    })
