...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.wenshuDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.wenshuDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.wenshuDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.wenshuDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.wenshuDesktop && text && normalizeExternalUrl(text) === target) {
