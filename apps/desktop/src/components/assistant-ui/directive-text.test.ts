...
import { formatRefValue, wenshuDirectiveFormatter } from './directive-text'
...
describe('wenshuDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = wenshuDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = wenshuDirectiveFormatter.parse('@file:src/main.tsx the entry point')
