...
const WENSHU_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type WenshuRefType = (typeof WENSHU_REF_TYPES)[number]
...
const ICON_PATHS: Record<WenshuRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as WenshuRefType] ?? ICON_FALLBACK
...
const WENSHU_DIRECTIVE_RE = new RegExp(
...
export const wenshuDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(WENSHU_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as WenshuRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: WenshuRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...wenshuDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing 文枢 directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.wenshuDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.wenshuDesktop?.readFileDataUrl(id)
