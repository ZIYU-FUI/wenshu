...
const SKIN_KEY = 'wenshu-desktop-theme-v2'
const MODE_KEY = 'wenshu-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'wenshu-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'wenshu-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'wenshu-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.wenshuTheme = skinName
  root.dataset.wenshuMode = rendered
  ...
  window.wenshuDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('wenshu-boot-background', chromeBg)
    window.localStorage.setItem('wenshu-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.wenshuThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.wenshuDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active 文枢 backend theme on connect. */
