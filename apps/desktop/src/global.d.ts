...
declare global {
  interface Window {
    wenshuDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<WenshuConnection>
      ...
      // 文枢 Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new WENSHU_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: WenshuApiRequest) => Promise<T>
      notify: (payload: WenshuNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<WenshuReadFileTextResult>
      selectPaths: (options?: WenshuSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<WenshuPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<WenshuPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: WenshuTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<WenshuReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<WenshuGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<WenshuGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<WenshuGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<WenshuRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: WenshuReviewScope, baseRef?: null | string) => Promise<WenshuReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: WenshuReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<WenshuReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: WenshuTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<WenshuTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: WenshuWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: WenshuPreviewFileChanged) => void) => () => void
...
export interface WenshuTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface WenshuTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `wenshu update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  wenshuRoot?: string
...
export interface WenshuConnection {
  baseUrl: string
...
export interface WenshuTitleBarTheme {
  background: string
  foreground: string
}

export interface WenshuWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a 文枢 Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted 文枢 Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected 文枢 Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the wenshu gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered 文枢 Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface WenshuApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface WenshuNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface WenshuPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface WenshuReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface WenshuPreviewWatch {
  id: string
  path: string
}
...
export interface WenshuGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface WenshuGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface WenshuGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface WenshuRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface WenshuRepoStatus {
  branch: null | string
...
// turn began.
export type WenshuReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface WenshuReviewFile {
  path: string
  added: number
  removed: number
...
export interface WenshuReviewList {
  files: WenshuReviewFile[]
...
export interface WenshuReviewPr {
  url: string
  state: string
  number: number
}
...
export interface WenshuReviewShipInfo {
  ghReady: boolean
  pr: WenshuReviewPr | null
}

export interface WenshuReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface WenshuReadDirResult {
  entries: WenshuReadDirEntry[]
  error?: string
}

export interface WenshuPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface WenshuSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
