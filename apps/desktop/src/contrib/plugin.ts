 ...
 * `WenshuPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/wenshu'
...
export type { PluginRestOptions } from '@/wenshu'
 ...
 *  under `wenshu.plugin.<id>.` — plugins can't read or clobber each other. */
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `wenshu.plugin.${pluginId}.${key}`
