 ...
 *   (`@wenshu/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default WenshuPlugin -> register(ctx)
 *
 ...
 * and `<wenshu home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
/** Rewrite ONLY mapped import specifiers (@wenshu/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @wenshu/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: WenshuPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default WenshuPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<wenshu home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    const { wenshu_home } = await getStatus()
    const { entries } = await desktop.readDir(`${wenshu_home}/desktop-plugins`)
