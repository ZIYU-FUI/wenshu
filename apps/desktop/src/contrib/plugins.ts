 ...
 *    `WenshuPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `wenshu-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<wenshu home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: WenshuPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default WenshuPlugin export — skipped`)
