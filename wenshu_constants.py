"""Shared constants for Wenshu Agent.
...
_UNSET = object()
_WENSHU_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_WENSHU_HOME_OVERRIDE", default=_UNSET
)


def set_wenshu_home_override(path: str | Path | None) -> Token:
    """Set a context-local Wenshu home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _WENSHU_HOME_OVERRIDE.set(value)


def reset_wenshu_home_override(token: Token) -> None:
    """Restore the previous context-local Wenshu home override."""
    _WENSHU_HOME_OVERRIDE.reset(token)


def get_wenshu_home_override() -> str | None:
    """Return the active context-local Wenshu home override, if any."""
    override = _WENSHU_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_wenshu_home() -> Path:
    """Return the platform-native default Wenshu home path."""
...
def _wenshu_home_from_env() -> Path:
    """Resolve WENSHU_HOME from the process environment only.

    Reads the ``WENSHU_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_wenshu_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_wenshu_home`
    and :func:`get_process_wenshu_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``WENSHU_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_wenshu_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[WENSHU_HOME fallback] WENSHU_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass WENSHU_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_wenshu_home() -> Path:
    """Return the Wenshu home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_wenshu_home_override`) → ``WENSHU_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``WENSHU_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``WENSHU_HOME`` explicitly (see the systemd
    template in ``wenshu_cli/gateway.py`` and the kanban dispatcher in
    ``wenshu_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    override = get_wenshu_home_override()
    if override:
        return Path(override)

    if not os.environ.get("WENSHU_HOME", "").strip():
        _warn_profile_fallback_once()

    return _wenshu_home_from_env()


def get_process_wenshu_home() -> Path:
    """Return the Wenshu home for the running process, ignoring task overrides.

    Unlike :func:`get_wenshu_home`, this never follows the context-local
    override set by :func:`set_wenshu_home_override`.  It resolves only the
    process ``WENSHU_HOME`` env var (falling back to the platform default),
...
def get_default_wenshu_root() -> Path:
    """Return the root Wenshu directory for profile-level operations.

    In standard deployments this is the platform-native Wenshu home
    (``~/.wenshu`` on POSIX, ``%LOCALAPPDATA%\\wenshu`` on native Windows).

    In Docker or custom deployments where ``WENSHU_HOME`` points outside
    ``~/.wenshu`` (e.g. ``/opt/data``), returns ``WENSHU_HOME`` directly
    — that IS the root.

    In profile mode where ``WENSHU_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.wenshu-hermes/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_wenshu_home()
    env_home = os.environ.get("WENSHU_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # WENSHU_HOME is under ~/.wenshu (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — WENSHU_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Wenshu is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``WENSHU_OPTIONAL_SKILLS``.
    """
    override = os.getenv("WENSHU_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_wenshu_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``WENSHU_OPTIONAL_MCPS``.
    """
    override = os.getenv("WENSHU_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_wenshu_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``WENSHU_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<WENSHU_HOME>/skills`` last-resort
    """
    override = os.getenv("WENSHU_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_wenshu_home() / "skills"


def get_wenshu_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Wenshu subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to WENSHU_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to WENSHU_HOME (e.g. ``"image_cache"``).
...
def iter_wenshu_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Wenshu-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\wenshu\\node``. POSIX installs use
    ``$WENSHU_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_wenshu_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with wenshuManagedNodePathEntries() in
...
_WENSHU_NODE_TARGET_MAJOR = int(os.environ.get("WENSHU_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Wenshu-managed Node trees live under ``$WENSHU_HOME/node`` (or a profile's
    ``WENSHU_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_wenshu_node_executable``
    used to trust file presence alone, so ``wenshu update`` would pick that
...
def wenshu_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Wenshu-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_wenshu_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%WENSHU_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_wenshu_home()
    index_url = f"https://nodejs.org/dist/latest-v{_WENSHU_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_WENSHU_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_wenshu_managed_node() -> bool:
    """Redownload Wenshu-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not wenshu_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "WENSHU_HOME": str(get_wenshu_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_wenshu_node_executable(command: str) -> str | None:
    """Return a Wenshu-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_wenshu_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_wenshu_managed_node():
        for directory in iter_wenshu_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Wenshu-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Wenshu-managed installs.

    This is for Wenshu-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_wenshu_node_executable(command)
    if managed:
        return managed
    if wenshu_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_wenshu_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Wenshu-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_wenshu_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``wenshu update`` — leaving a **dangling symlink** that ``which`` still
...
def display_wenshu_home() -> str:
    """Return a user-friendly display string for the current WENSHU_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.wenshu``
        profile:  ``~/.wenshu-hermes/profiles/coder``
        custom:   ``/opt/wenshu-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.wenshu``.  For code that needs a real ``Path``, use
    :func:`get_wenshu_home` instead.
    """
    home = get_wenshu_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``WENSHU_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{WENSHU_HOME}/home`` when the profile-home directory exists."""
    wenshu_home = get_wenshu_home_override() or (env or {}).get("WENSHU_HOME") or os.getenv("WENSHU_HOME")
    if not wenshu_home:
        return None
    profile_home = os.path.join(wenshu_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("WENSHU_REAL_HOME") or os.getenv("WENSHU_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Wenshu profile HOME.

    ``WENSHU_HOME`` scopes Wenshu state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={WENSHU_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{WENSHU_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{WENSHU_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Wenshu' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["WENSHU_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Wenshu itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/wenshu-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under WENSHU_HOME.

    Replaces the ``get_wenshu_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, wenshu_logging.py, wenshu_time.py, etc.).
    """
    return get_wenshu_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under WENSHU_HOME."""
    return get_wenshu_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under WENSHU_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_wenshu_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._wenshu_ipv4_patched = True  # type: ignore[attr-defined]
