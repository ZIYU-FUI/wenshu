"""disk_cleanup — ephemeral file cleanup for Wenshu Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under WENSHU_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly WENSHU_HOME and /tmp/wenshu-*
Never touches: ~/.wenshu-hermes/logs/ or any system directory.
"""
...
try:
    ...
    def get_wenshu_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("WENSHU_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".wenshu-hermes").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$WENSHU_HOME/logs/``."""
    return get_wenshu_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$WENSHU_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under WENSHU_HOME or ``/tmp/wenshu-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    wenshu_home = get_wenshu_home()
    try:
        path.resolve().relative_to(wenshu_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/wenshu-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("wenshu-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "wenshu-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $WENSHU_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so WENSHU_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        wenshu_home = get_wenshu_home()
        for parent in ("cron", "cronjobs"):
            base = wenshu_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside WENSHU_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under WENSHU_HOME, but never recurse into known
    # durable state trees.  Some installs place the Wenshu checkout, venv,
    # and desktop build under WENSHU_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    wenshu_home = get_wenshu_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in wenshu_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    wenshu_home = get_wenshu_home()
    try:
        rel = path.resolve().relative_to(wenshu_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "wenshu-agent",
        }:
            return None
        ...
        # Path isn't under WENSHU_HOME (e.g. /tmp/wenshu-*) — fall through.
        pass
