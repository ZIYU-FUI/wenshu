...
Auto-tracks and cleans up ephemeral files created during Wenshu Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$WENSHU_HOME` and `/tmp/wenshu-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `WENSHU_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `WENSHU_HOME` or `/tmp/wenshu-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$WENSHU_HOME/disk-cleanup/` is itself excluded
- `$WENSHU_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
