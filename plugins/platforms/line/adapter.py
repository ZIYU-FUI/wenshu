"""
LINE Messaging API platform adapter for Wenshu Agent.
...
to Wenshu Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``WENSHU_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            wenshu_home = Path(get_wenshu_home()).resolve()
        except Exception:
            wenshu_home = Path.home().joinpath(".wenshu-hermes").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``wenshu status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``wenshu status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``wenshu setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.wenshu-hermes/.env`` via ``wenshu_cli.config``.
    """
    print()
    ...
    try:
        from wenshu_cli.config import get_env_var, set_env_var
    except ImportError:
        print("wenshu_cli.config not available; set LINE_* vars manually in ~/.wenshu-hermes/.env")
        return
