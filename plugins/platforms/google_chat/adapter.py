...
# ``model_tools`` import time, so ``wenshu status``, ``wenshu chat``, etc.
...
# ``wenshu_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Wenshu is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a wenshu MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "wenshu-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${WENSHU_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _wenshu_home = _get_wenshu_home()
        except (ModuleNotFoundError, ImportError):
            _wenshu_home = _Path.home() / ".wenshu-hermes"
        self._thread_count_store = _ThreadCountStore(
            _wenshu_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Wenshu is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("WENSHU_HOME", str(_Path.home() / ".wenshu-hermes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${WENSHU_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a wenshu MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Wenshu is thinking…' marker message.
        ...
        "Wenshu is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Wenshu is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Wenshu is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Wenshu is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``wenshu setup``.

    The setup wizard at ``wenshu_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.wenshu-hermes/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. wenshu-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.wenshu-hermes/.env")
    print_info("Restart the gateway: wenshu gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``wenshu cron`` running as a
    separate process from ``wenshu gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Wenshu plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'wenshu-agent[google_chat]'",
        ...
        # the "Wenshu is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Wenshu is thinking…' marker message "
