"""SimpleX Chat platform adapter (Wenshu plugin).
...
This adapter ships as a Wenshu platform plugin under
``plugins/platforms/simplex/``. The Wenshu plugin loader scans the
...
Optional environment variables:
    ...
    WENSHU_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``wenshu setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Wenshu — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("WENSHU_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``wenshu setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.wenshu-hermes/.env`` via
    ``wenshu_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "wenshu_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.wenshu-hermes/.env"
        )
        return
