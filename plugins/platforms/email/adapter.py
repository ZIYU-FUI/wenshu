"""
Email platform adapter for the Wenshu gateway.

Allows users to interact with Wenshu by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "wenshu-agent" "version" "{_wenshu_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `wenshu config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `wenshu gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Wenshu Agent")
        ...
        msg_id = f"<wenshu-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# wenshu_cli/gateway.py, and the _send_email dispatch in
