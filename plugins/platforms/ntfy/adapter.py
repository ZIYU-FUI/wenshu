"""ntfy platform adapter (Wenshu plugin).
...
``plugins/platforms/ntfy/``. The Wenshu plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "wenshu-in"              # subscribe topic (incoming)
          publish_topic: "wenshu-out"     # optional — defaults to topic
...
_ECHO_TAG = "wenshu-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``wenshu cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Wenshu dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `wenshu gateway status` without
