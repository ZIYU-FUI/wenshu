...
This plugin connects Wenshu Agent to iMessage (and other Spectrum
...
TypeScript-only, Wenshu runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Wenshu-Sidecar-Token`.
...
There is no separate `login` command; like every other Wenshu channel,
...
Run `wenshu photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.wenshu-hermes/.env` (the same place every other
...
Management metadata lives in `~/.wenshu-hermes/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `wenshu photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `wenshu photon status`, a DM and a group roundtrip,
