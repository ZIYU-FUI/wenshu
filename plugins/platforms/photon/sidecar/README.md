...
Small Node helper that bridges Wenshu Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Wenshu is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Wenshu-Sidecar-Token`)
...
The Wenshu plugin's `wenshu photon setup` command runs `npm install`
here automatically.
