// Wenshu Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Wenshu): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Wenshu -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Wenshu-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `wenshu photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Wenshu can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Wenshu'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Wenshu allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Wenshu, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-wenshu-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Wenshu supplied them so a known-good
