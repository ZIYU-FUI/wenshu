...
Credential storage mirrors every other Wenshu channel:

    * runtime SDK creds  -> ``~/.wenshu-hermes/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.wenshu-hermes/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a wenshu dependency
...
# registers Wenshu as its own client_id.
...
# Default name of the project Wenshu provisions for the operator.
DEFAULT_PROJECT_NAME = "Wenshu Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of wenshu-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.wenshu-hermes/auth.json`` honouring the active Wenshu profile."""
    try:
        from wenshu_constants import get_wenshu_home
        return Path(get_wenshu_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.wenshu-hermes")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.wenshu-hermes/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.wenshu-hermes/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.wenshu-hermes/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: wenshu_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `wenshu photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `wenshu photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `wenshu photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `wenshu photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `wenshu photon setup`)"
