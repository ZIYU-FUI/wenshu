...
- Hosted: API key from [app.supermemory.ai/integrations?connect=wenshu](http://app.supermemory.ai/integrations?connect=wenshu)
...
Before running `wenshu memory setup`, add the local endpoint to
...
Then run `wenshu memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$WENSHU_HOME/supermemory.json`
...
| `container_tag` | `wenshu` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `wenshu-{identity}` → `wenshu-coder`). |
...
`https://api.supermemory.ai`. Wenshu resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: wenshu`, and document writes stamp
`metadata.sm_source: wenshu`. This is a **functional routing key, not telemetry**:
it groups Wenshu-written memories into a dedicated "Wenshu" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Wenshu profile:

```json
{
  "container_tag": "wenshu-{identity}"
}
```

For a profile named `coder`, this resolves to `wenshu-coder`. The default profile resolves to `wenshu-default`. Without `{identity}`, all profiles share the same container.
