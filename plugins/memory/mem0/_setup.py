...
def _save_mem0_json(wenshu_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(wenshu_home) / "mem0.json"
    existing = {}
...
def _setup_platform(wenshu_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "wenshu-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "wenshu"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.wenshu-hermes/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.wenshu-hermes/.env (or unset it) or Wenshu will keep "
    ...
    provider.save_config(provider_config, wenshu_home)

    if env_writes:
        _write_env(Path(wenshu_home) / ".env", env_writes)
...
def _setup_selfhosted(wenshu_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "wenshu-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "wenshu")
...
def _setup_oss(wenshu_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(wenshu_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "wenshu-user")
    ...
    if env_writes:
        _write_env(Path(wenshu_home) / ".env", env_writes)
    _save_mem0_json(wenshu_home, {"mode": "oss", "user_id": user_id, "agent_id": "wenshu", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, wenshu_home: str) -> str:
...
_PGVECTOR_CONTAINER = "wenshu-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "wenshu"
...
def _setup_oss_interactive(wenshu_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], wenshu_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], wenshu_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'wenshu-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "wenshu-user")

    agent_id = input("  Agent ID [wenshu]: ").strip()
    agent_id = agent_id or "wenshu"
    ...
    if env_writes:
        _write_env(Path(wenshu_home) / ".env", env_writes)
    _save_mem0_json(wenshu_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(wenshu_home: str, config: dict) -> None:
    """Entry point called by wenshu memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(wenshu_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(wenshu_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(wenshu_home, config, flags)
        return
