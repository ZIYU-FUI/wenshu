...
```bash
wenshu memory setup    # select "mem0"
```
...
Behavioral settings live in `$WENSHU_HOME/mem0.json` (set them via `wenshu memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.wenshu-hermes/.env`.
...
| `user_id` | `wenshu-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Wenshu session and call `mem0_search` — it connects to your server.
...
```bash
wenshu memory setup mem0 --mode oss \
...
```bash
wenshu memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$WENSHU_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.wenshu-hermes/mem0_qdrant"}}
  }
}
```
...
```bash
wenshu memory setup mem0 --mode platform --api-key sk-...
```
...
ls -la ~/.wenshu-hermes/mem0_qdrant
...
- Check `user_id` matches between sessions (`$WENSHU_HOME/mem0.json`).
