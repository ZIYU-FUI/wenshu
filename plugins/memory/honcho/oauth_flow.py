...
``wenshu://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Wenshu OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "wenshu-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``wenshu-desktop`` / ``wenshu-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Wenshu.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local WENSHU_HOME override can't reach.
