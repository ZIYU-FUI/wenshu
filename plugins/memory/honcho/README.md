...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/wenshu>
...
```bash
wenshu memory setup honcho   # configure Honcho directly (works on a fresh install)
wenshu memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
wenshu config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.wenshu-hermes/.env
```

> `wenshu honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `wenshu memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `wenshu model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$WENSHU_HOME/honcho.json` | Profile-local (isolated Wenshu instances) |
| 2 | `~/.wenshu-hermes/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Wenshu profile: `wenshu` (default) or `wenshu_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `wenshu honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Wenshu session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`wenshu-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`wenshu`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-wenshu-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening wenshu in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Wenshu session ID (timestamp + hex). Every `wenshu` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Wenshu profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`wenshu`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Wenshu profile: `wenshu` (default) or `wenshu_<profile>` (e.g. `wenshu -p coder` -> host key `wenshu_coder`). Older `wenshu.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `WENSHU_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `wenshu-agent`) |
...
|---------|-------------|
| `wenshu memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `wenshu honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `wenshu memory setup`) |
| `wenshu honcho status` | Show resolved config for active profile |
| `wenshu honcho enable` / `disable` | Toggle Honcho for active profile |
| `wenshu honcho mode <mode>` | Change recall or observation mode |
| `wenshu honcho peer --user <name>` | Update user peer name |
| `wenshu honcho peer --ai <name>` | Update AI peer name |
| `wenshu honcho tokens --context <N>` | Set context token budget |
| `wenshu honcho tokens --dialectic <N>` | Set dialectic max chars |
| `wenshu honcho map <name>` | Map current directory to a session name |
| `wenshu honcho sync` | Create host blocks for all Wenshu profiles |
