...
Optional Wenshu observability plugin that maps Wenshu observer hooks to
...
not replace Wenshu Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Wenshu emit NeMo Relay lifecycle events for
the work Wenshu already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Wenshu Agent can:

- Preserve Wenshu execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
wenshu plugins enable observability/nemo_relay
```

The `WENSHU_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `WENSHU_HOME` that the
agent run will use:

```bash
env WENSHU_HOME=/tmp/wenshu-nemo-relay-test \
  wenshu plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`WENSHU_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`WENSHU_HOME` is the Wenshu profile/config home used by both
`wenshu plugins enable ...` and the later `wenshu chat ...` run. If unset,
Wenshu uses the user's default home, usually `~/.wenshu`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export WENSHU_HOME=/tmp/wenshu-nemo-relay-test
wenshu plugins enable observability/nemo_relay
wenshu chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `wenshu` command you run is built from the
...
uv run wenshu plugins enable observability/nemo_relay
uv run wenshu chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/wenshu_agent-*.whl
...
The plugin can configure exporters directly from `WENSHU_NEMO_RELAY_*`
...
```bash
export WENSHU_NEMO_RELAY_ATOF_ENABLED=1
export WENSHU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export WENSHU_NEMO_RELAY_ATIF_ENABLED=1
export WENSHU_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `WENSHU_NEMO_RELAY_ATOF_FILENAME`
- `WENSHU_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `WENSHU_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `WENSHU_NEMO_RELAY_ATIF_AGENT_NAME`
- `WENSHU_NEMO_RELAY_ATIF_AGENT_VERSION`
- `WENSHU_NEMO_RELAY_ATIF_MODEL_NAME`
- `WENSHU_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Wenshu at it:

```bash
export WENSHU_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Wenshu Agent"
agent_version = "local"
```

When `WENSHU_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`WENSHU_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`WENSHU_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Wenshu does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Wenshu keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Wenshu routes LLM and tool
...
Wenshu feature-detects the dynamic-plugin activation API available in NeMo Relay
0.6 and later. Configure native or worker plugins with Wenshu-owned
...
with this Wenshu-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Wenshu rejects
...
Wenshu activates these plugins before registering its managed LLM and tool
...
activation API, Wenshu logs an actionable warning and continues with the
...
For the full generic Wenshu middleware contract, see
...
export WENSHU_HOME=/tmp/wenshu-nemo-relay-docs/wenshu-home
mkdir -p "$WENSHU_HOME"

cat > "$WENSHU_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Wenshu session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export WENSHU_NEMO_RELAY_ATOF_ENABLED=1
export WENSHU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/wenshu-nemo-relay-docs/subagent/atof
export WENSHU_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export WENSHU_NEMO_RELAY_ATOF_MODE=overwrite
export WENSHU_NEMO_RELAY_ATIF_ENABLED=1
export WENSHU_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/wenshu-nemo-relay-docs/subagent/atif
export WENSHU_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export WENSHU_NEMO_RELAY_ATIF_AGENT_NAME='Wenshu Agent E2E'
export WENSHU_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export WENSHU_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"wenshu.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Wenshu Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Wenshu dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/wenshu-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/wenshu-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/wenshu-nemo-relay-docs/workdir/beta.txt
cd /tmp/wenshu-nemo-relay-docs/workdir

export WENSHU_NEMO_RELAY_ATOF_ENABLED=1
export WENSHU_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/wenshu-nemo-relay-docs/parallel/atof
export WENSHU_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export WENSHU_NEMO_RELAY_ATOF_MODE=overwrite
export WENSHU_NEMO_RELAY_ATIF_ENABLED=1
export WENSHU_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/wenshu-nemo-relay-docs/parallel/atif
export WENSHU_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export WENSHU_NEMO_RELAY_ATIF_AGENT_NAME='Wenshu Agent E2E'
export WENSHU_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Wenshu sessions map to `agent` scopes.
- Wenshu API request hooks map to `llm` scope start/end events.
- Wenshu tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Wenshu execution middleware to hand
...
Enable it for Wenshu:

```bash
export WENSHU_NEMO_RELAY_PLUGINS_TOML=/tmp/wenshu-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Wenshu routes execution
through these boundaries:

```text
Wenshu provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Wenshu provider adapter next_call(...)

Wenshu tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Wenshu tool dispatcher next_call(...)
```
...
middleware for a local Wenshu run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by the supported
0.x release range beginning with 0.5.

```bash
export WENSHU_HOME=/tmp/wenshu-middleware-test/wenshu-home
mkdir -p "$WENSHU_HOME" /tmp/wenshu-middleware-test/nemo-relay
...
cat > /tmp/wenshu-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/wenshu-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/wenshu-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Wenshu Middleware E2E"
agent_version = "local"
...
export WENSHU_NEMO_RELAY_PLUGINS_TOML=/tmp/wenshu-middleware-test/nemo-relay/plugins.toml
