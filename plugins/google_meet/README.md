...
Let the wenshu agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where wenshu runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `wenshu meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `wenshu meet` CLI |
...
| `cli.py` | `wenshu meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$WENSHU_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `wenshu meet node {run,list,approve,remove,status,ping}` |
...
wenshu meet install                                      # pip + Chromium
wenshu meet setup                                        # preflight
wenshu meet auth                                         # optional
wenshu meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
wenshu meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.wenshu-hermes/.env
wenshu meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
wenshu meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
wenshu meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.wenshu-hermes/.env
wenshu meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, wenshu will **not** switch your system audio input automatically — the
...
wenshu plugins enable google_meet
wenshu meet node run --display-name my-mac --host 0.0.0.0 --port 18789
