...
``{title, url, description, position}`` rows every other Wenshu web
provider produces.
...
prefers Wenshu-managed xAI Grok OAuth (via ``wenshu auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.wenshu-hermes/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``wenshu tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `wenshu auth` to sign in with "
