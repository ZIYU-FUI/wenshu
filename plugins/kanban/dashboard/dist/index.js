/**
 * Wenshu Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.wenshu-hermes/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__WENSHU_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__WENSHU_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "wenshu-kanban-dot-triage",
    todo: "wenshu-kanban-dot-todo",
    ready: "wenshu-kanban-dot-ready",
    running: "wenshu-kanban-dot-running",
    blocked: "wenshu-kanban-dot-blocked",
    done: "wenshu-kanban-dot-done",
    archived: "wenshu-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-wenshu-task";
  ...
  const LS_BOARD_KEY = "wenshu.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="wenshu-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])wenshu-kanban-md-code\1/i.test(attrs)) {
      return ' class="wenshu-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "wenshu-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "wenshu-kanban-md",
  ...
  // standard `drop` event and our `wenshu-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("wenshu-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("wenshu-kanban-column--drop");
          if (target) target.classList.add("wenshu-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("wenshu-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("wenshu-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("wenshu-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__WENSHU_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "wenshu-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "wenshu-kanban-attention",
        "wenshu-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "wenshu-kanban-attention-bar" },
        h("span", { className: "wenshu-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "wenshu-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "wenshu-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "wenshu-kanban-attention-row",
                  "wenshu-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "wenshu-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "wenshu-kanban-attention-row-id" }, task.id),
                h("span", { className: "wenshu-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "wenshu-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "wenshu-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "wenshu-kanban-diag-action-btn",
      action.suggested ? "wenshu-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " wenshu-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".wenshu-kanban-drawer-comment-row input, .wenshu-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "wenshu-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("wenshu-kanban-diag", sevClass) },
      h("div", { className: "wenshu-kanban-diag-header" },
        h("span", { className: "wenshu-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "wenshu-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "wenshu-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "wenshu-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "wenshu-kanban-diag-data-row" },
                  h("span", { className: "wenshu-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "wenshu-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "wenshu-kanban-diag-data-row" },
                h("span", { className: "wenshu-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "wenshu-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "wenshu-kanban-diag-reassign-row" },
            h("span", { className: "wenshu-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "wenshu-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "wenshu-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "wenshu-kanban-diag-msg",
              msg.ok ? "wenshu-kanban-diag-msg--ok" : "wenshu-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "wenshu-kanban-section" },
      h("div", { className: "wenshu-kanban-section-head-row" },
        h("span", { className: "wenshu-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "wenshu-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "wenshu-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "wenshu-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "wenshu-kanban-docs-link",
      title: "Open Wenshu Kanban docs in a new tab",
      "aria-label": "Wenshu Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `wenshu kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "wenshu-kanban-msg-ok" : "wenshu-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "wenshu-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "wenshu-kanban-boardswitcher" },
      h("div", { className: "wenshu-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "wenshu-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "wenshu-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "wenshu-kanban-dialog-title" },
        ...
        h("div", { className: "wenshu-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Wenshu profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "wenshu-kanban-bulk" },
      h("span", { className: "wenshu-kanban-bulk-count" },
      ...
      h("div", { className: "wenshu-kanban-bulk-priority",
      ...
      h("div", { className: "wenshu-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Wenshu profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "wenshu-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("wenshu-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("wenshu-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "wenshu-kanban-trash",
        dragOver ? "wenshu-kanban-trash--drop" : "",
        props.draggingTaskId ? "wenshu-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "wenshu-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "wenshu-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".wenshu-kanban-card")) return true;
      if (target.closest && target.closest(".wenshu-kanban-column-add")) return true;
      if (target.closest && target.closest(".wenshu-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("wenshu-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("wenshu-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".wenshu-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "wenshu-kanban-columns",
        isScrollable ? "wenshu-kanban-columns--scrollable" : "",
        isPanning ? "wenshu-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("wenshu-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("wenshu-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "wenshu-kanban-column",
        dragOver ? "wenshu-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "wenshu-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "wenshu-kanban-col-check",
        ...
        h("span", { className: cn("wenshu-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "wenshu-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "wenshu-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "wenshu-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "wenshu-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "wenshu-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "wenshu-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "wenshu-kanban-lane" },
                  h("div", { className: "wenshu-kanban-lane-head" },
                    h("span", { className: "wenshu-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "wenshu-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "wenshu-kanban-card--stale-red";
    if (age >= tier.amber) return "wenshu-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".wenshu-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "wenshu-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "wenshu-kanban-card",
        props.selected ? "wenshu-kanban-card--selected" : "",
        props.failed ? "wenshu-kanban-card--failed" : "",
        props.draggingSource ? "wenshu-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "wenshu-kanban-card-content" },
          h("div", { className: "wenshu-kanban-card-row" },
            h("label", {
              className: "wenshu-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "wenshu-kanban-card-check",
            ...
            h("span", { className: "wenshu-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or wenshu kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "wenshu-kanban-warning-badge",
                    "wenshu-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "wenshu-kanban-needs-assignee",
          ...
          h("div", { className: "wenshu-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "wenshu-kanban-card-row wenshu-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "wenshu-kanban-assignee",
                            title: `Assigned to Wenshu profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "wenshu-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "wenshu-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "wenshu-kanban-ago",
  ...
  // centered modal reusing the wenshu-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "wenshu-kanban-dialog wenshu-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Wenshu profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Wenshu profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "wenshu-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "wenshu-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "wenshu-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "wenshu-kanban-drawer-close",
        ...
        data ? h("div", { className: "wenshu-kanban-drawer-comment-foot" },
          h("div", {
            className: "wenshu-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "wenshu-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "wenshu-kanban-section" },
      h("div", { className: "wenshu-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "wenshu-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "wenshu-kanban-drawer-body" },
      h("div", { className: "wenshu-kanban-drawer-title" },
        h("span", { className: cn("wenshu-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "wenshu-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "wenshu-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "wenshu-kanban-section" },
            h("div", { className: "wenshu-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "wenshu-kanban-section" },
            h("div", { className: "wenshu-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "wenshu-kanban-done-no-result wenshu-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "wenshu-kanban-section" },
            h("div", { className: "wenshu-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "wenshu-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "wenshu-kanban-section" },
        h("div", { className: "wenshu-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "wenshu-kanban-comment" },
            h("div", { className: "wenshu-kanban-comment-head" },
              h("span", { className: "wenshu-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "wenshu-kanban-diag-action-btn",
      ...
      h("div", { className: "wenshu-kanban-section" },
        h("div", { className: "wenshu-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "wenshu-kanban-comment" },
                h("div", { className: "wenshu-kanban-comment-head" },
                  h("span", { className: "wenshu-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "wenshu-kanban-comment-ago" },
      ...
      h("div", { className: "wenshu-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "wenshu-kanban-event",
              isDiag ? "wenshu-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "wenshu-kanban-event-header" },
                  h("span", { className: "wenshu-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "wenshu-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "wenshu-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "wenshu-kanban-event-header-plain" },
                  h("span", { className: "wenshu-kanban-event-kind" }, e.kind),
                  h("span", { className: "wenshu-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "wenshu-kanban-event-phantom-row" },
                  h("span", { className: "wenshu-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "wenshu-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "wenshu-kanban-event-payload" },
    ...
    return h("div", { className: "wenshu-kanban-section" },
      h("div", { className: "wenshu-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "wenshu-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `wenshu-kanban-run--${r.outcome || r.status || "ended"}`
          : "wenshu-kanban-run--active";
        return h("div", { key: r.id, className: cn("wenshu-kanban-run", outcomeClass) },
          h("div", { className: "wenshu-kanban-run-head" },
            h("span", { className: "wenshu-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "wenshu-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "wenshu-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "wenshu-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "wenshu-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "wenshu-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "wenshu-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "wenshu-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "wenshu-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "wenshu-kanban-pre wenshu-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "wenshu-kanban-meta-row" },
      h("span", { className: "wenshu-kanban-meta-label" }, props.label),
      h("span", { className: "wenshu-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "wenshu-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "wenshu-kanban-meta-row" },
        h("span", { className: "wenshu-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "wenshu-kanban-meta-value wenshu-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "wenshu-kanban-meta-row" },
        h("span", { className: "wenshu-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "wenshu-kanban-meta-value wenshu-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "wenshu-kanban-section" },
      h("div", { className: "wenshu-kanban-section-head-row" },
        h("span", { className: "wenshu-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "wenshu-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "wenshu-kanban-section" },
      h("div", { className: "wenshu-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "wenshu-kanban-deps-row" },
        h("span", { className: "wenshu-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "wenshu-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "wenshu-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "wenshu-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "wenshu-kanban-dep-chip-x",
      ...
      h("div", { className: "wenshu-kanban-deps-row" },
        h("span", { className: "wenshu-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "wenshu-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "wenshu-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "wenshu-kanban-msg-ok"
          : "wenshu-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "wenshu-kanban-section" },
      ...
      h("div", { className: "wenshu-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "wenshu-kanban-home-sub wenshu-kanban-home-sub--on"
              : "wenshu-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__WENSHU_PLUGINS__ && typeof window.__WENSHU_PLUGINS__.register === "function") {
    window.__WENSHU_PLUGINS__.register("kanban", KanbanPage);
  }
})();
