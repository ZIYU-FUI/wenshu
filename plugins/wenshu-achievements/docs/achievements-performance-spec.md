# Wenshu Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: wenshu-achievements plugin
...
### `GET /api/plugins/wenshu-achievements/achievements`
...
### `POST /api/plugins/wenshu-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/wenshu-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/wenshu-achievements/overview`
...
- Persist plugin data under:
  - `~/.wenshu-hermes/plugins/wenshu-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.wenshu-hermes/plugins/wenshu-achievements/scan_checkpoint.json`
...
- `~/.wenshu-hermes/plugins/wenshu-achievements/`
