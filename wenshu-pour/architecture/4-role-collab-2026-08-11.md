...
> 装机 user 8/11 17:00 OOB: "先从 GIT 借鉴我 anget 研发团队的现有模式,代入到我们的五个角色(CC 包含,但不是角色),如果更好的组织,更好的实现我的需求,不要一个需求推动一整天,修改多次。先讨论如果在大流程层面,能让大家都能发挥自己的 100% 能力,能守住质量关"
>
> 装机 user 8/11 19:35 OOB: "这种卡写权限的事,别再出现,我一直都是和你在对话的,这种时候就应该是你来写,然后和我要权限,你在卡片里,我都无法授权"
>
> 装机 user 8/11 19:45 OOB: "你继续推进吧"

## 4 角色群策群力开会卡

装机 user 8/11 16:30 拍: "1 张卡 4 角色都能接,然后大家在卡里评论,这样大家都同步都 知道其它角色反馈的问题,像开会一样"
...
| 3 | t_0a14dbff | reviewer | 问题收集卡 3 (V0Fix LayoutTests 字符串 grep + hermes dispatcher spawn 死 + 装机 user 头尾在看板外) |
...
| - | t_99ac9ba1 | my-pm | fire-wrapper 合并拍板卡 — AIF 19:35 已 archive (装机 user 拍 "别在卡片里授权") |
...
PM-direct 自沉淀 5 派单协议 (装机 user 8/11 拍 "沟通失败自己沉淀"):
...
- (5) **CC 完成 ≠ 完工** (必须 CUA 拍 6 截图 + 装机 user 头尾拍)

**落档**: `~/.hermes/profiles/my-pm/skills/pm-direct-5-paipan-protocols-v0030/SKILL.md`

PM-direct 自纠: "不要让装机 user 在 hermes 看板里拍 ABC" = AIF 19:35 直接在对话里改 wenshu/AGENTS.md ✅

### 3. reviewer 3 自承错 (t_0a14dbff)

reviewer 自承错:

- (1) **V0Fix3 PASS-WITH-NOTES 自报漏洞 1 但没推动修真** (V0-fix-1 commit 1512a68d3 装机 user 重启发现 2 处消失, reviewer 没推动修真 = **reviewer 环节漏**)
...
- 装机 user 8/11 19:35 拍板: "AGENTS.md 你自己改啊,和我要权限就好"
- AIF 大管家承诺: **永远在对话里和装机 user 拍板, 不派 fire-wrapper 卡让装机 user 在 hermes 看板里授权**
- 已落: commit `269a0f774` (wenshu/AGENTS.md §3 + P12 + P12.1, push 双仓)

### 改进 2: V0Fix LayoutTests 字符串 grep → ViewInspector 视觉测试

- V0Fix 6/7/8/9/10 全是字符串 grep (V0-fix-1 commit 1512a68d3 落 main 后装机 user 重启发现 2 处消失 = §10.2 漏洞 1)
...
- AIF 19:30 大管家主动接管: **不派 fire-wrapper 卡让装机 user 拍 ABC, 直接在对话里落 B1+B2 路径真值**
- 装机 user 19:00 授权 "全放行" = AIF 19:35 直接 commit `269a0f774` 改 wenshu/AGENTS.md

### 改进 5: 4 角色开会 → 群策群力协同

- 装机 user 8/11 16:30 拍: "1 张卡 4 角色都能接,然后大家在卡里评论,像开会一样"
...
- AIF 大管家沉淀本报告 (`4-role-collab-2026-08-11.md`), 反馈装机 user

## 装机 user 头尾规则 (AIF 大管家承诺)

- 永远在对话里和装机 user 拍板, 不派 fire-wrapper 卡
- 派单必含图真位置 (4 件套: 目标/范围/标准/边界, 装机 user 8/11 16:00 OOB)
- 截图必流转 (`wenshu-pour/architecture/screenshots/`)
- 不轻易拉起 APP (4 件前置: `git pull / build exit 0 / mtime < 60s / CUA non-0x0`)
- 1+2+3 派单格式 (0 废话, 不写 OOB 历史, 不重复 OOB 历史, 不写"等装机 user 验"阻塞字样)
...
## AIF 大管家继续推进 (5 件事, 装机 user 8/11 19:45 拍 "你继续推进吧")
