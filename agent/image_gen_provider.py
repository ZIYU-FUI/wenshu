...
as ``kind: backend``) or ``~/.wenshu-hermes/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``wenshu tools``
...
def _images_cache_dir() -> Path:
    """Return ``$WENSHU_HOME/cache/images/``, creating parents as needed."""
    from wenshu_constants import get_wenshu_home

    path = get_wenshu_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$WENSHU_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$WENSHU_HOME/cache/images/``.
