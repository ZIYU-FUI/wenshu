...
projection into 文枢' display, approval bridging, transcript projection into
...
"codex_app_server"`. 文枢' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Wenshu secret — gateway
        ...
        spawn_env = wenshu_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("WENSHU_KANBAN_TASK"):
            kanban_db = spawn_env.get("WENSHU_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "WENSHU_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("WENSHU_HOME", os.path.expanduser("~/.wenshu-hermes")),
                        "kanban",
                    ),
                )
            )
