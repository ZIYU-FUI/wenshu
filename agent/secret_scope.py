...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the WENSHU_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "WENSHU_HOME", "WENSHU_PROFILE", "WENSHU_GATEWAY_LOCK_DIR",
    "WENSHU_MAX_ITERATIONS", "WENSHU_MAX_TOKENS", "WENSHU_API_TIMEOUT",
    "WENSHU_REDACT_SECRETS", "WENSHU_NOUS_TIMEOUT_SECONDS",
    "_WENSHU_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "WENSHU_KANBAN_DB", "WENSHU_KANBAN_WORKSPACES_ROOT", "WENSHU_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "WENSHU_KANBAN_",
    "WENSHU_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(wenshu_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(wenshu_home) / ".env")
