...
from wenshu_constants import get_wenshu_home, get_skills_dir, is_wsl
...
_WENSHU_MD_NAMES = (".wenshu.md", "WENSHU.md")


def _find_wenshu_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.wenshu.md`` or ``WENSHU.md``.
    ...
    # could pick up a .wenshu.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _WENSHU_MD_NAMES:
...
WENSHU_AGENT_HELP_GUIDANCE = (
    "You run on 文枢 (by Nous Research). When the user needs help with "
    "文枢 itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://hermes-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `wenshu-agent` skill with skill_view(name='wenshu-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.wenshu-hermes/kanban.db`. Your task id is in "
    "`$WENSHU_KANBAN_TASK`; your workspace is `$WENSHU_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $WENSHU_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $WENSHU_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${WENSHU_KANBAN_BRANCH:-wt/$WENSHU_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$WENSHU_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`wenshu profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `wenshu kanban <verb>` for board operations. Use "
...
#
# The wenshu-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to wenshu-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `wenshu computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that 文枢 "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the 文枢 terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the 文枢 desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the 文枢 WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Wenshu itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host 文枢 runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where 文枢 itself is running. The host OS, home, and cwd "
                f"of the 文枢 process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where 文枢 "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Wenshu
    ...
    extra = (os.getenv("WENSHU_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_wenshu_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("WENSHU_PLATFORM") or os.environ.get("WENSHU_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.wenshu-hermes/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot 文枢 itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `wenshu-agent` skill "
            "first. It has the actual commands (e.g. `wenshu config set …`, `wenshu tools`, "
            "`wenshu setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: wenshu setup, wenshu setup tools, wenshu setup terminal, wenshu status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from WENSHU_HOME and return its content, or None.
    ...
    """
    try:
        from wenshu_cli.config import ensure_wenshu_home
        ensure_wenshu_home()
    except Exception as e:
        logger.debug("Could not ensure WENSHU_HOME before loading SOUL.md: %s", e)

    soul_path = get_wenshu_home() / "SOUL.md"
...
def _load_wenshu_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.wenshu.md / WENSHU.md — walk to git root."""
    wenshu_md_path = _find_wenshu_md(cwd_path)
    if not wenshu_md_path:
        return ""
    try:
        content = wenshu_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = wenshu_md_path.name
        try:
            rel = str(wenshu_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".wenshu.md", context_length=context_length,
            read_path=str(wenshu_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", wenshu_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .wenshu.md / WENSHU.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from WENSHU_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Wenshu install/source
    ...
    # explicitly configured cwd is honored verbatim — the Wenshu tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Wenshu in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the 文枢 install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_wenshu_md(cwd_path, context_length)
    ...
    # SOUL.md from WENSHU_HOME only — skip when already loaded as identity
    if not skip_soul:
