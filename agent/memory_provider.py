...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `wenshu backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - wenshu_home (str): The active WENSHU_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.wenshu``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "wenshu").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'wenshu memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], wenshu_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'wenshu memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``wenshu_home`` is the active WENSHU_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE WENSHU_HOME.

        ``wenshu backup`` only walks WENSHU_HOME, so any provider state kept
        ...
        of the archive, and ``wenshu import`` restores them to their original
