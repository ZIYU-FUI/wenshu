...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_WENSHU_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, 文枢 expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``wenshu_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_wenshu_home()`` themselves.
        """
        if "wenshu_home" not in kwargs:
            from wenshu_constants import get_wenshu_home
            kwargs["wenshu_home"] = str(get_wenshu_home())
        for provider in self._providers:
            try:
