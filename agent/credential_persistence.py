...
``auth.json``.  They intentionally have no dependency on ``wenshu_cli.auth`` so
...
# Sources Wenshu owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Wenshu-owned OAuth/device-code state)
