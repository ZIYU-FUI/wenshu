...
upstream prefix cache warm.  See ``wenshu-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``WENSHU_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("WENSHU_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  文枢 never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want WENSHU_HOME persona while keeping
    ...
    # Pointer to the wenshu-agent skill + docs for user questions about Wenshu itself.
    stable_parts.append(WENSHU_AGENT_HELP_GUIDANCE)
    ...
    # WENSHU_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Wenshu, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Wenshu profile the agent is running
    # under so it doesn't conflate ~/.wenshu-hermes/skills/ (default profile) with
    # ~/.wenshu-hermes/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active 文枢 profile: default. Other profiles (if any) live "
            "under " + str(get_wenshu_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active 文枢 profile: {active_profile}. This session reads "
            f"and writes {get_wenshu_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_wenshu_home()}/skills/, {get_wenshu_home()}/plugins/, "
            f"{get_wenshu_home()}/cron/, {get_wenshu_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Wenshu). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — 文枢 never rebuilds or reinjects parts of it
