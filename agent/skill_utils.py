...
from wenshu_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``WENSHU_KANBAN_TASK`` / ``WENSHU_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("WENSHU_KANBAN_TASK") or os.getenv("WENSHU_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Wenshu Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``wenshu_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``WENSHU_PLATFORM`` or
            ``WENSHU_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("WENSHU_PLATFORM")
        or get_session_env("WENSHU_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``wenshu`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.wenshu-hermes/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``wenshu`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against WENSHU_HOME, not cwd
        if not p.is_absolute():
            p = (wenshu_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.wenshu-hermes/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.wenshu-hermes/skills/`` (including via symlinks) or configured
    ...
    # ~/.wenshu-hermes/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: 文枢 can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    wenshu = metadata.get("wenshu") or {}
    if not isinstance(wenshu, dict):
        wenshu = {}
    return {
        "fallback_for_toolsets": wenshu.get("fallback_for_toolsets", []),
        "requires_toolsets": wenshu.get("requires_toolsets", []),
        "fallback_for_tools": wenshu.get("fallback_for_tools", []),
        "requires_tools": wenshu.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    wenshu = metadata.get("wenshu")
    if not isinstance(wenshu, dict):
        return []
    raw = wenshu.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes 文枢 metadata, VCS, virtualenv/dependency, cache, and skill
