...
Storage
-------
Bundles live in ``~/.wenshu-hermes/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``WENSHU_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``wenshu bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under WENSHU_HOME.

    Honors ``WENSHU_BUNDLES_DIR`` for tests; falls back to
    ``<WENSHU_HOME>/skill-bundles``.
    """
    override = os.environ.get("WENSHU_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_wenshu_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `wenshu bundles` CLI subcommand.
# ---------------------------------------------------------------------------
