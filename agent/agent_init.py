...
from wenshu_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: wenshu config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$WENSHU_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_wenshu_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$WENSHU_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .wenshu.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / WENSHU_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.wenshu-hermes/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from wenshu_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind WENSHU_DEV_CREDITS.
    ...
    # both live under ~/.wenshu-hermes/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from wenshu_logging import setup_logging, setup_verbose_logging
    setup_logging(wenshu_home=_ra()._wenshu_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that wenshu_logging.setup_logging() does not
        ...
        # handler level inside wenshu_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from wenshu_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from wenshu_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `wenshu model` to "
                        "select a provider, or run `wenshu setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff WENSHU_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.wenshu-hermes/sessions/ alongside gateway sessions
    wenshu_home = get_wenshu_home()
    agent.logs_dir = wenshu_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.wenshu-hermes/sessions/session_{sid}.json)
    ...
    try:
        from wenshu_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from wenshu_cli.config import load_config as _load_agent_config
    ...
    # management API (the historical Wenshu behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "wenshu"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Wenshu' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "wenshu", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, wenshu, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from wenshu_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from wenshu_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by 文枢.  Choose a model with at least "
    ...
    # Nous Wenshu 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from wenshu_cli.model_switch import _check_wenshu_model_warning

            _wenshu_warn = _check_wenshu_model_warning(agent.model or "")
            if _wenshu_warn:
                _user_msg = (
                    "⚠ Nous Research 文枢 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_wenshu_warn)
        except Exception:
            pass
