...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into 文枢 with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.wenshu-hermes/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of 文枢
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``wenshu tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``wenshu setup`` for
