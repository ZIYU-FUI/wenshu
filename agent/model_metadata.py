...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. 文枢 also honours WENSHU_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("WENSHU_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_wenshu_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Wenshu Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Wenshu from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Wenshu' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets 文枢 minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from wenshu_constants import get_wenshu_home
    return get_wenshu_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Wenshu grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from wenshu_cli.models import get_copilot_model_context
    ...
    # ``Wenshu-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets 文枢 sends to providers:
