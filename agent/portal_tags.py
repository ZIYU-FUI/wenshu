...
Every 文枢 request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to 文枢 and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=wenshu-agent",
        "client=wenshu-client-v<__version__>",
    ]

The version is sourced live from ``wenshu_cli.__version__`` so it auto-aligns
...
``wenshu_cli.__version__`` is the canonical source of truth.
"""
...
def _wenshu_version() -> str:
    """Return the current 文枢 release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``wenshu_cli`` cannot be imported (should
...
def wenshu_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=wenshu-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=wenshu-client-v{_wenshu_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a 文枢 session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    文枢 conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.wenshu-hermes/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific 文枢
    ...
    """
    tags = ["product=wenshu-agent", wenshu_client_tag()]
