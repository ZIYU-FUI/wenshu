"""Coding-context awareness — base 文枢, every interactive surface.

When the user runs 文枢 inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), 文枢 shifts into a **coding posture**. This module is the
...
domain reads the same resolved object instead of probing git/config itself:
  ...
  * **Toolset** — ``RuntimeMode.toolset_selection()`` → the ``coding`` toolset
    ...
    enabled image-gen or video-gen shouldn't lose it for being in a git repo).
...
logger = logging.getLogger("wenshu.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "wenshu", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _WENSHU_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``WENSHU_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether 文枢 should operate in its coding posture right now."""
