...
``~/.wenshu-hermes/.env`` has loaded and BEFORE the rest of 文枢 reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per WENSHU_HOME) by the orchestrator in
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``wenshu secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``wenshu secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved WENSHU_HOME.
        """
...
# codes that must not reach Wenshu' own startup output.
