...
# 5. Symlinks the 'wenshu' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Wenshu Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/wenshu-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink wenshu into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up wenshu command..."

WENSHU_BIN="$SCRIPT_DIR/venv/bin/wenshu"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$WENSHU_BIN" "$COMMAND_LINK_DIR/wenshu"
echo -e "${GREEN}✓${NC} Symlinked wenshu → $COMMAND_LINK_DISPLAY_DIR/wenshu"
...
# ============================================================================
# Seed bundled skills into ~/.wenshu-hermes/skills/
# ============================================================================

WENSHU_SKILLS_DIR="${WENSHU_HOME:-$HOME/.wenshu-hermes}/skills"
mkdir -p "$WENSHU_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.wenshu-hermes/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$WENSHU_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  wenshu status        # Check configuration"
if is_termux; then
    echo "  wenshu gateway       # Run gateway in foreground"
else
    echo "  wenshu gateway install # Install gateway service (messaging + cron)"
fi
echo "  wenshu cron list     # View scheduled jobs"
echo "  wenshu doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m wenshu_cli.main setup
fi
