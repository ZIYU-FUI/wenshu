...
**1 句总结**: t_c59a994e 在 v0.03.0 DESIGN-SYSTEM-INIT (commit bde233d42, 983 行) 基础上,加 §9 (P11 防回退 + P12 AIF 拉起必拍 6 截图 + 5 步真机拍回归 + 5 角色共同遵守) + §10 (装机 user 头尾流程审视 + 漏洞 + 修法),原 §9 → §11,原 §10 → §12,新增 P11/P12,旧 P10/P11/P12 → P13/P14/P15 让位 (P1-P14 → P1-P15),5 角色共同遵守 "做完不删 + 改前清单 + 改后逐项验 + 真机拍 6 截图" 4 件套长期原则

**2 句拍板理由**:
- 装机 user 8/10 15:55 OOB 拍 "做完的东西被改没了 / 上个版本还有新建项目的流程,虽然有问题 / 审核员是如何审核的" = v0-fix-1 commit `1512a68d3` 落 main 后,装机 user 重启 APP **实机拍才发现 2 处消失** (顶部 + 按钮 + 左上 5 tab 列表),真根因 = reviewer 7 测试只测字符串不测功能存在 + PM-direct 兜底 commit 没真机拍 + AIF 派单边界不严
- AGENTS §3 + §13 + wenshu-editor-fcp-viewer-pattern.md §0 已落 "FCP 结构 + 文枢功能 + 5 角色共同遵守",本卡落地 P11/P12 防回退原则 + §9.2 5 步真机拍回归流程 + §10.1 头尾流程图,**系统化修补头尾漏洞**,非功能创新

**3 句边界**:
- ✅ 出 SwiftUI 设计意图 + token + 组件 API + SF Symbol 映射 (design doc, markdown 文件)
- ❌ 不写任何 .swift 代码 (CC 翻译), 不动 v0.02.0 main 业务 / schema / 5 区几何 / .ws
- ❌ 0 阻塞字段 (不写 "等装机 user 验" / "review-required: 装机 user" / "用飞书" / "用飞机" 等阻塞语),装机 user 头尾在看板外 (P9)
- ❌ 不动 v0.03.0 DESIGN-SYSTEM-INIT 已落的 §1-§8 (12 原则 + token + 8 组件 + SF Symbol + 状态层 + 文案 + 5 角色 + AC 8 段) — 加 §9 + §10 补充,§1-§8 0 字节变动
- ❌ 不抢 PM-direct / AIF 拍板项 (§12),仅出 design doc 提议 (§9.4 派子卡给 PM-direct 拍板)

## 任务边界矛盾点 (designer 必列, 等 PM-direct / AIF 拍)

读了 v0-fix-1 commit `1512a68d3` (Diff 看了 LayoutShellView / ChatPanelView / ProjectCreateView + V0Fix1LayoutTests 7 测试) + v0-fix-2 commit `cfce73389` + `163a0ff4d` + v0.02.0 main (commit 492c96750) ProjectListView / ChatPanelView / LayoutShellView,发现 2 处跟现有拍板 / 现状冲突。designer 不替 PM / 装机 user 拍,列这里等拍。

### 矛盾 1: P10 / P11 新旧定义冲突

- **事实**: 原 §1 P10 = "设计系统 0 视觉创新" (FCP 范式真理源为本); P11 = "文案 ≤ 10 字"; P12 = "暗色优先"。 装机 user 15:55 OOB 拍新增 "P10 防回退" + "P11 AIF 拉起必拍 6 截图"
- **冲突**: 新旧 P10 / P11 编号冲突 — 原 P10 = 视觉创新,新 P10 = 防回退,语义完全不同
- **可能的真意**:
  - 读法 A (本卡采用): 旧 P10 / P11 / P12 → P13 / P14 / P15 让位,新 P11 / P12 = 防回退 + AIF 6 截图。系统号 P1-P14 → P1-P15
  - 读法 B: 旧 P10 删,新 P10 = 防回退 (合并);旧 P11 = 文案保留;旧 P12 = 暗色保留。系统号仍 P1-P12 (新加 P11 防回退直接叫 "P11 防回退",与原 P11 文案同名但语义不同,会混淆)
- **建议**: 读法 A (已采)。 等 PM-direct / 装机 user 拍板长期原则 (P10 / P11 入 AGENTS §3)

### 矛盾 2: 装机 user 实机拍截图引用

- **事实**: 任务 body 写 "必看装机 user 8/10 15:55 OOB 截图" + "必看 FCP 截图",但本 worktree 内**没有**这 2 张截图。 装机 user 用 Lark/Feishu 发的附件,在另一路径
- **冲突**: designer 出稿时无法直接 inspect 截图。 当前只能引用路径名 `composer_2026-08-10_07-47-03-040_10dd58.png` (FCP 截图) + "装机 user 8/10 15:55 OOB 截图"(待装机 user 提供)
- **可能的真意**:
  - 读法 A: designer 不需要看截图, 装机 user 15:55 OOB 描述的 2 处消失已明确 (顶部 + 按钮 + 左上 5 tab 列表), 本卡 design doc §9.1 before 状态清单已列 12 元素,无需截图
  - 读法 B: designer 必须 inspect 截图才出稿,等装机 user 提供
- **建议**: 读法 A (已采)。 但建议装机 user 后续把 15:55 OOB 截图存到 `~/.hermes/profiles/designer/design-notes/wenshu-8-10-oob-screenshots/`,供后续卡引用

-->

# DESIGN-SYSTEM-INIT · 文枢 macOS 端 SwiftUI UI 设计系统初版 (FCP 范式)

> **版本**: v0.03.0-INIT-r2 (t_c59a994e 补 §9 + §10) · **拍板**: 装机 user 8/10 15:50 OOB + 15:55 OOB
...
1. **读 §1 原则** (P1-P9 + designer 加的 P10-P15,文枢 UI 第一性约束)
...
## §1 · 原则 (P1-P15,文枢 UI 的 15 条铁律)

> **层级**: P1-P5 = 视觉第一性(必守);P6-P9 = 交互第一性(必守);P10-P15 = 协作第一性(designer 加的扩展,5 角色共同遵守)
...
### P11 · 防回退原则 (改前清单 + 改后逐项验 + 真机拍对比) — designer 加 (8/10 15:55 OOB 补)

- **规则**: 改任何代码前,designer / CC **必** 列出 "当前应该有" 的功能清单 (before 状态),改完**必**逐项回验清单,**不删任何不明确删的东⻄** (防误删)。 AIF 拉起 APP **必**真机拍 / CUA 拍 6 截图,与 v0-fix-N-1 逐张对比
- **真根因 (装机 user 8/10 15:55 OOB 拍)**:
  - v0-fix-1 commit `1512a68d3` 落 main 后,装机 user 重启 APP **实机拍才发现 2 处消失** (顶部 + 按钮 + 左上 5 tab 列表)
  - "上个版本还有新建项目的流程,虽然有问题" = v0.02.0 main (commit 492c96750) 之前有 + 按钮和 5 tab,**功能在但有 BUG**; v0-fix-1 落 main 后**功能被改没了** = "做完的东西被改没了"
- **5 步真机拍回归流程** (5 角色共同遵守,§9.2 详):
  1. **AIF 拉 main 启 WenshuApp 前**,AIF 拍 "当前功能清单" = 拍 6 张截图 (标题栏 / 左上 5 tab / 中上 / 右上 / 底部 chat / 底部时间线)
  2. **CC 写完代码,swift build + swift test 全过后**,AIF **主动拉起 APP 拍 6 张对比截图**
  3. AIF 拉起后**真机拍或 CUA 拍** 6 张截图,与步骤 1 逐张对比,**任何功能消失 = 不通过**,必**回退到 v0-fix-N-1 + 重写** = 强制 fail
  4. AIF 把 6 张对比截图 + 装机 user 真机拍截图 一起贴 AGENTS §3 拍板历史 (或本文件 §11) — 装机 user 头尾**主动反馈**用
  5. 装机 user 在电脑前 = 主动反馈 BUG,装机 user 不在 = AIF CUA 验 6 截图自动对比 — 头尾在看板外
- **不允许**:
  - ❌ 改任何代码前不列 "当前应该有" 清单
  - ❌ 改完不逐项回验清单就 commit
  - ❌ "顺手清理" / "重构" / "优化" 时删除 "不太理解" 的代码
  - ❌ AIF 拉起 APP 不拍 6 截图直接 reviewer 7 测试通过就 merge
- **仅允许**:
  - ✅ 改前先 `git diff <branch>` 看清当前状态
  - ✅ 列 "before 状态清单":例如 v0-fix-2 → v0-fix-3 = "5 tab 容器 + onCreate closure + 4 chat tab Picker + 右上 2 tab Picker" 共 12 个 UI 元素
  - ✅ 改完逐项 grep / ViewInspector 验证清单全在
  - ✅ AIF 拉起 APP 拍 6 截图 vs before 6 截图,像素级对比
- **反例** (v0-fix-1 误删案例):
  - 顶部 + 按钮被 v0-fix-1 commit 改没了 (1 处消失)
  - 左上 5 tab 列表容器被 v0-fix-1 commit 改没了 (1 处消失)
  - 测试 `testLayoutShellView_xxx_noH1Text` 测 "contains '项目管理视图'" 失败 = **测试方法错**,应测 "功能存在" (e.g. `testProjectListView_5tabList_present`)
- **v0-fix-3 派生**: v0-fix-3 CC 卡**必须跑本规则**:
  1. CC 写代码前 = 必读本文件 §9 + §10
  2. CC 必跑 before 状态对比 (用 `git show <fix-1>:<file>` 拿原文件)
  3. CC 必新加 6 个回归测试 (12 元素存在性,详 §10.3 漏洞 1)
  4. CC commit 必在 commit message 写 "before/after 6 截图" 路径

### P12 · AIF 拉起 APP 必拍 6 截图原则 (P11 子集, AIF 必走) — designer 加 (8/10 15:55 OOB 补)

- **规则**: AIF 拉起 APP (CC merge main 后,装机 user 重启前) = **必**真机拍 / CUA 拍 6 截图 + 与 v0-fix-N-1 对比 + 任一功能消失 = 必回退
- **6 截图固定清单** (5 区域 + 标题栏):
  1. **标题栏** — P1 验证: 38pt 高 + 0 文字 + ICON-only + 最左 + ICON
  2. **左上项目管理** — P3 + P4 验证: 5 tab (项目/章节/设定/资料/看板) 都在 + 无 H1
  3. **中上文档** — P3 + P4 验证: 无 H1 + 章节正文区
  4. **右上 inspector** — P3 + P4 + P5 验证: 2 tab (伏笔/修订) 都在 + ICON-only + 无 H1
  5. **底部 chat** — P2 + P5 验证: 4 子 tab (聊天实装 + 时间线/关系图/大纲 disabled) 都在 + ICON-only + 无 "聊天区视图" H1
  6. **底部时间线** (或状态区) — P3 + P4 验证: 多视图切换 ICON-only + 无 H1
- **对比规则**:
  - AIF 拉起前 = 已拍 before 6 截图 (v0-fix-N-1 状态)
  - AIF 拉起后 = 拍 after 6 截图 (v0-fix-N 状态)
  - 逐张对比,任一**功能消失** (按钮 / tab / H1 / 容器 / 弹窗 / 拆分条) = 强制回退到 v0-fix-N-1 + 重写
- **不允许**:
  - ❌ AIF merge main 后不拍 6 截图就通知装机 user 重启
  - ❌ AIF 只看 reviewer 7 测试报告就推进
  - ❌ 装机 user 重启发现 BUG 后才补拍 (已经晚了)
- **仅允许**:
  - ✅ AIF merge main 后**先拍 6 截图** (有 Mac mini 在身边时 = 真机拍,不在时 = `computer_use` CUA 拍)
  - ✅ 6 截图存档到本文件 §11 (拍板历史) 或 AGENTS §3
  - ✅ 装机 user 重启后**主动反馈** BUG (与 before 对比) 或 ✅ 无 BUG

### P13 · 文案 = 中文 ≤ 10 字 + 英文 (代码/identifier) + tooltip 装长描述 — designer 加
...
## §9 · 回归防退化 (P11 防回退 + P12 AIF 拉起必拍 6 截图) — 5 角色共同遵守

> **真根因 (装机 user 8/10 15:55 OOB 拍)**: v0-fix-1 commit `1512a68d3` 落 main 后,装机 user 重启 APP 实机拍才发现 2 处消失 (顶部 + 按钮 + 左上 5 tab 列表)。**功能被改没了**,不是 BUG 修了
> **本节目的**: 5 角色共同遵守 "做完不删 + 改前清单 + 改后逐项验 + 真机拍 6 截图" 4 件套,**长期原则**,不为单次例外
> **真理源**: AGENTS §3 (拍板历史)+ §13 (项目基线) + wenshu-editor-fcp-viewer-pattern.md §0 总原则

### 9.1 核心原则 (P11 防回退)

- **P11 (防回退)**: 改任何代码前,designer / CC **必** 列出 "当前应该有" 的功能清单 (before 状态),改完**必**逐项回验清单,**不删任何不明确删的东⻄** (防误删)。 AIF 拉起 APP **必**真机拍 / CUA 拍 6 截图,与 v0-fix-N-1 逐张对比,**任一功能消失 = 强制回退**。**长期原则**,5 角色共同遵守,不为单次例外

**before 状态清单模板** (CC 写代码前必填):

```markdown
## before 状态清单 (v0-fix-N → v0-fix-N+1)

CC 必先 `git show <last-good-commit>:<file>` 拿以下元素:

| # | 元素 | 位置 | 验证方式 | 必须保留? |
|---|---|---|---|---|
| 1 | 顶部 + 按钮 | LayoutShellView.topLeftPanelWithTitleBar | grep "plus.circle.fill" + .help("新建项目") | ✅ 必 |
| 2 | 顶部 title-bar 38pt 高度 | LayoutShellView.topLeftPanelWithTitleBar | grep ".frame(height: 38)" | ✅ 必 |
| 3 | 左上 5 tab 容器 | ProjectListView | grep "TabBar\|WenshuTabBar\|Picker.*5" | ✅ 必 |
| 4 | 左上 5 tab ICON-only | ProjectListView | grep ".pickerStyle(.iconOnly)" | ✅ 必 |
| 5 | 左上 onCreate closure | ProjectListView | grep "navPath.append(AppRoute.createProject)" | ✅ 必 |
| 6 | 底部 chat 4 tab Picker | ChatPanelView | grep "Picker.*activeTab" | ✅ 必 |
| 7 | 底部 chat 4 tab .iconOnly | ChatPanelView | grep ".pickerStyle(.iconOnly)" | ✅ 必 |
| 8 | 底部 chat "聊天区视图" H1 = 无 | ChatPanelView | grep -c "Text(\"聊天区视图\")" = 0 | ✅ 必 |
| 9 | 右上 inspector 2 tab Picker | InspectorView | grep "Picker.*activeTab" | ✅ 必 |
| 10 | 右上 inspector 2 tab .iconOnly | InspectorView | grep ".pickerStyle(.iconOnly)" | ✅ 必 |
| 11 | 右上 inspector "检视" H1 = 无 | InspectorView | grep -c "Text(\"检视\")" = 0 | ✅ 必 |
| 12 | 新建项目弹窗 540×480 | ProjectCreateView | grep ".frame(width: 540, height: 480)" | ✅ 必 |
```

CC 改完代码后,逐项 grep 验证清单,12 元素全在 = 通过,任一不在 = 必重写。

### 9.2 5 步真机拍回归流程 (装机 user 头尾流程 + AIF CUA 验 fallback)

> **背景**: 装机 user 头尾在看板外 (P9 / 0 阻塞),但**重启 APP 时 = 必主动反馈 BUG**。 本流程保证 BUG 在 AIF merge main 前就被发现,不是装机 user 重启才发现
> **触发时机**: 任何 v0-fix-N commit 落 main 后 + 装机 user 重启 APP 前

| 步骤 | 动作 | 谁做 | 触发条件 | 输出 |
|---|---|---|---|---|
| **步骤 1** | 拉 main 启 WenshuApp 前的 "当前功能清单" = 拍当前 6 张截图 (标题栏 / 左上 5 tab / 中上 / 右上 / 底部 chat / 底部时间线) | **AIF 必做** (AIF 1 阶验收) | CC merge main 准备 AIF 1 阶验收前 | before-6-截图/ 目录 |
| **步骤 2** | CC 写完代码,swift build + swift test 全过后,AIF 主动拉起 APP 拍 6 张对比截图 | **AIF 必做** (AIF 1 阶验收) | swift test 全过后 | after-6-截图/ 目录 |
| **步骤 3** | AIF 拉起后**真机拍或 CUA 拍** 6 张截图,与步骤 1 逐张对比,**任何功能消失 = 不通过**,必**回退到 v0-fix-N-1 + 重写** = 强制 fail | **AIF 必做** (AIF 1 阶验收) | after 截图拍完 | 对比报告 (6 张 before/after 对比) |
| **步骤 4** | AIF 把 6 张对比截图 + 装机 user 真机拍截图 一起贴 AGENTS §3 拍板历史 (或本文件 §11) — 装机 user 头尾**主动反馈**用 | **AIF 必做** (AIF 1 阶验收) | 对比通过 (无功能消失) | 6 张对比截图存档 |
| **步骤 5** | 装机 user 在电脑前 = 主动反馈 BUG,装机 user 不在 = AIF CUA 验 6 截图自动对比 — 头尾在看板外 | **装机 user 主动反馈** / AIF CUA 自动 | 装机 user 重启 APP 时 | BUG 反馈 (如发现) |

**6 截图固定清单** (5 区域 + 标题栏,与 P12 同步):
...
**AIF CUA 验 fallback** (装机 user 不在电脑前):

- AIF 用 `computer_use` CUA 拍 6 张截图 (有 Mac mini 在身边) 或 `screencapture` CLI 拍
- 6 截图存到 `~/.hermes/profiles/designer/design-notes/wenshu-before-after-screenshots/v0-fix-N/`
- 自动对比 (像素 diff + 元素检测) vs before 6 截图

**CC commit message 必填字段**:

```
v0-fix-N+1: <一句话修复什么>

before 状态清单: <12 元素全在/部分在>
before 6 截图: <路径>
after 6 截图: <路径>
对比结论: <AIF 1 阶验收通过 / 任一功能消失需回退>
12 元素 grep 验证: <全在 / 缺哪些>

回归测试:
  - testProjectListView_5tabList_present ✅
  - testLayoutShellView_topLeft_plusButton_present ✅
  - testChatPanelView_4chatTabs_iconOnly ✅
  - testChatPanelView_noChatPanelH1 ✅
  - testInspectorView_2inspectorTabs_iconOnly ✅
  - testInspectorView_noInspectorH1 ✅
```

### 9.3 5 角色共同遵守 (AIF / PM-direct / designer / CC / reviewer)

**AIF 派单时**:

- 派单 body 必加 **"不要误删 X 容器 / + 按钮 / 5 tab / 任何在 before 状态有的功能"**
- 派单 body 必加 **"before 状态清单: <12 元素>"** (或引用本文件 §9.1 模板)
- 派单 body 必加 **"必跑 §9.2 5 步真机拍回归"** + **"6 截图存档到 design-notes/wenshu-before-after-screenshots/v0-fix-N/"**
- 跨阶段 / 跨区卡必同步本文件

**designer 出稿时**:

- 出稿前**必跑 before 状态功能清单** (5 角色每个功能存在性),列 12 个 UI 元素
- 出稿后**必拍 6 截图对比** (有 Mac mini 在身边时 = 真机拍,不在时 = CUA 拍)
- sign-off comment 必引本文件 P11 + P12 + §9.1 模板

**CC 实现时**:

- 写代码前**必读 §9 + §10**,必跑 before 状态对比 (用 `git show <last-good>:<file>`)
- 写代码后**必逐项 grep 12 元素** 验证清单,任一不在 = 必重写不 commit
- 必跑 `swift build exit 0` + `swift test` + **必新加 6 个回归测试** (12 元素存在性,详 §10.3 漏洞 1)
- **不**改任何不在派单 body 明确列出的东西 (P9 0 阻塞延伸)

**reviewer 审查时**:

- 审查必**真机拍** (或 CUA 拍) **6 张对比** (before vs after),**不只**看测试字符串
- 必列 "before/after 6 截图" + before/after 12 元素清单 in 8 段审查报告
- 不只测 "字符串不存在" (e.g. `testLayoutShellView_xxx_noH1Text`) — **要测 "功能存在"** (e.g. `testProjectListView_5tabList_present`)

**PM-direct 派子卡 / 兜底 commit 时**:

- 兜底 commit 前**必真机拍 6 截图对比** (有 Mac mini 在身边时)
- **不 commit + push** 直到**装机 user 头尾流程审视完** (§10)
- 派子卡 body 必**引用** §9 + **附** before 状态清单 + **附** 真机拍模板

### 9.4 AGENTS §3 加 "防回退原则" 长期原则 (designer 出,PM-direct 拍)

> **本节内容 8/10 15:55 OOB 装机 user 拍,待 PM-direct 在 AGENTS.md §3 拍板后,正式升长期原则**

- **P10 (防回退)**: 改代码前 designer/CC 必**列 before 功能清单**,改完必**逐项验清单 + 真机拍对比**,**任何功能消失 = 强制回退**,长期原则,不为单次例外
- **P11 (AIF 拉起必拍 6 截图)**: AIF 拉起 APP = 必**真机拍 6 张** (标题栏 / 左上 5 tab / 中上 / 右上 / 底部 chat / 底部时间线) + 与 v0-fix-N-1 对比,任何功能消失 = 必回退,长期原则

**派给 PM-direct 的子卡** (designer 出稿时建议派):

- 卡标题: `[V0.03.0 DESIGN-SYSTEM-r3 AGENTS §3 加 P10/P11 长期原则] PM-direct 拍板`
- 卡 body: 引用本文件 §9.4 + 派单理由 (真根因 = 装机 user 8/10 15:55 OOB 拍"做完的东西被改没了")
- 边界: 不改 AGENTS §1 / §2 / §3 已有内容,只在 §3 加 2 条 P10/P11
- 0 阻塞,PM-direct 自决

---

## §10 · 装机 user 头尾流程审视 (5 角色 + 当前流程 + 漏洞 + 修法)

> **背景**: 装机 user 头尾在看板外 (P9 / 0 阻塞),但**重启 APP 时 = 必主动反馈 BUG**。 头尾流程审视 = AIF 凌驾 PM-direct 之上 1 阶验收,装机 user 主动反馈
> **真根因 (装机 user 8/10 15:55 OOB)**: "做完的东西被改没了" + "两次不符合规则" + "上个版本还有新建项目的流程,虽然有问题" + "审核员是如何审核的" = 头尾流程有 3 漏洞,真根因不在 CC / PM-direct, 在 AIF 1 阶验收缺真机拍

### 10.1 当前流程 (5 角色头尾)

```
[AIF 派单] ─── 1+2+3 派单格式, 装机 user 头在看板外
   ↓
[PM-direct 派子卡] ─── 继承父卡 owner + 边界, 自驱 loop
   ↓
[designer 出稿] ─── 5 段, 必引用本系统 (原则 + 组件 + token)
   ↓
[CC 实现] ─── 必读 DESIGN-SYSTEM-INIT + before 状态清单
   ↓
[reviewer 审查] ─── 必跑 swift build + 真机拍 (或 CUA 拍) 6 张对比
   ↓
[PM-direct 2 阶验收] ─── 接力 merge main, 0 阻塞
   ↓
[AIF 拉起 APP + 主动反馈] (尾) ─── 装机 user 头尾在看板外, AIF 主动驱动
   ↓
[装机 user 重启 APP + 主动反馈] ─── 在电脑前时主动反馈 BUG, 不在 = AIF CUA 验
```

**关键角色职责**:

- **AIF (头)**: 派单 body 必显式 1+2+3 格式, 必引用本系统 P 号 + before 状态清单
- **AIF (尾)**: merge main 后**必拉起 APP 拍 6 截图** vs before, 任一功能消失 = 必回退
- **designer (出稿)**: 出稿前必跑 before 状态功能清单 (12 元素), 出稿后必拍 6 截图对比
- **CC (实现)**: 写代码前必读 §9 + §10, 改完必逐项 grep 12 元素 + 必新加 6 回归测试
- **reviewer (审查)**: 审查必真机拍 6 张对比 + 必列 before/after 12 元素清单
- **PM-direct (派单 + 验收)**: 派子卡 body 必引 §9, 兜底 commit 前必真机拍 6 截图

### 10.2 漏洞 (装机 user 15:55 OOB 真值)

**漏洞 1 (reviewer 测字符串不测功能存在性)**:

- **现象**: v0-fix-1 commit `1512a68d3` 落 main 后,装机 user 重启发现 2 处消失 (顶部 + 按钮 + 左上 5 tab 列表)
- **根因**: reviewer 7 个测试只测**字符串包含** (e.g. `testLayoutShellView_xxx_noH1Text` 测 "contains '项目管理视图'" 失败 → CC 删了字符串 → 测试过 → 但**功能消失**没测到)
- **真值**: **测试方法错** — 测字符串不存在 ≠ 测功能存在。 应测 **"5 tab 容器渲染"** (e.g. `testProjectListView_5tabList_present` 用 ViewInspector 模拟) 而非 **"字符串不包含"**
- **影响**: v0-fix-1 落 main 后**功能消失**没被任何测试拦下,直到装机 user 实机拍才发现

**漏洞 2 (AIF 派单 body 边界不严)**:

- **现象**: v0-fix-1 派单 body 写 "修 6 处 BUG" + "swift build exit 0 + kanban_complete",**没写** "不要误删 5 tab 容器" 边界
- **根因**: AIF 派单时**没明确列"不要改的东西"**,CC 改过头删了 v0.02.0 main 之前的 + 按钮和 5 tab 列表容器
- **真值**: **派单边界** = "要改什么" + "**不要改什么**" (P11 防回退延伸)。 CC 看到 "不要改 X" 才会不删

**漏洞 3 (PM-direct 兜底 commit 没真自检 before 状态清单)**:

- **现象**: v0-fix-1 PM-direct 兜底 commit `1512a68d3` 后没拉起 APP 拍 6 截图对比,直接 commit + push 双仓,**没真机拍**
- **根因**: PM-direct 兜底 commit 时**没真机拍 6 截图对比**,只 swift build + swift test 通过就推进
- **真值**: **兜底 commit ≠ 安全 commit**。 PM-direct 兜底时**必**真机拍 6 截图对比 (有 Mac mini 在身边时),**不 commit** 直到装机 user 头尾流程审视完

### 10.3 修法 (5 角色共同遵守)

**AIF 修法**:

- 派单 body 必**附 before 状态 6 截图路径** (AIF merge main 前拍的 before) + §9.1 P11 防回退原则 + §10.1 当前流程
- 派单 body 必**加边界** "不要误删 X 容器" + "before 状态 12 元素全在"
- 派跨阶段 / 跨区卡前,**同步本设计系统** (AIF 派单 §7.5)

**designer 修法**:

- 出稿前**必跑 before 状态功能清单** (列 5 角色每个功能存在性)
- 出稿后**必拍 6 截图对比** (有 Mac mini 在身边时 = 真机拍,不在时 = CUA 拍)
- sign-off comment 必引 §9.1 + §9.3 + 本系统 P11/P12

**CC 修法**:

- 写代码前**必读** §9 + §10, 必跑 before 状态对比 (`git show <last-good>:<file>`)
- 写代码后**必逐项 grep 12 元素** 验证清单, 任一不在 = 必重写不 commit
- **必新加 6 个回归测试** (12 元素存在性):
  - `testProjectListView_5tabList_present` — ProjectListView 5 tab 容器渲染 (用 ViewInspector 模拟)
  - `testLayoutShellView_topLeft_plusButton_present` — LayoutShellView.topLeftPanelWithTitleBar 必含 `plus.circle.fill` + `.help("新建项目")`
  - `testChatPanelView_4chatTabs_iconOnly` — ChatPanelView Picker 必含 `.pickerStyle(.iconOnly)`
  - `testChatPanelView_noChatPanelH1` — ChatPanelView 必不含 `Text("聊天区视图")`
  - `testInspectorView_2inspectorTabs_iconOnly` — InspectorView Picker 必含 `.pickerStyle(.iconOnly)`
  - `testInspectorView_noInspectorH1` — InspectorView 必不含 `Text("检视")`

**reviewer 修法**:

- 审查必**真机拍** (或 CUA 拍) **6 张对比** (before vs after),**不只**看测试字符串
- 必列 "before/after 6 截图" + before/after 12 元素清单 in 8 段审查报告
- 测**功能存在性** (e.g. `testProjectListView_5tabList_present`) 而非 **字符串不存在** (e.g. `testLayoutShellView_xxx_noH1Text`)

**PM-direct 修法**:

- 派子卡 body 必**引用** §9 + **附** before 状态清单 + **附** 真机拍模板
- 兜底 commit 前**必真机拍 6 截图对比**, **不 commit + push** 直到**装机 user 头尾流程审视完**

---

## §11 · 拍板历史 & 已知 BUG 反向验证 (旧 §9,内容保留)

### 11.1 拍板历史

- ✅ 2026-08-10 装机 user OOB 15:50 拍: 设计师出 FCP 范式设计系统初版,5 角色共同遵守
- ✅ 2026-08-10 装机 user OOB 15:55 拍: "做完的东西被改没了 / 上个版本还有新建项目的流程,虽然有问题 / 审核员是如何审核的" = 加 P11 防回退 + P12 AIF 必拍 6 截图 + §9.2 5 步真机拍回归 + §10 头尾流程审视 (t_c59a994e 落地)
...
### 11.2 已知 BUG 反向验证 (本系统必须修复的全部 BUG)
...
### 11.3 与 swiftui-design-patterns skill 的关系
...
### 11.4 与 wenshu-designer-onboarding 的关系
...
## §12 · 留给 PM-direct / AIF 拍板项 (designer 不拍)
...
**初版落档**: 装机 user 8/10 15:50 OOB · designer 落档 v0.03.0-INIT (commit bde233d42, 983 行)
**§9+§10 补充**: 装机 user 8/10 15:55 OOB "做完的东西被改没了" → t_c59a994e designer 加 §9 (P11 防回退 + P12 AIF 必拍 6 截图 + 5 步真机拍回归) + §10 (装机 user 头尾流程审视 + 漏洞 + 修法),原 §9 → §11,原 §10 → §12,旧 P10/P11/P12 → P13/P14/P15 (P1-P14 → P1-P15),**5 角色共同遵守 "做完不删" 长期原则**
**下次 sync**: v0-fix-3 CC 完成后,补"实施证据"(实际代码 P 号对照表 + §9.2 6 截图 before/after)
**真理源优先级**: 本文件 = P1-P15 原则真理源;wenshu-editor-fcp-viewer-pattern.md = 视觉灵感真理源;AGENTS.md = 流程 + layout 真理源

---

*DESIGN-SYSTEM-INIT v0.03.0-INIT-r2 · 2026-08-10 · 装机 user 15:50 OOB 拍"抽象出 FCP 范式设计系统" + 15:55 OOB 拍"做完的东西被改没了" = 加 §9+§10,5 角色共同遵守"做完不删"原则 · 截图为真理源*
