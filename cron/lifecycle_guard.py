...
``wenshu gateway restart`` (or ``launchctl kickstart ai.wenshu.gateway``
or ``systemctl restart wenshu-gateway``).  When the cron fires, the
...
``wenshu cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``wenshu gateway``, ``launchctl ... wenshu-gateway``,
``systemctl ... wenshu-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_WENSHU_GATEWAY=1``, and
``wenshu gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `wenshu gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:wenshu\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a wenshu-gateway label. macOS launchd
    # labels look like `ai.wenshu.gateway` / `wenshu-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated wenshu services (e.g.
    # `launchctl unload ai.wenshu.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bwenshu[.\-]?gateway)"
    # Branch C: systemctl ops on a wenshu-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bwenshu[.\-]?gateway)"
    # Branch D: pkill / kill targeting the wenshu gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<WENSHU_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.wenshu-hermes/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_wenshu_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `wenshu gateway restart` from a shell outside "
