...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``wenshu://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``wenshu://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"wenshu://blueprint/{quote(blueprint.key)}{qs}"
