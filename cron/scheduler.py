...
Uses a file-based lock (~/.wenshu-hermes/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `wenshu update` reloads
# the module) fail with ModuleNotFoundError for wenshu_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from wenshu_constants import get_wenshu_home
from wenshu_cli._subprocess_compat import windows_hide_flags
from wenshu_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy wenshu_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from wenshu_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``wenshu tools`` config for the ``cron`` platform.
    ...
    try:
        from wenshu_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``wenshu update`` / ``wenshu gateway stop`` / systemd restart, or an
...
_wenshu_home: Path | None = None


def _get_wenshu_home() -> Path:
    """Resolve 文枢 home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active WENSHU_HOME at call time
    ...
    """
    return _wenshu_home or get_wenshu_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    wenshu_home = _get_wenshu_home()
    lock_dir = wenshu_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("WENSHU_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid WENSHU_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within WENSHU_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Wenshu-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against WENSHU_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_wenshu_home() / "scripts"
    ...
    # escape — scripts MUST reside within WENSHU_HOME/scripts/.
    try:
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _wenshu_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    #
    # Bounded with its own timeout (separate from WENSHU_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("WENSHU_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid WENSHU_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_wenshu_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_wenshu_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["WENSHU_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed WENSHU_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads WENSHU_SESSION_PLATFORM
    #     and WENSHU_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via WENSHU_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     WENSHU_SESSION_PLATFORM == "telegram".
    ...
    #     WENSHU_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read WENSHU_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the WENSHU_CRON_AUTO_DELIVER_* vars set
    # below, so clearing WENSHU_SESSION_* here does not affect delivery.
    _ctx_tokens = set_session_vars(
        platform="",
        chat_id="",
        chat_name="",
        # A cron job cannot receive a completion after its turn ends. We clear the
        # WENSHU_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ WENSHU_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "WENSHU_CRON_AUTO_DELIVER_PLATFORM",
        "WENSHU_CRON_AUTO_DELIVER_CHAT_ID",
        "WENSHU_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_wenshu_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this WENSHU_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_wenshu_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from wenshu_cli.env_loader import (
            load_wenshu_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_wenshu_dotenv(wenshu_home=_get_wenshu_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["WENSHU_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["WENSHU_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["WENSHU_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > WENSHU_MODEL env >
        ...
        model = job.get("model") or os.getenv("WENSHU_MODEL") or ""
        ...
        try:
            import yaml
            _cfg_path = str(_get_wenshu_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"WENSHU_MODEL={os.getenv('WENSHU_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `wenshu model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from wenshu_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("WENSHU_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_wenshu_home() / pfpath
            if pfpath.exists():
                try:
        ...
        from wenshu_cli.runtime_provider import (
        ...
        try:
            # Do not inject WENSHU_INFERENCE_PROVIDER here. resolve_runtime_provider()
            ...
            for entry in fb_list:
                ...
                if not fb_provider or not fb_model:
                    continue
                try:
                    from wenshu_cli.fallback_config import resolve_entry_api_key
        ...
        agent = AIAgent(
            ...
            # WENSHU_HOME. When a workdir is configured, also inject project
        ...
        # override via WENSHU_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("WENSHU_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid WENSHU_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_wenshu_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_wenshu_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_wenshu_home())
        )
    ...
    *,
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _wenshu_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _wenshu_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set WENSHU_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("WENSHU_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid WENSHU_CRON_MAX_PARALLEL value; defaulting to unbounded")
