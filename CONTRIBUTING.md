# Contributing to Wenshu Agent

Thank you for contributing to Wenshu Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Wenshu to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/wenshu-agent "<your terms>"
  gh search prs --repo NousResearch/wenshu-agent --state all "<your terms>"
  ```
...
Related: #38284 covers the agent-side analog — Wenshu itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Wenshu install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `wenshu skills browse` (labeled "official") and install it with `wenshu skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `wenshu skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.wenshu-hermes/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(wenshu_home, config)` for setup-wizard integration
...
- Integrate with `wenshu memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Wenshu ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.wenshu-hermes/plugins/`, project `.wenshu-hermes/plugins/`, or a pip entry point) — see [Build a Wenshu Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-wenshu-plugin)
...
The installer creates the Wenshu venv, wires the `wenshu` command, stamps the
install method for `wenshu update`, and clones the full git project into
`$WENSHU_HOME/wenshu-agent` (usually `~/.wenshu-hermes/wenshu-agent`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash
cd "${WENSHU_HOME:-$HOME/.wenshu}/wenshu-agent"
...
Use this only if you intentionally do not want Wenshu' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `wenshu` entrypoint from this venv; running the
system `python3 -m wenshu_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.wenshu-hermes/venvs/wenshu-dev --python 3.11
export VIRTUAL_ENV="$HOME/.wenshu-hermes/venvs/wenshu-dev"
...
```bash
mkdir -p ~/.wenshu-hermes/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.wenshu-hermes/config.yaml
touch ~/.wenshu-hermes/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.wenshu-hermes/.env
```

### Run

```bash
# The standard installer already put `wenshu` on PATH.
wenshu doctor
wenshu chat -q "Hello"
```

If you used the manual clone fallback, run `./wenshu` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/wenshu" ~/.local/bin/wenshu
```
...
├── cli.py                    # WenshuCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (wenshu-cli, wenshu-telegram, etc.)
├── wenshu_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── wenshu_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.wenshu-hermes/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.wenshu-hermes/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.wenshu-hermes/`)

| Path | Purpose |
|------|---------|
| `~/.wenshu-hermes/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.wenshu-hermes/.env` | API keys and secrets |
| `~/.wenshu-hermes/auth.json` | OAuth credentials (Nous Portal) |
| `~/.wenshu-hermes/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.wenshu-hermes/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.wenshu-hermes/state.db` | SQLite session database |
| `~/.wenshu-hermes/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.wenshu-hermes/cron/` | Scheduled job data |
| `~/.wenshu-hermes/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`wenshu_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.wenshu-hermes/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_WENSHU_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.wenshu`:
...
The user may skip setup and keep loading the skill. Wenshu only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `wenshu setup` or update `~/.wenshu-hermes/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Wenshu tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Wenshu Agent" is the secondary collaborator. If the contributor's commit shows "Wenshu Agent" as author because they used Wenshu to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Wenshu tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `wenshu --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Wenshu uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.wenshu-hermes/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `wenshu_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `wenshu_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Wenshu runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the wenshu wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `wenshu_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `wenshu_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `wenshu_cli/gateway_windows.py::
...
Wenshu has terminal access. Security matters.
...
2. **Test manually**: Run `wenshu` and exercise the code path you changed
...
- Include: OS, Python version, Wenshu version (`wenshu version`), full error traceback
