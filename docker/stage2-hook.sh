...
#
# Wired into the image as /etc/cont-init.d/01-wenshu-setup by the
...
WENSHU_HOME="${WENSHU_HOME:-/opt/data}"
INSTALL_DIR="/opt/wenshu"

# Drop to wenshu via s6-setuidgid, but skip it when already non-root.
as_wenshu() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid wenshu "$@"; }
...
# starts non-root. The baked install tree under /opt/wenshu is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $WENSHU_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass WENSHU_UID/WENSHU_GID — or the PUID/PGID aliases — which the
...
# `id -u wenshu` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# wenshu UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u wenshu)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-wenshu UID).
...
and the baked /opt/wenshu install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e WENSHU_UID=\$(id -u) -e WENSHU_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the wenshu user to that UID/GID at boot and chowns the data
...
# --- Bootstrap WENSHU_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid wenshu mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom WENSHU_HOME paths whose parents only
# root can create (e.g. `WENSHU_HOME=/home/wenshu/.wenshu` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for WENSHU_UID/WENSHU_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  WENSHU_UID/
# WENSHU_GID still win when both are set.  See #15290, salvages #25872.
WENSHU_UID="${WENSHU_UID:-${PUID:-}}"
WENSHU_GID="${WENSHU_GID:-${PGID:-}}"

if [ -n "${WENSHU_UID:-}" ] && validate_uid_gid "$WENSHU_UID" && [ "$WENSHU_UID" != "$(id -u wenshu)" ]; then
    echo "[stage2] Changing wenshu UID to $WENSHU_UID"
    usermod -u "$WENSHU_UID" wenshu
fi
if [ -n "${WENSHU_GID:-}" ] && validate_uid_gid "$WENSHU_GID" && [ "$WENSHU_GID" != "$(id -g wenshu)" ]; then
    echo "[stage2] Changing wenshu GID to $WENSHU_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$WENSHU_GID" wenshu 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised wenshu user (UID 10000)
...
# wenshu process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes wenshu. Idempotent across container
...
#   - wenshu is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G wenshu 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] wenshu already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" wenshu 2>/dev/null; then
        echo "[stage2] Added wenshu to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group wenshu failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When WENSHU_UID is remapped or the top-level $WENSHU_HOME isn't owned by
# the runtime wenshu UID, restore ownership to wenshu — but ONLY for the
# directories wenshu actually writes to. The full $WENSHU_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of wenshu-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_wenshu_uid=$(id -u wenshu)
...
chown_wenshu_tree() {
    ...
    chown -R wenshu:wenshu "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$WENSHU_HOME" 2>/dev/null)" != "$actual_wenshu_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $WENSHU_HOME (targeted) to wenshu ($actual_wenshu_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $WENSHU_HOME: chown the directory itself (not its contents)
    # so wenshu can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$WENSHU_HOME"; then
        :
    else
        chown wenshu:wenshu "$WENSHU_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $WENSHU_HOME failed (rootless container?) — continuing"
    fi
    # Wenshu-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by wenshu (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$WENSHU_HOME/$sub" ]; then
            chown_wenshu_tree "$WENSHU_HOME/$sub"
        fi
    done
fi
...
# wenshu user. Hosted/container instances keep mutable state under
# $WENSHU_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# WENSHU_DISABLE_LAZY_INSTALLS=1. Keeping /opt/wenshu root-owned and
...
# $WENSHU_HOME/lazy-packages dir on the data volume (Dockerfile sets
# WENSHU_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to wenshu
...
# Always reset ownership of $WENSHU_HOME/profiles to wenshu on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> wenshu …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as wenshu and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$WENSHU_HOME/profiles" ]; then
    chown_wenshu_tree "$WENSHU_HOME/profiles"
fi

# Always reset ownership of $WENSHU_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged wenshu runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$WENSHU_HOME/cron" ]; then
    chown_wenshu_tree "$WENSHU_HOME/cron"
fi
...
# wenshu pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged wenshu gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $WENSHU_HOME is
...
# is negligible.
if [ -d "$WENSHU_HOME/platforms/pairing" ]; then
    chown_wenshu_tree "$WENSHU_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$WENSHU_HOME/pairing" ]; then
    chown_wenshu_tree "$WENSHU_HOME/pairing"
fi

# Reset ownership of wenshu-owned top-level state files on every boot.
# The targeted data-volume chown above only covers wenshu-owned
# *subdirectories*; loose state files living directly under $WENSHU_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> wenshu …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged wenshu runtime then hits
...
# sweep so host-owned files in a bind-mounted $WENSHU_HOME are never
...
# entries of wenshu_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$WENSHU_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$WENSHU_HOME/$f"; then
            :
        else
            chown wenshu:wenshu "$WENSHU_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the wenshu runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$WENSHU_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$WENSHU_HOME/config.yaml"; then
        :
    else
        chown wenshu:wenshu "$WENSHU_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$WENSHU_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as wenshu user ---
# Run as wenshu via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $WENSHU_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_wenshu mkdir -p \
    "$WENSHU_HOME/backups" \
    "$WENSHU_HOME/cron" \
    "$WENSHU_HOME/sessions" \
    "$WENSHU_HOME/logs" \
    "$WENSHU_HOME/logs/gateways" \
    "$WENSHU_HOME/hooks" \
    "$WENSHU_HOME/memories" \
    "$WENSHU_HOME/skills" \
    "$WENSHU_HOME/skins" \
    "$WENSHU_HOME/plans" \
    "$WENSHU_HOME/workspace" \
    "$WENSHU_HOME/home" \
    "$WENSHU_HOME/pairing" \
    "$WENSHU_HOME/platforms/pairing" \
    "$WENSHU_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/wenshu/.install_method (see Dockerfile), NOT written here into
# $WENSHU_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $WENSHU_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.wenshu:/opt/data) and sometimes shared with a
...
# 'wenshu update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $WENSHU_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/wenshu stamp, so deleting the data-dir copy is safe).
if [ -f "$WENSHU_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$WENSHU_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$WENSHU_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$WENSHU_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$WENSHU_HOME/$dest"; then
            :
        else
            as_wenshu cp "$INSTALL_DIR/$src" "$WENSHU_HOME/$dest"
        fi
    fi
}
...
if [ -f "$WENSHU_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$WENSHU_HOME/.env"; then
        :
    else
        chown wenshu:wenshu "$WENSHU_HOME/.env" 2>/dev/null || true
        chmod 600 "$WENSHU_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $WENSHU_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `wenshu update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set WENSHU_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$WENSHU_HOME/config.yaml" ]; then
    s6-setuidgid wenshu "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$WENSHU_HOME/auth.json" ] && [ -n "${WENSHU_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$WENSHU_HOME/auth.json"; then
        :
    else
        printf '%s' "$WENSHU_AUTH_JSON_BOOTSTRAP" > "$WENSHU_HOME/auth.json"
        chown wenshu:wenshu "$WENSHU_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$WENSHU_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via WENSHU_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$WENSHU_HOME/auth.json" ] && [ -n "${WENSHU_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$WENSHU_HOME/auth.json"; then
        :
    else
        s6-setuidgid wenshu "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$WENSHU_HOME/auth.json" \
...
# can set WENSHU_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# WENSHU_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$WENSHU_HOME/gateway_state.json" ] && \
        [ "${WENSHU_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$WENSHU_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$WENSHU_HOME/gateway_state.json"
        chown wenshu:wenshu "$WENSHU_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$WENSHU_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_wenshu "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/wenshu/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Wenshu spawns for the
...
# main-wrapper.sh propagates it into the supervised ``wenshu`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-wenshu, dashboard, etc.).
