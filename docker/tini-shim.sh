...
# Wenshu WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: WENSHU_TINI_SHIM_TARGET overrides the `/init` path (and
# WENSHU_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${WENSHU_TINI_SHIM_TARGET:-/init}"
WRAPPER="${WENSHU_TINI_SHIM_WRAPPER:-/opt/wenshu/docker/main-wrapper.sh}"
