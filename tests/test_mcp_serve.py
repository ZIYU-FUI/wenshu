"""
Tests for mcp_serve — Wenshu MCP server.
...
def _isolate_wenshu_home(tmp_path, monkeypatch):
    """Redirect WENSHU_HOME to a temp directory."""
    monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
    try:
        import wenshu_constants
        monkeypatch.setattr(wenshu_constants, "get_wenshu_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking wenshu_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <WENSHU_HOME>/state.db for its mtime gate; the autouse
        # fixture points WENSHU_HOME at tmp_path.
        db_path = tmp_path / "state.db"
