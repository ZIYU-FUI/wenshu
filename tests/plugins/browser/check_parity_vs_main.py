...
Run from the PR worktree:
    ...
    cd ~/.wenshu-hermes/wenshu-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.wenshu-hermes/wenshu-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.wenshu-hermes/wenshu-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a wenshu-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a wenshu-agent checkout"
)
...
# Isolated WENSHU_HOME for the config write.
