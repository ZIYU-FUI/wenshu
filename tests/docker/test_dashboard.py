"""Harness: dashboard opt-in via WENSHU_DASHBOARD.

Today (tini): dashboard starts once when WENSHU_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``wenshu`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without WENSHU_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "wenshu dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without WENSHU_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without WENSHU_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `wenshu doctor` and any other health check).
    ...
    WENSHU_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without WENSHU_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with WENSHU_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # WENSHU_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "WENSHU_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With WENSHU_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'wenshu dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with WENSHU_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """WENSHU_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "wenshu dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as wenshu, so the
    # wenshu user can kill it (same UID).
...
# whenever `WENSHU_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``WENSHU_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        built_image, container_name,
        "WENSHU_DASHBOARD=1",
        "WENSHU_DASHBOARD_HOST=0.0.0.0",
        "WENSHU_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"WENSHU_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and WENSHU_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``WENSHU_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
