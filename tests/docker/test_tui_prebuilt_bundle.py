...
only a subset of the npm monorepo workspaces (root/web/legacy-terminal-ui, never apps/*),
so the actualized node_modules permanently disagrees with the canonical
package-lock.json. Without WENSHU_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV WENSHU_TUI_DIR=/opt/wenshu/legacy-terminal-ui`` in the Dockerfile, which
...
pytest.skip("legacy terminal UI image contract removed", allow_module_level=True)


def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the wenshu user, return stdout."""
    inner = (
        "source /opt/wenshu/.venv/bin/activate && "
        "cd /opt/wenshu && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the wenshu user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "wenshu", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_wenshu_tui_dir_env_is_set(built_image: str) -> None:
    """WENSHU_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$WENSHU_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/wenshu/legacy-terminal-ui", (
        f"WENSHU_TUI_DIR={r.stdout.strip()!r} (expected /opt/wenshu/legacy-terminal-ui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from wenshu_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/wenshu/legacy-terminal-ui')\n"
    ...
    assert out["dist_entry_exists"], "prebuilt legacy-terminal-ui/dist/entry.js missing from image"
    # With WENSHU_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
