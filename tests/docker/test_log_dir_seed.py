"""Runtime smoke test for Docker $WENSHU_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the wenshu user after container boot.
...
registered later runs its log service as the dropped wenshu user and
...
def test_logs_gateways_seeded_and_wenshu_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by wenshu after boot."""
    ...
    assert "logs=wenshu" in r.stdout, (
        f"logs/ not owned by wenshu: {r.stdout}"
    )
    assert "gateways=wenshu" in r.stdout, (
        f"logs/gateways/ not owned by wenshu: {r.stdout}"
    )
