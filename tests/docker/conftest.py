...
Override the image with ``WENSHU_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("WENSHU_TEST_IMAGE", "wenshu-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``WENSHU_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("WENSHU_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged wenshu user
# ---------------------------------------------------------------------------
#
# Background: every Wenshu runtime path inside the container drops to UID
# 10000 (the ``wenshu`` user) via ``s6-setuidgid wenshu``. ``docker exec``
...
# fails (PermissionError swallowed) for wenshu, so a test that ran as root
...
# helpers below run every probe under ``-u wenshu`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: wenshu).
