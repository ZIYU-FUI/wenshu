...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.wenshu-hermes/wenshu-agent/utils.py)
...
2. ``wenshu update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``wenshu_cli/main.py`` (the ``wenshu update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``wenshu_constants`` after the
