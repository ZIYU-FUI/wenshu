...
Uses a throwaway WENSHU_HOME so it never touches ~/.wenshu. No real names.
"""
...
def _fresh_home():
    """Point WENSHU_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_wenshu_home() at import time)."""
...
# Import AFTER WENSHU_HOME is set.
...
# a different get_wenshu_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"WENSHU_HOME (throwaway): {HOME}")
