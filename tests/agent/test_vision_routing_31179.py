...
def isolated_home(monkeypatch):
    """Temp WENSHU_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="wenshu_test_31179_")
    wenshu_home = os.path.join(test_home, ".wenshu-hermes")
    os.makedirs(wenshu_home)
...
def _fresh_modules():
    """Drop cached wenshu modules so each test reloads against current env."""
