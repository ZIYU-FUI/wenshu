...
@pytest.fixture
def wenshu_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.wenshu-hermes/`` with a config.yaml referencing one external dir."""
...
def test_returns_configured_external_dir(wenshu_home_with_config):
    _home, external, _cfg = wenshu_home_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(wenshu_home_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = wenshu_home_with_config
...
def test_cache_invalidates_on_mtime_change(wenshu_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = wenshu_home_with_config
...
def test_returned_list_is_a_copy(wenshu_home_with_config):
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different WENSHU_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".wenshu-hermes"
    ...
    home_b = tmp_path / "home_b" / ".wenshu-hermes"
    ...
    monkeypatch.setenv("WENSHU_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("WENSHU_HOME", str(home_b))
