...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Wenshu passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("WENSHU_HOME", "/users/alice/.wenshu-hermes/profiles/backend-worker")
        monkeypatch.setenv("WENSHU_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "WENSHU_KANBAN_DB",
            "/users/alice/.wenshu-hermes/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.wenshu-hermes/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through wenshu_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Wenshu secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "WENSHU_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "WENSHU_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through wenshu_subprocess_env must not
