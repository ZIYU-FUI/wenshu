...
Damien's incident (Discord, 2026-05-28): a long Wenshu session in a Discord
...
def _make_legacy_session_db_class() -> type:
    ...
    imports the same still-loaded ``wenshu_state`` module, whose ``SessionDB``
    ...
    namespace = {"__name__": "wenshu_state"}
    source = '''
class SessionDB:
...
_NominalSessionDBImpostor.__module__ = "wenshu_state"
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path, monkeypatch) -> None:
    ...
    ``wenshu_state.SessionDB`` class remains pre-lock. The compatibility path is
    ...
    real_session_db_type = wenshu_state.SessionDB
    monkeypatch.setattr(wenshu_state, "SessionDB", legacy_type)
    try:
        ...
        monkeypatch.setattr(wenshu_state, "SessionDB", real_session_db_type)
