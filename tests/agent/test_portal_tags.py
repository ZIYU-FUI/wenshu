...
def test_wenshu_client_tag_includes_current_version():
    """The client tag must reflect wenshu_cli.__version__ verbatim."""
    from wenshu_cli import __version__
    from agent.portal_tags import wenshu_client_tag

    assert wenshu_client_tag() == f"client=wenshu-client-v{__version__}"


def test_wenshu_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import wenshu_client_tag

    tag = wenshu_client_tag()
    assert tag.startswith("client=wenshu-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import wenshu_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=wenshu-agent" in tags
    assert wenshu_client_tag() in tags
    assert len(tags) == 2
