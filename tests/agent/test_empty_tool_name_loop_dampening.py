...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated WENSHU_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="wenshu_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".wenshu-hermes"))
    prev_home = os.environ.get("WENSHU_HOME")
    os.environ["WENSHU_HOME"] = os.path.join(test_home, ".wenshu-hermes")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("wenshu_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["WENSHU_HOME"] = prev_home
