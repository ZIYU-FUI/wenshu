...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_wenshu_env(self):
        """Wenshu' own .env inside WENSHU_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: wenshu internal .env is in ~/.wenshu-hermes/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.wenshu-hermes/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.wenshu-hermes/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Wenshu cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        cache = wenshu_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._wenshu_home_path", return_value=wenshu_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Wenshu cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        hub = wenshu_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_wenshu_home(self, tmp_path):
        """The env basename guard does not depend on WENSHU_HOME resolution."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        cache = wenshu_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
