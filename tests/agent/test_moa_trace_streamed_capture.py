...
path via ``wenshu chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp WENSHU_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point WENSHU_HOME at a temp dir and turn moa.save_traces on."""
    wenshu_home = tmp_path / ".wenshu-hermes"
    wenshu_home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))

    # save_moa_turn reads config via wenshu_cli.config.load_config; stub it to
