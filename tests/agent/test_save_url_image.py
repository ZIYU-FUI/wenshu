...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate WENSHU_HOME under tmp_path."""
    monkeypatch.setenv("WENSHU_HOME", str(tmp_path / ".wenshu-hermes"))
    (tmp_path / ".wenshu-hermes").mkdir()

    # Force the constants/image cache helpers to re-read WENSHU_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("wenshu_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_wenshu_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under WENSHU_HOME — gateway cleanup
