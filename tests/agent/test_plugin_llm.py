...
""",
        ...
        from wenshu_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        (wenshu_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
        from wenshu_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from wenshu_cli.plugins import PluginContext, PluginManifest, PluginManager
