...
The ``wenshu update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m wenshu_cli.main gateway run`` — image name
    ``pythonw``, not ``wenshu.exe`` — so the ``taskkill /IM wenshu.exe`` guard
    ...
    # The wenshu.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM wenshu.exe' in text
