"""Tests for IRC gateway configuration via `wenshu setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Wenshu install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Wenshu install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#wenshu")
            monkeypatch.setenv("IRC_NICKNAME", "wenshu-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `wenshu setup gateway` experience with IRC present."""
