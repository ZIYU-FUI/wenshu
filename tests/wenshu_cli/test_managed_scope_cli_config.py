...
wenshu_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `wenshu config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_wenshu_home = get_wenshu_home()`` at import time (module
    singleton), so monkeypatching WENSHU_HOME after import doesn't move it.
    ...
    this only matters for tests that swap WENSHU_HOME.)
    """
    import cli

    cli._wenshu_home = home
