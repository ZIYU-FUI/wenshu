"""Regression test: ``wenshu mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``wenshu_cli/_parser.py``).  The dispatcher in ``wenshu_cli/main.py``
...
result, ``wenshu mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the wenshu parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`wenshu mcp add foo --url ...` must keep args.command == "mcp".
