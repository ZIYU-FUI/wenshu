...
the SAME provider universe as ``wenshu model`` (the CLI/TUI picker).
...
def test_every_wenshu_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `wenshu model` universe."""
    ...
    assert not missing, (
        "providers shown in `wenshu model` but not configurable in the desktop "
