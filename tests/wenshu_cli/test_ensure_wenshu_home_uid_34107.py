"""Regression tests for #34107 — Docker UID/GID handling in ensure_wenshu_home.

When Wenshu runs in Docker with ``WENSHU_UID=1000`` / ``WENSHU_GID=911``,
the entrypoint chowns the top-level ``WENSHU_HOME`` once at startup. But
...
The fix is a ``_chown_to_wenshu_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_wenshu_uid_gid
# ---------------------------------------------------------------------------


class TestResolveWenshuUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("WENSHU_UID", "1000")
        monkeypatch.setenv("WENSHU_GID", "911")
        from wenshu_cli.config import _resolve_wenshu_uid_gid
        uid, gid = _resolve_wenshu_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("WENSHU_UID", raising=False)
        monkeypatch.delenv("WENSHU_GID", raising=False)
        from wenshu_cli.config import _resolve_wenshu_uid_gid
        uid, gid = _resolve_wenshu_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("WENSHU_UID", "not-a-number")
        monkeypatch.setenv("WENSHU_GID", "911")
        from wenshu_cli.config import _resolve_wenshu_uid_gid
        uid, gid = _resolve_wenshu_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("WENSHU_UID", "")
        monkeypatch.setenv("WENSHU_GID", "")
        from wenshu_cli.config import _resolve_wenshu_uid_gid
        uid, gid = _resolve_wenshu_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_wenshu_uid
# ---------------------------------------------------------------------------


class TestChownToWenshuUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_wenshu_uid(d)  # must not raise
