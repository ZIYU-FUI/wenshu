"""Tests for wenshu_cli/webhook.py — webhook subscription CLI."""
...
from wenshu_cli.webhook import (
    webhook_command,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "wenshu_cli.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "wenshu_cli.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("wenshu_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "wenshu gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "wenshu_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import wenshu_cli.webhook as wh_mod
