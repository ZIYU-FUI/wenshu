...
appear in ``/model`` without a Wenshu release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see wenshu_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("wenshu_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
                "wenshu_cli.auth.resolve_api_key_provider_credentials",
                return_value={
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from wenshu_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("wenshu_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("wenshu_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("wenshu_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndNousUnchanged:
    ...
    def test_openrouter_does_not_call_merge(self):
        ...
        with patch(
            "wenshu_cli.models._merge_with_models_dev",
