...
def test_config_get_unset_subcommands_parse():
    """`wenshu config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="wenshu")
    sub = parser.add_subparsers(dest="command")
    handler = _h("config")
...
# ── deprecated `wenshu login` fails gracefully, not with argparse error ────
#
# `wenshu login` is a removed command; its handler (`login_command` in
# `wenshu_cli/auth.py`) prints a deprecation notice pointing at `wenshu auth` /
# `wenshu model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `wenshu --help`.
