...
from wenshu_cli import gateway as gateway_cli
...
def test_positive_watchdog_config_generates_notify_system_unit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda _user: ("wenshu", "wenshu", str(tmp_path)),
    )

    unit = gateway_cli.generate_systemd_unit(system=True, run_as_user="wenshu")
...
def test_system_unit_reads_watchdog_from_target_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WENSHU_HOME", str(caller_home))
    monkeypatch.setattr(
        gateway_cli,
        "_system_service_identity",
...
def test_managed_watchdog_override_controls_generated_unit(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
    monkeypatch.setenv("WENSHU_MANAGED_DIR", str(managed_home))
