...
profile switcher can target any profile's WENSHU_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via WENSHU_HOME.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_wenshu_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_wenshu_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import wenshu_cli.mcp_config as mcp_config
        from wenshu_constants import get_wenshu_home
...
class TestProfileScopedModel:
    ...
    def test_model_options_hides_unconfigured_providers_by_default(self, client, monkeypatch):
        ...
        monkeypatch.setattr(
            "wenshu_cli.inventory.load_picker_context",
            lambda: object(),
        )
        ...
        monkeypatch.setattr(
            "wenshu_cli.inventory.build_models_payload",
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same WENSHU_HOME the rest
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_wenshu_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "wenshu_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_wenshu_home()))
            return None
        ...
        assert resp.json()["wenshu_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_wenshu_home(self, isolated_profiles, monkeypatch):
        import wenshu_cli.web_server as web_server

        monkeypatch.setattr(
            "wenshu_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["WENSHU_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "WENSHU_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("WENSHU_HOME") != str(isolated_profiles["worker_beta"])
