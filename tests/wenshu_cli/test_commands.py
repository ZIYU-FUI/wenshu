...
from wenshu_cli.commands import (
    COMMAND_REGISTRY,
    COMMANDS,
...
class TestSlackNativeSlashes:
    ...
    def test_wenshu_catchall_is_first(self):
        """``/wenshu`` must be reserved as the first slot so the legacy
        ``/wenshu <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "wenshu"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /wenshu <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/wenshu <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /wenshu <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_wenshu_norm = {_norm(n) for n in _SLACK_VIA_WENSHU_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_wenshu_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `wenshu slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from wenshu_cli import commands as commands_mod

        # `web` is enabled, `kanban` is disabled — enabling should only offer
        # the disabled ones.
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "wenshu_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
    ...
    def test_tools_enable_partial_filters(self, monkeypatch):
        ...
        completions = _completions(SlashCommandCompleter(), "/tools enable ka")
        texts = {c.text for c in completions}
        assert texts == {"kanban"}
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        completions = _completions(SlashCommandCompleter(), "/tools enable kanban ")
        texts = {c.text for c in completions}
        assert "kanban" not in texts
...
from wenshu_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch wenshu_cli.plugins.get_plugin_commands() to a fixed dict."""
        from wenshu_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/wenshu metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
