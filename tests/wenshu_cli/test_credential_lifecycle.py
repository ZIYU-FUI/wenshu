...
WENSHU_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def wenshu_home(monkeypatch, tmp_path):
    """Fresh WENSHU_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
    from wenshu_cli.config import invalidate_env_cache
...
def test_delete_env_key_prunes_env_seeded_pool_entry(wenshu_home):
    _write_env(wenshu_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(wenshu_home, _zai_pool_fixture())
...
def test_delete_env_key_removes_provider_pool_key_when_emptied(wenshu_home):
...
def test_delete_survives_pool_reload(wenshu_home):
    """#59761: the pool loader must not resurrect the entry after 'restart'."""
    _write_env(wenshu_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(wenshu_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
...
def test_delete_clears_provider_models_cache(wenshu_home):
    _write_env(wenshu_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(wenshu_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = wenshu_home / "provider_models_cache.json"
...
def test_delete_unknown_key_404s(wenshu_home):
...
def test_delete_does_not_touch_other_providers(wenshu_home):
    _write_env(wenshu_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    other_key = "dk-" + "e" * 24
...
def test_update_rotates_config_yaml_model_mirror(wenshu_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(wenshu_home, OPENAI_API_KEY=old)
    _write_config(
        wenshu_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = wenshu_home.joinpath("config.yaml").read_text(encoding="utf-8")
...
def test_update_rotates_custom_provider_mirror(wenshu_home):
    old = "sk-cp-" + "h" * 24
    new = "sk-cp-" + "i" * 24
    _write_env(wenshu_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_update_leaves_unrelated_config_keys_alone(wenshu_home):
    """A DIFFERENT key configured inline must not be rewritten by value-match."""
    old = "sk-un-" + "j" * 24
    unrelated = "sk-un-" + "k" * 24
    _write_env(wenshu_home, OPENAI_API_KEY=old)
    _write_config(wenshu_home, f"model:\n  provider: custom\n  api_key: {unrelated}\n")
...
def test_delete_scrubs_config_yaml_mirror(wenshu_home):
    old = "sk-dl-" + "m" * 24
    _write_env(wenshu_home, OPENAI_API_KEY=old)
    _write_config(wenshu_home, f"model:\n  provider: custom\n  api_key: {old}\n")
...
def test_delete_then_resave_round_trip(wenshu_home):
    _write_env(wenshu_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(wenshu_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    ...
    assert not is_source_suppressed("zai", "env:ZAI_API_KEY"), (
        "an explicit re-save must lift the suppression (like `wenshu auth add`)"
    )
