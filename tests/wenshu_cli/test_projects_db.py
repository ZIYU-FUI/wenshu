"""Tests for the per-profile Projects store (wenshu_cli/projects_db)."""
...
from wenshu_cli import projects_db as pdb
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Wenshu Agent", folders=["/tmp/wenshu"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "wenshu-agent"
    assert proj.name == "Wenshu Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/wenshu"
    assert [f.path for f in proj.folders] == ["/tmp/wenshu"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "wenshu-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Wenshu Agent")
    pdb.create_project(conn, name="Wenshu Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["wenshu-agent", "wenshu-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' WENSHU_HOME.
...
def test_db_path_under_wenshu_home():
    # Resolves under WENSHU_HOME (set by the autouse isolation fixture).
