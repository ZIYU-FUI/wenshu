"""Unit tests for wenshu_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Wenshu.
"""
...
# A representative set of real toolset names. `wenshu` is deliberately absent —
# that is the corruption #38798 reported (`wenshu-cli` rewritten to `wenshu`).
_KNOWN = {
    "wenshu-cli",
    "wenshu-telegram",
    "wenshu-discord",
...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["wenshu-cli"], "telegram": ["wenshu-telegram"]}
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'wenshu' instead of 'wenshu-cli'.
    warnings = validate_platform_toolsets({"cli": ["wenshu"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'wenshu'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'wenshu-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["wenshu-cli"], "discord": ["bogus"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # wenshu-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "wenshu-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "wenshu-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "wenshu-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["wenshu"], "discord": ["wenshu"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_wenshu_cli_valid_and_wenshu_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `wenshu-cli` is the real toolset and `wenshu` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("wenshu-cli") is True
    assert validate_toolset("wenshu") is False
    warnings = validate_platform_toolsets({"cli": ["wenshu"]}, validate_toolset)
    assert any("did you mean 'wenshu-cli'?" in w for w in warnings)
