...
def wenshu_home(tmp_path, monkeypatch):
    ...
    # /etc/wenshu on the dev/CI box can't influence the test.
    monkeypatch.setenv("WENSHU_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(wenshu_home, monkeypatch):
    from wenshu_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(wenshu_home, monkeypatch):
    from wenshu_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(wenshu_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_wenshu_dotenv(wenshu_home=str(home))
