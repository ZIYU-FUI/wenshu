...
Regression guard for the bug where `wenshu model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`wenshu_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageResolver:
    ...
    def test_solar_alias_normalizes_to_upstage(self):
        from wenshu_cli.providers import normalize_provider, resolve_provider_full
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in WENSHU_OVERLAYS
        overlay = WENSHU_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `wenshu_cli/main.py` (cmd_model): read
