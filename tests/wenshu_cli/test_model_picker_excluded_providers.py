...
interactive ``wenshu model`` CLI picker.

The CLI picker (``wenshu_cli.main.select_provider_and_model``) builds its
...
def test_cli_picker_hides_excluded_provider(config_home):
    ...
    from the ``wenshu model`` provider menu."""
...
def test_cli_picker_hides_excluded_provider_by_alias(config_home):
    ...
    wenshu_id / alias names."""
    # 'openai' is an alias-style wenshu id; ensure excluding it hides the
    # canonical openai provider row if present. Use the canonical slug's
    # alias from _PROVIDER_ALIASES to stay robust to renames.
    from wenshu_cli.models import _PROVIDER_ALIASES, CANONICAL_PROVIDERS
