"""Tests for the top-level `./wenshu` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./wenshu` should use `wenshu_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "wenshu"
    called = []

    fake_main_module = types.ModuleType("wenshu_cli.main")

    def fake_main():
        called.append("wenshu_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "wenshu_cli.main", fake_main_module)
