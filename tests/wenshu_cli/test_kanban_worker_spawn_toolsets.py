...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    WENSHU_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_wenshu_argv", lambda: ["wenshu"])
    ...
    assert pid == 4242
    assert captured["env"]["WENSHU_HOME"] == str(profile)
    assert captured["env"]["WENSHU_KANBAN_TASK"] == "t_spawn_tools"
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited WENSHU_TUI=1 (or a TUI-default
    ...
    flag) and strips WENSHU_TUI from the child env."""
    root = tmp_path / ".wenshu-hermes"
    ...
    assert "WENSHU_TUI" not in captured["env"]
