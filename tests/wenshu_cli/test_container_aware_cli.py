...
def container_env(tmp_path, monkeypatch):
    """Set up a fake WENSHU_HOME with .container-mode file."""
    wenshu_home = tmp_path / ".wenshu-hermes"
    wenshu_home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
    monkeypatch.delenv("WENSHU_DEV", raising=False)

    container_mode = wenshu_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=wenshu-agent\n"
        "exec_user=wenshu\n"
        "wenshu_bin=/data/current-package/bin/wenshu\n"
    )
    return wenshu_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("wenshu_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "wenshu-agent"
    assert info["exec_user"] == "wenshu"
    assert info["wenshu_bin"] == "/data/current-package/bin/wenshu"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("wenshu_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_wenshu_dev(container_env, monkeypatch):
    """Returns None when WENSHU_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("WENSHU_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_wenshu_dev_zero(container_env, monkeypatch):
    """WENSHU_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("WENSHU_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        wenshu_home = Path(tmpdir) / ".wenshu-hermes"
        wenshu_home.mkdir()
        (wenshu_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("wenshu_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_wenshu_home": lambda: wenshu_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("WENSHU_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=wenshu-custom\n"
        "exec_user=myuser\n"
        "wenshu_bin=/opt/wenshu/bin/wenshu\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "wenshu-custom"
    assert info["exec_user"] == "myuser"
    assert info["wenshu_bin"] == "/opt/wenshu/bin/wenshu"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from wenshu_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "wenshu"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "wenshu-agent" in cmd
    assert "/data/current-package/bin/wenshu" in cmd
    assert "chat" in cmd
