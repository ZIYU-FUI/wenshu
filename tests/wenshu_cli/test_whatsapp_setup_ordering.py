...
Before the fix, ``wenshu whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``wenshu gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    wenshu = home / ".wenshu-hermes"
    wenshu.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("WENSHU_HOME", str(wenshu))
...
def _env_value(wenshu_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from wenshu_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("wenshu_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``wenshu whatsapp`` with an existing paired session and
