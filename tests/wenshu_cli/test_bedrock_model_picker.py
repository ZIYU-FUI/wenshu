...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (WENSHU_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via WENSHU_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in WENSHU_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from wenshu_cli.providers import WENSHU_OVERLAYS
        assert "bedrock" in WENSHU_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from wenshu_cli.providers import WENSHU_OVERLAYS
        assert WENSHU_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from wenshu_cli.providers import WENSHU_OVERLAYS
        assert WENSHU_OVERLAYS["bedrock"].auth_type == "aws_sdk"
