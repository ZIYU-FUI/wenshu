...
``wenshu_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``wenshu --help``, ``wenshu version``,
``wenshu logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``wenshu --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from wenshu_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["wenshu", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["wenshu", "-w", "--tui"], None),
        (["wenshu", "version"], "version"),
        (["wenshu", "--tui", "chat"], "chat"),
        (["wenshu", "-w", "logs"], "logs"),
        (["wenshu", "chat", "hello world"], "chat"),
        (["wenshu", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["wenshu", "-m", "gpt5", "chat"], "chat"),
        (["wenshu", "--model", "gpt5", "chat", "hi"], "chat"),
        (["wenshu", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["wenshu", "-z", "hello world"], None),
        (["wenshu", "-z", "hello", "chat"], "chat"),
        (["wenshu", "--model=gpt5", "chat"], "chat"),     # inline form
        (["wenshu", "--", "chat"], "chat"),               # -- terminator
        (["wenshu", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["wenshu", "some-plugin-cmd"], "some-plugin-cmd"),
        (["wenshu", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["wenshu"],                          # bare → chat
        ["wenshu", "--help"],                # top-level help
        ["wenshu", "-h"],
        ["wenshu", "version"],               # known built-in
        ["wenshu", "logs"],
        ["wenshu", "gateway", "run"],
        ["wenshu", "--tui"],
        ["wenshu", "-w", "--tui"],
        ["wenshu", "chat", "hi"],
        ["wenshu", "help"],                  # accepted built-in-ish
        ["wenshu", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``wenshu help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"wenshu_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
