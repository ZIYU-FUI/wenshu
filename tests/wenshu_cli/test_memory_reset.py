"""Tests for the `wenshu memory reset` CLI command.
...
- Profile-scoped reset (uses WENSHU_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake WENSHU_HOME with memory files."""
    wenshu_home = tmp_path / ".wenshu-hermes"
    memories = wenshu_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nWenshu repo is at ~/.wenshu-hermes/wenshu-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `wenshu memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        (wenshu_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's WENSHU_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_wenshu_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
