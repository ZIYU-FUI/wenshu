...
binary download are exercised manually by `wenshu secrets bitwarden
...
def wenshu_home(tmp_path, monkeypatch):
    """Point Wenshu at an isolated home directory."""
    home = tmp_path / ".wenshu-hermes"
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
    # Some modules cache get_wenshu_home; clear if needed.
    import wenshu_constants
    if hasattr(wenshu_constants, "_WENSHU_HOME_CACHE"):
        wenshu_constants._WENSHU_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(wenshu_home, monkeypatch):
...
def test_install_bws_happy_path(wenshu_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(wenshu_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(wenshu_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from wenshu_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under wenshu_home/cache/."""
    home = tmp_path / ".wenshu-hermes"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
