...
from wenshu_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(wenshu_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <wenshu_home>/plugins/<name> and opt it in."""
    plugin_dir = wenshu_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("WENSHU_HOME", str(tmp_path / "wenshu_empty"))
