"""Tests for wenshu_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Wenshu entry point
(wenshu, wenshu-agent, wenshu-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Wenshu entry point imports wenshu_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported wenshu_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("wenshu_bootstrap", None)
    import wenshu_bootstrap  # noqa: WPS433
    return wenshu_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Wenshu CLI
...
class TestEntryPointsImportBootstrap:
    """Every Wenshu entry point must import wenshu_bootstrap as its
    ...
    # Entry points that invoke Wenshu as a process.  Each one must
    # import wenshu_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "wenshu_cli/main.py",   # wenshu CLI (console_script)
        "run_agent.py",          # wenshu-agent (console_script)
        "acp_adapter/entry.py",  # wenshu-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import wenshu_bootstrap' and that
        ...
        half-finished ``wenshu update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``wenshu_bootstrap`` as a top-level module) leaves wenshu
        ...
        """
        # Resolve relative to the wenshu-agent repo root.  Tests live
        # at tests/test_wenshu_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # wenshu start even when wenshu_bootstrap hasn't been
        ...
        assert first_import_name == "wenshu_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'wenshu_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import wenshu_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Wenshu's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("WENSHU_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["WENSHU_PYTHON_SRC_ROOT"] = env
            elif "WENSHU_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["WENSHU_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/wenshu")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("WENSHU_PYTHON_SRC_ROOT", None)
            else:
                os.environ["WENSHU_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/wenshu", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Wenshu root.  The guard must relocate Wenshu to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/wenshu"])
        assert result[0] == "/opt/wenshu"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Wenshu root.
        assert result.index("/opt/wenshu") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/wenshu", "/opt/wenshu", ""])
        assert result.count("/opt/wenshu") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["WENSHU_PYTHON_SRC_ROOT"] = "/env/wenshu"
            hb.harden_import_path()
            assert sys.path[0] == "/env/wenshu"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("WENSHU_PYTHON_SRC_ROOT", None)
