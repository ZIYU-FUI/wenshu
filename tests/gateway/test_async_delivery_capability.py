...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("WENSHU_SESSION_PLATFORM") == "api_server"
        finally:
            clear_session_vars(tokens)
...
class TestDeclareStatelessChannel:
    """``wenshu -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``WENSHU_SESSION_*`` routing keys, so an async delegation's
