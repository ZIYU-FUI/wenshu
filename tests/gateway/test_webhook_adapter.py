...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under WENSHU_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.wenshu-hermes/data/watchlist.json",
                            },
                            {
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_wenshu_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.wenshu-hermes/scripts paths must resolve through WENSHU_HOME for profiles."""
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.wenshu-hermes/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
