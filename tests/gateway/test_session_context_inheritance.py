...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``WENSHU_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "WENSHU_SESSION_CHAT_ID": env.get("WENSHU_SESSION_CHAT_ID"),
        "WENSHU_SESSION_THREAD_ID": env.get("WENSHU_SESSION_THREAD_ID"),
        "WENSHU_SESSION_KEY": env.get("WENSHU_SESSION_KEY"),
    }
...
def test_child_task_inherits_foreign_session_without_reset():
    ...
    assert captured["window"]["WENSHU_SESSION_CHAT_ID"] == "MINE_CHAT", (
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("WENSHU_SESSION_CHAT_ID", "WENSHU_SESSION_THREAD_ID", "WENSHU_SESSION_KEY"):
    ...
    assert captured["bound"]["WENSHU_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["WENSHU_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``WENSHU_SESSION_*``
