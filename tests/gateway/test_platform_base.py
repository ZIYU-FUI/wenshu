...
class TestSecretCaptureGuidance:
    def test_gateway_secret_capture_message_points_to_local_setup(self):
        ...
        assert "~/.wenshu-hermes/.env" in message
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.wenshu-hermes/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("WENSHU_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("WENSHU_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("WENSHU_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        self._patch_roots(monkeypatch)
        monkeypatch.setenv("WENSHU_KANBAN_HOME", str(tmp_path / "wenshu"))
        monkeypatch.setenv("WENSHU_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "wenshu" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("WENSHU_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("WENSHU_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("WENSHU_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("WENSHU_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("WENSHU_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("WENSHU_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.wenshu-hermes/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_wenshu_credentials(self, tmp_path, monkeypatch):
        """~/.wenshu-hermes/.env and ~/.wenshu-hermes/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        wenshu_dir = fake_home / ".wenshu-hermes"
        wenshu_dir.mkdir(parents=True)
        env_file = wenshu_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._WENSHU_HOME",
            wenshu_dir,
        )
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.wenshu-hermes/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        wenshu_dir = fake_home / ".wenshu-hermes"
        (wenshu_dir / "mcp-tokens").mkdir(parents=True)
        secret = wenshu_dir / rel
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._WENSHU_ROOT",
            wenshu_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(secret)) is None

    def test_denylist_blocks_wenshu_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        wenshu_dir = fake_home / ".wenshu-hermes"
        wenshu_dir.mkdir(parents=True)
        config_file = wenshu_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_wenshu_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Wenshu root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".wenshu-hermes" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        wenshu_root = fake_home / ".wenshu-hermes"
        config_file = wenshu_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the WENSHU_HOME root (google_token.json)
        ...
        wenshu_dir = fake_home / ".wenshu-hermes"
        wenshu_dir.mkdir(parents=True)
        token = wenshu_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._WENSHU_HOME", wenshu_dir)
        monkeypatch.setattr("gateway.platforms.base._WENSHU_ROOT", wenshu_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.wenshu-hermes/pairing/ (platform pairing tokens) are
    ...
    def test_wenshu_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        wenshu_dir = fake_home / ".wenshu-hermes"
        cache_dir = wenshu_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_wenshu_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.wenshu
        ...
        wenshu_dir = fake_home / ".wenshu-hermes"
        wenshu_dir.mkdir(parents=True)
        artifact = wenshu_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting WENSHU_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``WENSHU_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("WENSHU_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_wenshu_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.wenshu-hermes/.env`` stays blocked under the $HOME exception — it is a
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose WENSHU_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.wenshu) emits
        ...
        wenshu_root = denied_root / ".wenshu-hermes"
        prof_cache = wenshu_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._WENSHU_ROOT", wenshu_root
        )
    ...
    def test_profile_scoped_credential_still_blocked_under_root(self, tmp_path, monkeypatch):
        ...
        denied_root = tmp_path / "root"
        wenshu_root = denied_root / ".wenshu-hermes"
        prof_dir = wenshu_root / "profiles" / "myprof"
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"WENSHU_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("WENSHU_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"WENSHU_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "WENSHU_HUMAN_DELAY_MODE": "natural",
            "WENSHU_HUMAN_DELAY_MIN_MS": "oops",
            "WENSHU_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "WENSHU_HUMAN_DELAY_MODE": "custom",
            "WENSHU_HUMAN_DELAY_MIN_MS": "100",
            "WENSHU_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"WENSHU_MEDIA_DELIVERY_STRICT": "1",
                                     "WENSHU_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Wenshu
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../wenshu/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.wenshu-hermes/cache/media/sensitive_host_path_abc123.bin"
