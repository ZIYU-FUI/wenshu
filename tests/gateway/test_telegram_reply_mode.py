...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        wenshu_home.mkdir()
        (wenshu_home / "config.yaml").write_text(content, encoding="utf-8")
        return wenshu_home
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        wenshu_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
    ...
    def test_extra_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        wenshu_home = self._write_config(
            tmp_path, "telegram:\n  extra:\n    reply_to_mode: \"off\"\n"
        )
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
        monkeypatch.delenv("TELEGRAM_REPLY_TO_MODE", raising=False)
...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Wenshu-created DM topic
