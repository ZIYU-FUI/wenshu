...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_wenshu_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <WENSHU_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``wenshu pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <WENSHU_HOME>/profiles/yangyang/pairing/."""
        from wenshu_constants import get_wenshu_home
        monkeypatch.setattr("wenshu_constants.get_wenshu_home", lambda: tmp_path)
