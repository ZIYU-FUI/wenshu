"""Shared fixtures for the wenshu-agent test suite.
...
2. **Isolated WENSHU_HOME.** WENSHU_HOME points to a per-test tempdir so
   code reading ``~/.wenshu-hermes/*`` via ``get_wenshu_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".wenshu-hermes"`` instead of the canonical
   ``get_wenshu_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No WENSHU_SESSION_* inheritance** — the agent's current gateway
...
# WENSHU_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_WENSHU_BEHAVIORAL_VARS = frozenset({
    "WENSHU_YOLO_MODE",
    "WENSHU_INTERACTIVE",
    "WENSHU_QUIET",
    "WENSHU_TOOL_PROGRESS",
    "WENSHU_TOOL_PROGRESS_MODE",
    "WENSHU_MAX_ITERATIONS",
    "WENSHU_SESSION_PLATFORM",
    "WENSHU_SESSION_CHAT_ID",
    "WENSHU_SESSION_CHAT_NAME",
    "WENSHU_SESSION_THREAD_ID",
    "WENSHU_SESSION_SOURCE",
    "WENSHU_SESSION_KEY",
    "WENSHU_GATEWAY_SESSION",
    "WENSHU_CRON_SESSION",
    "_WENSHU_GATEWAY",
    "WENSHU_PLATFORM",
    "WENSHU_MODEL",
    "WENSHU_INFERENCE_MODEL",
    "WENSHU_INFERENCE_PROVIDER",
    "WENSHU_TUI_PROVIDER",
    "WENSHU_MANAGED",
    "WENSHU_MANAGED_DIR",
    "WENSHU_DEV",
    ...
    "WENSHU_TIMEZONE",
    "WENSHU_REDACT_SECRETS",
    "WENSHU_BACKGROUND_NOTIFICATIONS",
    "WENSHU_EXEC_ASK",
    "WENSHU_HOME_MODE",
    "WENSHU_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.wenshu-hermes/kanban.db instead of the per-test WENSHU_HOME.
    "WENSHU_KANBAN_DB",
    "WENSHU_KANBAN_BOARD",
    "WENSHU_KANBAN_HOME",
    "WENSHU_KANBAN_WORKSPACES_ROOT",
    "WENSHU_KANBAN_LOGS_ROOT",
    "WENSHU_KANBAN_TASK",
    "WENSHU_KANBAN_WORKSPACE",
    "WENSHU_KANBAN_RUN_ID",
    "WENSHU_KANBAN_CLAIM_LOCK",
    "WENSHU_KANBAN_DISPATCH_IN_GATEWAY",
    "WENSHU_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "wenshu" host.
    "WENSHU_HONCHO_HOST",
    ...
    "WENSHU_DASHBOARD_OAUTH_CLIENT_ID",
    "WENSHU_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and WENSHU_HOME to per-test tempdirs so code that
    reads ``~/.wenshu-hermes/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral WENSHU_* vars that could change test semantics.
    for name in _WENSHU_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("WENSHU_HONCHO_HOST", "wenshu")

    # 3. Redirect WENSHU_HOME to a per-test tempdir. Code that reads
    #    ``~/.wenshu-hermes/*`` via ``get_wenshu_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.wenshu-hermes/*`` via
    #    ``Path.home() / ".wenshu-hermes"`` instead of ``get_wenshu_home()``
    #    is a bug to fix at the callsite.
    fake_wenshu_home = tmp_path / "wenshu_test"
    fake_wenshu_home.mkdir()
    (fake_wenshu_home / "sessions").mkdir()
    (fake_wenshu_home / "cron").mkdir()
    (fake_wenshu_home / "memories").mkdir()
    (fake_wenshu_home / "skills").mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(fake_wenshu_home))
    ...
    #    ~/.wenshu-hermes/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import wenshu_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_wenshu_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal wenshu config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``wenshu-gateway`` process
...
# correlated with a ``tests/wenshu_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> wenshu-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart wenshu-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting wenshu/python patterns are also blocked.
    """
    ...
    def _matches_wenshu_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _WENSHU_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_wenshu_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch wenshu-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m wenshu_cli.main".
                if (
                    "wenshu" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live wenshu-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting wenshu/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `wenshu update` (or the
        # equivalent `python -m wenshu_cli.main update`).  These commands
        ...
        if "update" in low and (
            # wenshu update / wenshu update --gateway / setsid bash -c ... wenshu update
            ("wenshu" in low and "update" in low.split())
            or
            # python -m wenshu_cli.main update --gateway
            ("wenshu_cli" in low and "update" in low.split())
            or
            # venv/bin/wenshu update  (absolute path variant used in tests)
            (".venv/bin/wenshu" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`wenshu update` against the real checkout, fetching "
