"""Tests for wenshu_constants module."""
...
from wenshu_constants import (
    ...
    find_wenshu_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_wenshu_root,
    get_wenshu_dir,
    get_wenshu_home,
    get_process_wenshu_home,
    heal_wenshu_managed_node,
    wenshu_managed_node_tree_present,
    iter_wenshu_node_dirs,
    is_container,
    node_tool_runnable,
    ...
    with_wenshu_node_path,
)


class TestGetDefaultWenshuRoot:
    """Tests for get_default_wenshu_root() — Docker/custom deployment awareness."""

    def test_no_wenshu_home_returns_native(self, tmp_path, monkeypatch):
        """When WENSHU_HOME is not set, returns ~/.wenshu."""
        monkeypatch.delenv("WENSHU_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_wenshu_root() == tmp_path / ".wenshu-hermes"

    def test_wenshu_home_is_native(self, tmp_path, monkeypatch):
        """When WENSHU_HOME = ~/.wenshu, returns ~/.wenshu."""
        native = tmp_path / ".wenshu-hermes"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WENSHU_HOME", str(native))
        assert get_default_wenshu_root() == native

    def test_wenshu_home_is_profile(self, tmp_path, monkeypatch):
        """When WENSHU_HOME is a profile under ~/.wenshu, returns ~/.wenshu."""
        native = tmp_path / ".wenshu-hermes"
    ...
    def test_wenshu_home_is_docker(self, tmp_path, monkeypatch):
        """When WENSHU_HOME points outside ~/.wenshu (Docker), returns WENSHU_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WENSHU_HOME", str(docker_home))
        assert get_default_wenshu_root() == docker_home

    def test_wenshu_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any WENSHU_HOME outside ~/.wenshu is treated as the root."""
        custom = tmp_path / "my-wenshu-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WENSHU_HOME", str(custom))
        assert get_default_wenshu_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (WENSHU_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("WENSHU_HOME", str(profile))
        assert get_default_wenshu_root() == docker_root

    def test_no_wenshu_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\wenshu, not ~/.wenshu."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("WENSHU_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(wenshu_constants.sys, "platform", "win32")

        assert get_default_wenshu_root() == local_appdata / "wenshu"

    def test_no_wenshu_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/wenshu without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("WENSHU_HOME", raising=False)
        ...
        assert get_default_wenshu_root() == home / "AppData" / "Local" / "wenshu"


class TestGetWenshuHome:
    """Tests for get_wenshu_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When WENSHU_HOME is unset on Windows, use %LOCALAPPDATA%\\wenshu."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("WENSHU_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(wenshu_constants.sys, "platform", "win32")
        monkeypatch.setattr(wenshu_constants, "_profile_fallback_warned", False)

        assert get_wenshu_home() == local_appdata / "wenshu"


class TestGetProcessWenshuHome:
    """Tests for get_process_wenshu_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_wenshu_home() == home

    def test_env_unset_returns_platform_default(self, tmp_path, monkeypatch):
        monkeypatch.delenv("WENSHU_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_process_wenshu_home() == tmp_path / ".wenshu-hermes"
    ...
    def test_ignores_context_local_override(self, tmp_path, monkeypatch):
        ...
        token = set_wenshu_home_override(profile_home)
        try:
            # get_wenshu_home() follows the override; the process-scoped
            # variant must not.
            assert get_wenshu_home() == profile_home
            assert get_process_wenshu_home() == launch_home
        finally:
            reset_wenshu_home_override(token)


class TestWenshuManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        home = tmp_path / "wenshu"
        node_dir = home / "node"
        ...
        assert iter_wenshu_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(wenshu_constants.sys, "platform", "win32")
        monkeypatch.setenv("WENSHU_HOME", str(home))
        monkeypatch.setattr(wenshu_constants, "node_tool_runnable", lambda path: True)

        assert find_wenshu_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(wenshu_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(wenshu_constants, "heal_wenshu_managed_node", lambda: False)
        monkeypatch.setattr(
            wenshu_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert wenshu_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_wenshu_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "wenshu"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_wenshu_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Wenshu-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(wenshu_constants, "heal_wenshu_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_wenshu_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(wenshu_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(wenshu_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from wenshu_constants import resolve_per_model_reasoning_effort
...
class TestReasoningOverridesDefaultConfig:
    ...
    def test_load_config_preserves_user_reasoning_overrides(self, tmp_path, monkeypatch):
        ...
        import yaml
        from wenshu_cli.config import load_config, get_config_path
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".wenshu-hermes"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import wenshu_cli._subprocess_compat as subprocess_compat
...
class TestGetWenshuDir:
    """Tests for ``get_wenshu_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_wenshu_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_wenshu_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_wenshu_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_wenshu_dir("cache/audio", "audio_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert wenshu_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert wenshu_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert wenshu_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert wenshu_constants.windows_path_to_wsl("/home/alex") is None
        assert wenshu_constants.windows_path_to_wsl("relative\\dir") is None

    def test_wsl_unc_to_posix_both_spellings(self):
        assert wenshu_constants.wsl_unc_path_to_posix(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        assert wenshu_constants.wsl_unc_path_to_posix(r"\\wsl$\Ubuntu\home\alex") == "/home/alex"
        # Forward-slash spelling and distro root.
        assert wenshu_constants.wsl_unc_path_to_posix("//wsl.localhost/Debian/srv/app") == "/srv/app"
        assert wenshu_constants.wsl_unc_path_to_posix("\\\\wsl.localhost\\Ubuntu\\") == "/"

    def test_wsl_unc_ignores_non_unc_paths(self):
        assert wenshu_constants.wsl_unc_path_to_posix(r"C:\Users\alex") is None
        assert wenshu_constants.wsl_unc_path_to_posix("/home/alex") is None

    def test_translate_is_noop_off_wsl(self, monkeypatch):
        monkeypatch.setattr(wenshu_constants, "is_wsl", lambda: False)
        assert wenshu_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == r"C:\Users\alex"

    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(wenshu_constants, "is_wsl", lambda: True)
        assert wenshu_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert wenshu_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert wenshu_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
