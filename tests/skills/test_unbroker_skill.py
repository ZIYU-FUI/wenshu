...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and wenshu-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # wenshu-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_wenshu_home():
    prev = os.environ.get("WENSHU_HOME")
    with tempfile.TemporaryDirectory() as d:
        os.environ["WENSHU_HOME"] = d
        try:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("WENSHU_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
