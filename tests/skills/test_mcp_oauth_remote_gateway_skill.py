...
SESSION_REVOKED / REFRESH_DEAD), the WENSHU_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, WENSHU_HOME=str(tokens_dir.parent))), \
         patch.object(mod.urllib.request, "urlopen", side_effect=fake_urlopen), \
         patch.object(sys, "argv", ["diagnose-oauth-mcp.py", *argv]):
        # Force the env-var fallback path (ignore any importable wenshu_constants).
        with patch.object(mod, "_wenshu_home", lambda: str(tokens_dir.parent)):
            buf = io.StringIO()
...
def test_wenshu_home_env_fallback(tmp_path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("WENSHU_HOME", str(tmp_path / "custom-home"))
    # Block the wenshu_constants import so the env fallback is exercised
    with patch.dict(sys.modules, {"wenshu_constants": None}):
        home = mod._wenshu_home()
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "Wenshu Agent"  # human credited first
