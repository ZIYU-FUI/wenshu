...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED Wenshu
        ...
        due-check compares against ``_wenshu_now()`` (configured zone), so the
        ...
        monkeypatch.setattr("cron.jobs._wenshu_now", lambda: configured_now)
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured Wenshu timezone differs from the
    ...
    def test_recent_past_oneshot_is_due_under_diverging_tz(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._wenshu_now", lambda: now)
...
class TestAdvanceNextRun:
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _wenshu_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _wenshu_now(), "next_run_at should be in the future after advance"
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from cron.jobs import _ensure_aware, _wenshu_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _wenshu_now(), "next_run_at should remain in the future"
...
class TestGetDueJobs:
    ...
    def test_long_execution_does_not_perpetually_defer(self, tmp_cron_dir, monkeypatch):
        ...
        stale = (_wenshu_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["next_run_at"] = stale
        jobs[0]["last_run_at"] = (_wenshu_now() - timedelta(minutes=1)).isoformat()
        save_jobs(jobs)
    ...
    def test_stale_repeat_limited_job_consumes_one_run_on_catchup(self, tmp_cron_dir, monkeypatch):
        ...
        from cron.jobs import _wenshu_now
        job = create_job(prompt="Limited", schedule="every 5m", repeat=3)
        jobs = load_jobs()
        jobs[0]["next_run_at"] = (_wenshu_now() - timedelta(minutes=11)).isoformat()
        jobs[0]["last_run_at"] = (_wenshu_now() - timedelta(minutes=11)).isoformat()
        save_jobs(jobs)
    ...
    def test_one_shot_not_redispatched_while_running(self, tmp_cron_dir, monkeypatch):
        ...
        for gap in (28, 61, 130):
            monkeypatch.setattr("cron.jobs._wenshu_now",
    ...
    def test_one_shot_run_claim_expires_after_ttl(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.delenv("WENSHU_CRON_TIMEOUT", raising=False)
        from cron.jobs import _wenshu_now, _oneshot_run_claim_ttl_seconds
    ...
    def test_run_claim_ttl_derived_from_cron_timeout(self, tmp_cron_dir, monkeypatch):
        """The stale-recovery TTL tracks WENSHU_CRON_TIMEOUT (3x headroom), with
    ...
    def test_cron_next_run_offset_migration_is_rescheduled_not_fired(self, tmp_cron_dir, monkeypatch):
        ...
        # A 21:00 cron was stored while Wenshu/system local time was UTC+10.
...
class TestCronOutputRetention:
    ...
    def test_save_job_output_prunes_old_runs(self, tmp_cron_dir, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._wenshu_now", lambda: next(seq))
        for _ in range(8):
    ...
    def test_cron_output_keep_reads_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "wenshu_cli.config.load_config", lambda: {"cron": {"output_retention": 7}}
        )
    ...
    def test_cron_output_keep_defaults_on_bad_config(self, monkeypatch):
        import cron.jobs as jobs
        monkeypatch.setattr(
            "wenshu_cli.config.load_config", lambda: {"cron": {"output_retention": "oops"}}
        )
...
class TestLateEnvRepointScopesStore:
    """A WENSHU_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_unchanged_home_returns_import_time_constants(self, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(jobs.WENSHU_DIR))
        store = jobs._current_cron_store()
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path / "env-home"))
    ...
    def test_patched_compatibility_constants_beat_env(self, tmp_path, monkeypatch):
        ...
        process-wide escape hatch — they win over a repointed WENSHU_HOME."""
        import cron.jobs as jobs
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import WENSHU_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "WENSHU_DIR", sim_old_home)
        ...
        monkeypatch.setenv("WENSHU_HOME", str(new_home))
