...
def test_root_session_no_compression(db):
    ...
    assert prov["currentWenshuSessionId"] == "root1"
    assert prov["rootWenshuSessionId"] == "root1"
    assert prov["parentWenshuSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_wenshu_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentWenshuSessionId"] == "old"
    assert prov["rootWenshuSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousWenshuSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootWenshuSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootWenshuSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_wenshu_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousWenshuSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"wenshu"}
    assert "sessionProvenance" in meta["wenshu"]
    assert meta["wenshu"]["sessionProvenance"]["currentWenshuSessionId"] == "root1"
