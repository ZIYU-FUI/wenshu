...
class TestToolsetConsistency:
    ...
    def test_wenshu_platforms_share_core_tools(self):
        """All wenshu-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on wenshu-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["wenshu-cli", "wenshu-telegram", "wenshu-discord", "wenshu-whatsapp", "wenshu-slack", "wenshu-signal", "wenshu-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_wenshu_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("wenshu-whatsapp")

    def test_wenshu_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("wenshu-api-server")
