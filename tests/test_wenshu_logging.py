"""Tests for wenshu_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
# Use whatever RotatingFileHandler class wenshu_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. wenshu_logging
...
from wenshu_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    wenshu_logging._logging_initialized = False
    ...
    wenshu_logging._reset_queued_handlers()
    ...
    wenshu_logging._install_session_record_factory()
    yield
    ...
    wenshu_logging._logging_initialized = False
    wenshu_logging.clear_session_context()


@pytest.fixture
def wenshu_home(tmp_path, monkeypatch):
    """Provide an isolated WENSHU_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_wenshu_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["WENSHU_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, wenshu_home):
        log_dir = wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        assert log_dir == wenshu_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in wenshu_logging.rotating_file_handlers()
    ...
    def test_creates_errors_log_handler(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, force=True)
    ...
    def test_custom_log_level(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, wenshu_home):
        wenshu_logging.setup_logging(
            wenshu_home=wenshu_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
    ...
    def test_writes_to_agent_log(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)

        test_logger = logging.getLogger("test_wenshu_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        wenshu_logging.flush_log_queue()

        agent_log = wenshu_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)

        test_logger = logging.getLogger("test_wenshu_logging.warning_test")
        test_logger.warning("this is a warning")

        wenshu_logging.flush_log_queue()

        agent_log = wenshu_home / "logs" / "agent.log"
        errors_log = wenshu_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)

        test_logger = logging.getLogger("test_wenshu_logging.info_test")
    ...
    def test_explicit_params_override_config(self, wenshu_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (wenshu_home / "config.yaml").write_text(yaml.dump(config))

        wenshu_logging.setup_logging(wenshu_home=wenshu_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, wenshu_home):
        """The custom record factory injects session_tag on all records."""
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_wenshu_session_injector", False), (
            "Record factory should have _wenshu_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, wenshu_home):
        ...
        gw_log = wenshu_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, wenshu_home):
    ...
    def test_gateway_log_receives_gateway_records(self, wenshu_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, wenshu_home):
    ...
    def test_agent_log_still_receives_all(self, wenshu_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="cli")
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="gui")

        logging.getLogger("wenshu_cli.web_server").info("dashboard online")
        logging.getLogger("gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        wenshu_logging.flush_log_queue()

        gui_log = wenshu_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, wenshu_home):
        """When session context is set, log lines include [session_id]."""
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        wenshu_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, wenshu_home):
    ...
    def test_clear_session_context(self, wenshu_home):
        """After clearing, session tag disappears."""
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        wenshu_logging.set_session_context("xyz789")
        wenshu_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, wenshu_home):
        ...
        def thread_a():
            wenshu_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            wenshu_logging.flush_log_queue()

        def thread_b():
            wenshu_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        wenshu_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        wenshu_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = wenshu_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = wenshu_logging._ComponentFilter(
            wenshu_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = wenshu_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in wenshu_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the wenshu_plugins
        ...
        # break this test.
        gateway_prefixes = wenshu_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "wenshu_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = wenshu_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == wenshu_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = wenshu_logging.COMPONENT_PREFIXES["cli"]
        assert "wenshu_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == wenshu_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = wenshu_logging.COMPONENT_PREFIXES["gui"]
        assert "wenshu_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        wenshu_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_wenshu_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        wenshu_logging.setup_verbose_logging()
        wenshu_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        wenshu_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = wenshu_logging._ComponentFilter(("test",))
        ...
        handlers = [h for h in wenshu_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        wenshu_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("wenshu_cli.config.is_managed", return_value=True):
                wenshu_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("wenshu_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in wenshu_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                wenshu_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = wenshu_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(wenshu_logging.sys, "platform", "win32"):
            assert wenshu_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not wenshu_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(wenshu_logging.sys, "platform", "linux"):
            assert not wenshu_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, wenshu_home):
        level, max_size, backup = wenshu_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, wenshu_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, wenshu_home):
        import yaml
        config = {"model": "test"}
        (wenshu_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> wenshu_logging._ManagedRotatingFileHandler:
        handler = wenshu_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that wenshu_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        wenshu_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        wenshu_logging.setup_logging(wenshu_home=wenshu_home, mode="gateway")
        gw_path = wenshu_home / "logs" / "gateway.log"
        rotated = wenshu_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = wenshu_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_wenshu_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in wenshu_logging.rotating_file_handlers()
        )

    def test_records_reach_file_through_queue(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
    ...
    def test_queue_preserves_per_handler_levels(self, wenshu_home):
        wenshu_logging.setup_logging(wenshu_home=wenshu_home)
