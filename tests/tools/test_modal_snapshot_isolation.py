...
def _restore_tool_modules():
    original_wenshu_home = os.environ.get("WENSHU_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_wenshu_home is None:
            os.environ.pop("WENSHU_HOME", None)
        else:
            os.environ["WENSHU_HOME"] = original_wenshu_home
        _reset_modules(("tools", "wenshu_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    wenshu_cli = types.ModuleType("wenshu_cli")
    wenshu_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["wenshu_cli"] = wenshu_cli
    wenshu_home = tmp_path / "wenshu-home"
    os.environ["WENSHU_HOME"] = str(wenshu_home)
    sys.modules["wenshu_cli.config"] = types.SimpleNamespace(
        get_wenshu_home=lambda: wenshu_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="wenshu-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": wenshu_home / "modal_snapshots.json",
