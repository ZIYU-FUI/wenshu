...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        wenshu_home.mkdir()
        (wenshu_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(wenshu_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.wenshu-hermes/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        (wenshu_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        (wenshu_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        (wenshu_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.wenshu-hermes/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        wenshu_home = tmp_path / ".wenshu-hermes"
        (wenshu_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"WENSHU_HOME": str(wenshu_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.wenshu-hermes")

        assert mounts[0]["container_path"] == "/home/user/.wenshu-hermes/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.wenshu-hermes/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.wenshu-hermes/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape WENSHU_HOME."""
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path / ".wenshu-hermes"))
        (tmp_path / ".wenshu-hermes").mkdir()

        # Create a sensitive file one level above wenshu_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside wenshu_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside WENSHU_HOME must still be registered."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
        (wenshu_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_wenshu_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of WENSHU_HOME must be allowed."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        subdir = wenshu_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside WENSHU_HOME pointing outside must be rejected."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))

        # Create a sensitive file outside wenshu_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside wenshu_home pointing outside
        symlink = wenshu_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes WENSHU_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, wenshu_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape WENSHU_HOME."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
        ...
        sensitive.write_text("{}")
        self._write_config(wenshu_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(wenshu_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside WENSHU_HOME via config must still mount."""
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))

        (wenshu_home / "oauth.json").write_text("{}")
        self._write_config(wenshu_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        (wenshu_home / "cache" / "documents").mkdir(parents=True)
        (wenshu_home / "cache" / "audio").mkdir(parents=True)
        (wenshu_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.wenshu-hermes/cache/documents" in paths
        assert "/root/.wenshu-hermes/cache/audio" in paths
        assert "/root/.wenshu-hermes/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.wenshu-hermes/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_wenshu_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        wenshu_home = tmp_path / ".wenshu-hermes"
        wenshu_home.mkdir()
        # Use legacy dir name with content — get_wenshu_dir prefers
        # populated old over new.
        legacy_doc = wenshu_home / "document_cache"
        legacy_img = wenshu_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(wenshu_home / "document_cache") in host_paths
        assert str(wenshu_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.wenshu-hermes/cache/documents" in container_paths
        assert "/root/.wenshu-hermes/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        wenshu_home = tmp_path / ".wenshu-hermes"
        img_dir = wenshu_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.wenshu-hermes/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.wenshu-hermes")
            == "/home/agent/.wenshu-hermes/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        wenshu_home = tmp_path / ".wenshu-hermes"
        (wenshu_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("WENSHU_HOME", str(wenshu_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(wenshu_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        doc_dir = wenshu_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        wenshu_home = tmp_path / ".wenshu-hermes"
        ss_dir = wenshu_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.wenshu-hermes/cache/screenshots/session_abc/screen1.png"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — WENSHU_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping WENSHU_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"WENSHU_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.wenshu-hermes/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.wenshu-hermes/google_token.json" in paths
        assert "/root/.wenshu-hermes/.env" not in paths
