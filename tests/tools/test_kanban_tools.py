...
Verifies:
  - Tools are gated on WENSHU_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `wenshu chat` sessions (no WENSHU_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("WENSHU_KANBAN_TASK", raising=False)
    ...
    schema = registry.get_definitions(set(resolve_toolset("wenshu-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("WENSHU_KANBAN_TASK", "t_fake")
    home = tmp_path / ".wenshu-hermes"
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the WENSHU_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: WENSHU_HOME isolated, WENSHU_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".wenshu-hermes"
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
    monkeypatch.setenv("WENSHU_PROFILE", "test-worker")
    monkeypatch.delenv("WENSHU_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("WENSHU_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["wenshu_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("WENSHU_SESSION_ID", "session-trusted")
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("WENSHU_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated WENSHU_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to WENSHU_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``WENSHU_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``wenshu-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "wenshu-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from WENSHU_PROFILE in the fixture, not the
        # caller-supplied "wenshu-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("WENSHU_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no WENSHU_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("WENSHU_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from wenshu_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
    monkeypatch.setenv("WENSHU_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no WENSHU_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (WENSHU_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has WENSHU_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no WENSHU_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # WENSHU_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("WENSHU_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("WENSHU_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no WENSHU_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via WENSHU_KANBAN_BOARD env var,
...
# restarting Wenshu. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Wenshu home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    WENSHU_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".wenshu-hermes"
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
    # Make sure neither WENSHU_KANBAN_DB nor WENSHU_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("WENSHU_KANBAN_DB", raising=False)
    monkeypatch.delenv("WENSHU_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("WENSHU_KANBAN_TASK", raising=False)
    monkeypatch.setenv("WENSHU_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning WENSHU_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".wenshu-hermes"
    home.mkdir()
    monkeypatch.setenv("WENSHU_HOME", str(home))
    monkeypatch.setenv("WENSHU_PROFILE", "alt-worker")
    monkeypatch.delenv("WENSHU_KANBAN_DB", raising=False)
    monkeypatch.delenv("WENSHU_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: WENSHU_SESSION_PLATFORM + WENSHU_SESSION_CHAT_ID set.
# - TUI sessions: WENSHU_SESSION_KEY (or WENSHU_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("WENSHU_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("WENSHU_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("WENSHU_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("WENSHU_SESSION_USER_ID", "user-9")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports WENSHU_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("WENSHU_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("WENSHU_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("WENSHU_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("WENSHU_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_KEY", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.wenshu; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".wenshu-hermes"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("WENSHU_HOME", str(home))
    monkeypatch.setenv("WENSHU_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("WENSHU_SESSION_CHAT_ID", "channel-1")
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("WENSHU_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("WENSHU_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_KEY", raising=False)
    monkeypatch.delenv("WENSHU_SESSION_ID", raising=False)
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting WENSHU_ALLOW_PRIVATE_URLS on a private network.
    Resets the url_safety process-lifetime cache on both sides so the
    override neither leaks in nor out of the test.
    """
    from tools import url_safety

    monkeypatch.setenv("WENSHU_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    Clears WENSHU_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("WENSHU_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
