...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("wenshu_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("wenshu_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_SESSION_KEY", session_key)
        monkeypatch.setenv("WENSHU_EXEC_ASK", "1")
        monkeypatch.delenv("WENSHU_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("wenshu-verify-", "wenshu-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "wenshu-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/wenshu-verify-example.py",
            "rm -f /tmp/wenshu-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/wenshu-verify-example.py",
            "rm -f /tmp/nested/../wenshu-verify-example.py",
            "rm -f /tmp/./wenshu-verify-example.py",
            "rm -f /tmp//wenshu-verify-example.py",
            "rm -f /tmp/a/../../tmp/wenshu-verify-example.py",
            "rm -f /var/tmp/wenshu-verify-example.py",
            "rm -f /tmp/unrelated.py",
            "rm -f /tmp/wenshu-verify-*",
            "rm -f /tmp/wenshu-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/wenshu-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/wenshu-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\wenshu-victim\file.txt"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\wenshu-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\wenshu-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\wenshu-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\wenshu-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"WENSHU_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_wenshu_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.wenshu-hermes/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_wenshu_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $WENSHU_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_wenshu_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$WENSHU_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestWenshuConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.wenshu-hermes/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.wenshu-hermes/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.wenshu-hermes/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.wenshu-hermes/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.wenshu-hermes/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.wenshu-hermes/config.yaml")
        assert dangerous is True
        assert "wenshu config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.wenshu-hermes/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_wenshu_home_config(self):
        config_path = get_wenshu_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_sed_in_place_absolute_wenshu_home_env(self):
        env_path = get_wenshu_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_wenshu_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $WENSHU_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.wenshu-hermes/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_wenshu_home_config(self):
        config_path = get_wenshu_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.wenshu-hermes/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_wenshu_home_env(self):
        env_path = get_wenshu_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.wenshu-hermes/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.wenshu-hermes/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.wenshu-hermes/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.wenshu-hermes/config.yaml"
        )
        assert dangerous is True
        assert key != "in-place edit of Wenshu config/env (perl/ruby)"

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.wenshu-hermes/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Wenshu config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_wenshu_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $WENSHU_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.wenshu-hermes/config.yaml/.env must require approval — the
    ...
    def test_cp_to_wenshu_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.wenshu-hermes/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Wenshu-home prefixes must fold to ~/ and
    ~/.wenshu-hermes/ in dangerous-command detection.
    ...
    SSH, and Wenshu config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_wenshu_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/WENSHU_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_wenshu_home_config_folds(self, monkeypatch):
        # Wenshu home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("WENSHU_HOME", r"C:\Users\tester\.wenshu-hermes")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.wenshu-hermes\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(
        ...
        monkeypatch.setenv("WENSHU_LANGUAGE", lang)
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.wenshu-hermes/wenshu-agent && source venv/bin/activate && python -m wenshu_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m wenshu_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m wenshu_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "wenshu_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart wenshu-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_wenshu_gateway_stop_detected(self):
        cmd = "wenshu gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_wenshu_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `wenshu` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "wenshu -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_wenshu_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "wenshu --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_wenshu_gateway_multiple_flags_detected(self):
        cmd = "wenshu -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_wenshu_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "wenshu -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_wenshu_gateway_start_not_flagged(self):
        cmd = "wenshu gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_wenshu_detected(self):
        """pkill targeting wenshu/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_wenshu_detected(self):
        cmd = "killall wenshu"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Wenshu security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.wenshu-hermes/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep wenshu) bypasses the pkill/killall name-matching
    ...
    def test_pkill_wenshu_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof wenshu)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof wenshu_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof wenshu`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Wenshu service
    label achieves the same effect as `wenshu gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_wenshu_detected(self):
        cmd = "launchctl stop ai.wenshu.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "wenshu" in desc.lower()

    def test_launchctl_kickstart_wenshu_detected(self):
        cmd = "launchctl kickstart -k system/ai.wenshu.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_wenshu_detected(self):
        cmd = "launchctl bootout system/ai.wenshu.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_wenshu_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.wenshu.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Wenshu label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("WENSHU_GATEWAY_SESSION", "WENSHU_CRON_SESSION",
                      "WENSHU_YOLO_MODE",
                      "WENSHU_SESSION_KEY", "WENSHU_INTERACTIVE")
        }
        os.environ.pop("WENSHU_YOLO_MODE", None)
        os.environ.pop("WENSHU_INTERACTIVE", None)
        # WENSHU_CRON_SESSION takes priority over WENSHU_GATEWAY_SESSION in
        ...
        os.environ.pop("WENSHU_CRON_SESSION", None)
        os.environ["WENSHU_GATEWAY_SESSION"] = "1"
        os.environ["WENSHU_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("wenshu_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"WENSHU_INTERACTIVE": "1"}, clear=False):
