...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.wenshu-hermes/)``
    ...
    def test_refuses_wenshu_home_itself(self, tmp_path):
        """``~/.wenshu-hermes/`` itself is what the #48200 wipe destroyed."""
        from tools.skills_sync import _rmtree_writable

        wenshu = tmp_path / "home"
        wenshu.mkdir()
        (wenshu / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", wenshu / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(wenshu)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        wenshu = tmp_path / "home"
        wenshu.mkdir()
        skills = wenshu / "skills"
        skills.mkdir()
        not_skills = wenshu / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "wenshu skills reset new-skill" in captured
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """WENSHU_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("WENSHU_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("WENSHU_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty WENSHU_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("WENSHU_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `wenshu profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.wenshu) rely on so bundled
    skills are never seeded at install time NOR re-injected by `wenshu update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        wenshu_home = tmp_path / "home"
        wenshu_home.mkdir()
        (wenshu_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.WENSHU_HOME", wenshu_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`wenshu skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.WENSHU_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.WENSHU_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
