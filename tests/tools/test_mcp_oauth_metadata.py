...
Covers:
- :class:`WenshuTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.WenshuMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _WENSHU_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# WenshuTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = WenshuTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        storage = WenshuTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (WenshuMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: WenshuTokenStorage, **context_attrs):
    ...
    provider = _WENSHU_PROVIDER_CLS.__new__(_WENSHU_PROVIDER_CLS)
    provider._wenshu_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        storage = WenshuTokenStorage("mgr-srv")
        ...
        with patch.object(
            _WENSHU_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        storage = WenshuTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        storage = WenshuTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        storage = WenshuTokenStorage("noop-srv")
        ...
        with patch.object(
            WenshuTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("WENSHU_HOME", str(tmp_path))
        storage = WenshuTokenStorage("flow-srv")
        ...
        with patch.object(
            _WENSHU_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
