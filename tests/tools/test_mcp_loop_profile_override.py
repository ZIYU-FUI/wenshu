"""Regression tests for WENSHU_HOME override propagation onto the MCP loop.
...
get_wenshu_home() inside the coroutine, most visibly OAuth token-store
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: WenshuTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(WenshuTokenStorage("probe-srv")._tokens_path())
