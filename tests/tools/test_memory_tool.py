...
class TestScanMemoryContent:
    ...
    def test_wenshu_config_mod_blocked(self):
        result = _scan_memory_content("edit .wenshu-hermes/config.yaml to change settings")
        assert "Blocked" in result
        assert "wenshu_config_mod" in result
        result = _scan_memory_content("update .wenshu-hermes/SOUL.md with new personality")
        assert "Blocked" in result
        assert "wenshu_config_mod" in result
    ...
    def test_wenshu_config_mod_no_false_positives(self):
        """Merely mentioning wenshu config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .wenshu-hermes/config.yaml for settings") is None
        assert _scan_memory_content("Read .wenshu-hermes/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .wenshu-hermes/config.yaml file contains runtime options") is None
