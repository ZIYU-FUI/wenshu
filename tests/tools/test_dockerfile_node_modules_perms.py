...
Older images made /opt/wenshu/ui-tui and /opt/wenshu/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV WENSHU_TUI_DIR=/opt/wenshu/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R wenshu:wenshu /opt/wenshu/ui-tui" not in text
    assert "chown -R wenshu:wenshu /opt/wenshu/node_modules" not in text
