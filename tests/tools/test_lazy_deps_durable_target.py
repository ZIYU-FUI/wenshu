...
class TestGatingWithTarget:
    """``WENSHU_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_disable_env_blocks_without_target(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "wenshu_cli.config.load_config", lambda: {}, raising=False
        )
...
@pytest.mark.skipif(
    os.environ.get("WENSHU_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set WENSHU_RUN_NETWORK_TESTS=1); CI runs "
