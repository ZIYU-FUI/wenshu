...
class TestWriteDenyExactPaths:
    ...
    def test_wenshu_env(self):
        # ``.env`` under the active WENSHU_HOME (profile-aware, not just
        # ``~/.wenshu``) must be write-denied. The hermetic test conftest
        # points WENSHU_HOME at a tempdir — resolve via get_wenshu_home()
        # to match the denylist.
        from wenshu_constants import get_wenshu_home
        path = str(get_wenshu_home() / ".env")
        assert _is_write_denied(path) is True

    def test_wenshu_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.wenshu-hermes/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "wenshu_root"
        ...
        # Sanity check: WENSHU_HOME does point to the profile dir, not the root.
        from wenshu_constants import get_wenshu_home, get_default_wenshu_root
        assert get_wenshu_home() == profile_home
        assert get_default_wenshu_root() == root
...
class TestWriteAllowed:
    ...
    def test_wenshu_control_files_requested_writable(self):
        from wenshu_constants import get_wenshu_home
