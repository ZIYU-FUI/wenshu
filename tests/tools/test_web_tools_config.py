...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_wenshu_home_override(self, tmp_path):
        """Auth lookup should read from WENSHU_HOME/auth.json, not ~/.wenshu-hermes/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".wenshu-hermes").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``wenshu tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("wenshu_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "wenshu_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from wenshu_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.wenshu-hermes/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.wenshu-hermes/.env), not bare os.getenv. Parametrized over the four
