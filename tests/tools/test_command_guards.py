...
def _clean_state():
    ...
    saved = {}
    for k in ("WENSHU_INTERACTIVE", "WENSHU_GATEWAY_SESSION", "WENSHU_EXEC_ASK", "WENSHU_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["WENSHU_INTERACTIVE"] = "1"
        session_key = os.getenv("WENSHU_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No WENSHU_INTERACTIVE or WENSHU_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
