...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._wenshu_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(wenshu_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(wenshu_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_wenshu_bin(self):
        """After _INSTALL_FAILED, manual install in WENSHU_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        wenshu_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(wenshu_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(wenshu_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._wenshu_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == wenshu_bin
            assert _tirith_mod._resolved_path == wenshu_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# WENSHU_HOME isolation
# ---------------------------------------------------------------------------

class TestWenshuHomeIsolation:
    def test_wenshu_bin_dir_respects_wenshu_home(self):
        """_wenshu_bin_dir must use WENSHU_HOME, not hardcoded ~/.wenshu."""
        from tools.tirith_security import _wenshu_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"WENSHU_HOME": tmpdir}):
            result = _wenshu_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_wenshu_home(self):
        """_failure_marker_path must use WENSHU_HOME, not hardcoded ~/.wenshu."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"WENSHU_HOME": "/custom/wenshu"}):
            result = _failure_marker_path()
        assert result == "/custom/wenshu/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets WENSHU_HOME; verify it's active."""
        wenshu_home = os.getenv("WENSHU_HOME")
        assert wenshu_home is not None, "WENSHU_HOME should be set by conftest"
        assert "wenshu_test" in wenshu_home, "Should point to test temp dir"

    def test_get_wenshu_home_fallback(self):
        """Without WENSHU_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_wenshu_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove WENSHU_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".wenshu-hermes")
            result = _get_wenshu_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._wenshu_bin_dir",
