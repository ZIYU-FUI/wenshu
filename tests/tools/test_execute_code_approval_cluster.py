...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad WENSHU_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: WENSHU_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("WENSHU_EXEC_ASK", raising=False)
...
def test_guard_headless_local_approved(monkeypatch):
    ...
    monkeypatch.delenv("WENSHU_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("WENSHU_INTERACTIVE", raising=False)
    monkeypatch.delenv("WENSHU_CRON_SESSION", raising=False)
    monkeypatch.delenv("WENSHU_EXEC_ASK", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setattr(A, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setenv("WENSHU_CRON_SESSION", "1")
    monkeypatch.delenv("WENSHU_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_wenshu_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "WENSHU_HOME": "/h", "WENSHU_PROFILE": "p",
        "WENSHU_CONFIG": "/c.yaml", "WENSHU_ENV": "/e",
        # other WENSHU_* → dropped (broad prefix removed)
        "WENSHU_BASE_URL": "https://x", "WENSHU_INTERACTIVE": "1",
        "WENSHU_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("WENSHU_HOME", "WENSHU_PROFILE", "WENSHU_CONFIG", "WENSHU_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "WENSHU_BASE_URL", "WENSHU_INTERACTIVE", "WENSHU_KANBAN_DB",
...
def test_env_scrub_logs_dropped_wenshu_vars(caplog):
    """Dropping a non-allowlisted, non-secret WENSHU_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset WENSHU_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "WENSHU_HOME": "/h",          # allowlisted → kept, not logged
        "WENSHU_BASE_URL": "https://x",   # dropped → logged
        "WENSHU_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "WENSHU_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "WENSHU_HOME" in out and "PATH" in out
    assert "WENSHU_BASE_URL" not in out and "WENSHU_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "WENSHU_BASE_URL" in msgs and "WENSHU_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "WENSHU_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped WENSHU_* vars."""
    import logging
