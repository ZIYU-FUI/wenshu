...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    wenshu_state = ModuleType("wenshu_state")
    wenshu_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "wenshu_state", wenshu_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Wenshu"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Wenshu"
