"""Centralized logging setup for Wenshu Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.wenshu-hermes/logs/`` (profile-aware via ``get_wenshu_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``wenshu_cli.web_server``,
    ``wenshu_cli.pty_bridge``, the desktop chat transport, and ``uvicorn.*``.
...
# essentially always in Wenshu (TUI, gateway, ``hy_memory`` server, MCP
...
from wenshu_constants import get_config_path, get_wenshu_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_wenshu_session_injector", False):
    ...
    _session_record_factory._wenshu_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``wenshu logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``wenshu logs --component gateway``.
    "gateway": ("gateway", "wenshu_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("wenshu_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "wenshu_cli.web_server",
        "wenshu_cli.pty_bridge",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    wenshu_home: Optional[Path] = None,
    ...
    """Configure the Wenshu logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    wenshu_home
        Override for the Wenshu home directory.  Falls back to
        ``get_wenshu_home()`` (profile-aware).
    log_level
    ...
    home = wenshu_home or get_wenshu_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_wenshu_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._wenshu_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the wenshu group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from wenshu_cli.config import is_managed
...
# the module header): when several Wenshu processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._wenshu_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="wenshu-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_wenshu_queue", False):
