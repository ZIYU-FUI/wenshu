 ...
 *
 * The root ``package.json`` is installed by ``wenshu update`` on every user,
 ...
 *   ``wenshu tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``wenshu update`` for
 ...
 * guidance in the ``wenshu-agent-dev`` skill before changing the
 * expectations.
 */
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in wenshu_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`wenshu tools` → Browser Automation → Camofox.'
  )
})
...
test('agent-browser stays eager (default backend)', () => {
  ...
  assert.ok(
    ...
    'agent-browser is the default browser-tool backend used by every ' +
      ...
      'after `wenshu setup` / `wenshu update` without an explicit ' +
      'post_setup step.'
  )
})
