#!/usr/bin/env python3
"""
文枢 CLI - Interactive Terminal Interface

A beautiful command-line interface for the 文枢, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills wenshu-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: wenshu_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See wenshu_bootstrap.py for full rationale.
try:
    import wenshu_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when wenshu_bootstrap isn't registered in the venv
    # yet — happens during partial ``wenshu update`` where git-reset landed
...
os.environ["WENSHU_QUIET"] = "1"  # Our own modules

import yaml

from wenshu_cli.fallback_config import get_fallback_chain
from wenshu_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from wenshu_cli.cli_commands_mixin import CLICommandsMixin
from wenshu_cli.cli_billing_mixin import CLIBillingMixin
...
try:
    from wenshu_cli.pt_input_extras import (
...
def _reverse_alias_for_display(model_name: str) -> str:
    ...
    and ``model.aliases:`` (string-based, set via ``wenshu config set``)
...
from wenshu_cli.banner import _format_context_length, format_banner_version_label
...
from wenshu_cli.browser_connect import (
...
_wenshu_home = get_wenshu_home()
_project_env = Path(__file__).parent / '.env'
load_wenshu_dotenv(wenshu_home=_wenshu_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.wenshu-hermes/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("WENSHU_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def _parse_reasoning_config(effort) -> dict | None:
    ...
    """
    from wenshu_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.wenshu-hermes/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If WENSHU_IGNORE_USER_CONFIG=1 is set (via ``wenshu chat --ignore-user-config``),
    the user config at ``~/.wenshu-hermes/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({WENSHU_HOME}/config.yaml)
    user_config_path = _wenshu_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("WENSHU_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain 文枢, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me 文枢 - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see wenshu_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
            ...
            # wenshu_cli/config.py DEFAULT_CONFIG (display.show_reasoning).
            "show_reasoning": True,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from wenshu_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and WenshuCLI.__init__ checks "default" first.
    ...
    from wenshu_cli.config import _expand_env_vars
    ...
    # wenshu_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `wenshu config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (wenshu_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && wenshu` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _WENSHU_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_WENSHU_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["WENSHU_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.wenshu-hermes/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from wenshu_logging import setup_logging
...
try:
    from wenshu_cli.config import print_config_warnings
...
try:
    from wenshu_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``wenshu --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from wenshu_cli.mcp_startup import wait_for_mcp_discovery
...
from wenshu_cli.banner import build_welcome_banner
from wenshu_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from wenshu_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("WENSHU_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("WENSHU_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _arm_exit_watchdog(timeout_s: float | None = None) -> None:
    ...
    Tune with ``WENSHU_EXIT_WATCHDOG_S`` (seconds); ``0`` disables.
    """
    if timeout_s is None:
        try:
            timeout_s = float(os.getenv("WENSHU_EXIT_WATCHDOG_S", "30"))
...
def _arm_exit_watchdog_on_shutdown_signal() -> None:
    ...
    (observed: ``wenshu --tui`` alive ~47 min at 4% CPU after terminal close —
    the #65998 class).
    ...
    try:
        base = float(os.getenv("WENSHU_EXIT_WATCHDOG_S", "30"))
    except (TypeError, ValueError):
        base = 30.0
    if base <= 0:
...
def _notify_session_finalize(
    *,
    ...
    try:
        from wenshu_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.wenshu-hermes/wenshu-agent`` clone is updated only by ``wenshu update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run wenshu -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"wenshu-{short_id}"
    branch_name = f"wenshu/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"wenshu pid={os.getpid()}", str(wt_path)],
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    ``wenshu -w`` locks each worktree with reason ``wenshu pid=<pid>`` so a
    concurrent wenshu process' startup prune leaves an in-use worktree alone.
    ...
    - ``"dead"``  — locked but the owning pid is gone, or the reason isn't a
                    parseable wenshu lock (safe to unlock + reap).
    ...
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"wenshu pid=(\d+)", reason)
            if not m:
                # Locked by something we don't recognize as a wenshu session
                # (or lock reason unavailable). Treat as dead — a foreign lock
                # on a wenshu -w worktree is almost certainly a leftover, and
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`wenshu_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from wenshu_cli.config import load_config as _load_full_config
        from wenshu_constants import get_wenshu_home as _get_wenshu_home
        _wenshu_home_maint = _get_wenshu_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_wenshu_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`wenshu_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Lock handling (orthogonal to age): ``wenshu -w`` locks each worktree with
    reason ``wenshu pid=<pid>`` so a concurrent wenshu process leaves an in-use
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    Also prunes orphaned ``wenshu/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("wenshu-"):
            continue
        ...
        # wenshu process means the worktree is actively in use — never touch
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``wenshu/wenshu-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``wenshu -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("wenshu/wenshu-") or b.startswith("pr-"))
    ]
...
#
# Mirrors the terminal UI theme module detectLightMode().  Used to decide whether
...
#
# Detection priority:
#   1. WENSHU_LIGHT / WENSHU_TUI_LIGHT env (true/false) — explicit override
#   2. WENSHU_TUI_THEME=light|dark — explicit theme
#   3. WENSHU_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy 文枢 assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("WENSHU_LIGHT", "WENSHU_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("WENSHU_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("WENSHU_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from wenshu_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_wenshu_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._wenshu_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from wenshu_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in 文枢 output it's common to display cron schedules verbatim.
    #
...
from wenshu_constants import is_termux as _is_termux_environment
...
def _strip_leaked_bracketed_paste_wrappers(text: str) -> str:
    from wenshu_cli.input_sanitize import strip_leaked_bracketed_paste_wrappers
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_wenshu_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_wenshu_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._wenshu_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_wenshu_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._wenshu_bp_start = now
        ...
        _vt100_mod._wenshu_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``WenshuCLI._busy_command()``.
        """
        yield self

# ASCII Art - WENSHU-AGENT logo (full width, single line - requires ~95 char terminal)
WENSHU_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - 文枢 Caduceus (compact, fits in left panel)
WENSHU_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS WENSHU - AI Agent Framework"
        tiny_line = "⚕ NOUS WENSHU"
    else:
        agent_name = _skin.get_branding("agent_name", "文枢") if _skin else "文枢"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("WENSHU_FAST_STARTUP_BANNER") == "1":
        from wenshu_cli import __release_date__ as _release_date
        from wenshu_cli import __version__ as _version

        version_line = f"文枢 v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from wenshu_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.wenshu-hermes/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.wenshu-hermes/config.yaml on first use)
...
class WenshuCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    """
    Interactive CLI for the 文枢.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: WENSHU_MAX_TOKENS)
        _env_mt = os.environ.get("WENSHU_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from wenshu_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("WENSHU_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("WENSHU_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("WENSHU_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `wenshu chat --ignore-rules` in wenshu_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("WENSHU_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("WENSHU_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # shared chokepoint in wenshu_constants (Closes #21256).
        from wenshu_constants import resolve_reasoning_config
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `wenshu update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all 文枢 processes for this WENSHU_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.wenshu-hermes/checkpoints/.  Opt-in via
        ...
        self._history_file = _wenshu_home / ".wenshu_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from wenshu_cli.active_sessions import try_acquire_active_session
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        """
        try:
            from wenshu_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        if model_short == model_name:
            ...
            from wenshu_cli.model_switch import format_model_for_display
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``wenshu pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from wenshu_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else '文枢'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from wenshu_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from wenshu_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from wenshu_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from wenshu_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from wenshu_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ 文枢")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - WenshuCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``wenshu doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``wenshu doctor`` so the banner stays
        small.
        """
        try:
            from wenshu_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   文枢 needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Wenshu LLM (not agentic)
        from wenshu_cli.model_switch import is_nous_wenshu_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_wenshu_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research 文枢 3 & 4 models are NOT agentic and are not "
                "designed for use with 文枢.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.wenshu-hermes/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from wenshu_cli.clipboard import save_clipboard_image

        img_dir = get_wenshu_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'wenshu setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from wenshu_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from wenshu_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from wenshu_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with 文枢!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from wenshu_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from wenshu_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [文枢 #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``wenshu sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `wenshu sessions list` (gemini-cli#27770 port).
        ...
        if _config_model and _config_model != getattr(self, "model", None):
            ...
            try:
                from wenshu_cli.model_switch import switch_model as _switch_model
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("WENSHU_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.wenshu-hermes/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``wenshu --resume <id>``
        ...
        saved_dir = get_wenshu_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"wenshu_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: wenshu --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from wenshu_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current 文枢 session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from wenshu_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        try:
            from wenshu_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `wenshu model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from wenshu_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from wenshu_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        from wenshu_cli.providers import get_label
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from wenshu_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_wenshu_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_wenshu_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in wenshu_cli/commands.py instead of touching every dispatch site.
        from wenshu_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from wenshu_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from wenshu_cli.skin_engine import get_active_skin
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from wenshu_state import format_session_db_unavailable
            ...
            from wenshu_cli.main import _print_version_info
            ...
            from wenshu_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `wenshu plugins
                ...
                # catalog lives behind `wenshu plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: wenshu plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_wenshu_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: wenshu plugins list)")
                else:
                    # Loaded-plugin details (tools/hooks/commands counts, errors)
                    # keyed by name, when available.
                    loaded: dict = {}
                    try:
                        from wenshu_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: wenshu plugins list)")
                    print("  Enable/disable: wenshu plugins enable/disable <name>")
            except Exception as e:
            ...
            from wenshu_cli.moa_config import (
                moa_usage,
            ...
            # Check for plugin-registered slash commands
                ...
                from wenshu_cli.plugins import (
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from wenshu_cli.goals import GoalManager
            from wenshu_cli.config import load_config
        except Exception as exc:
    ...
    def _drain_process_notifications(self, consumer: str) -> None:
        ...
        process using the same 文枢 profile.  Always pass this CLI's stable
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from wenshu_cli.goals import gather_background_processes as _gather_bg
    ...
    def _toggle_verbose(self):
        ...
        from wenshu_cli.colors import Colors as _Colors
        labels = {
    ...
    def _transfer_session_yolo(self, old_session_id: str, new_session_id: str) -> None:
        ...
        Mirrors ``the desktop chat server`` (~line 1297-1305) which performs the
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``WENSHU_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``the desktop chat server`` key=="yolo"). We deliberately do NOT mutate
        ``WENSHU_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from wenshu_cli.partial_compress import (
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if normalized != "openai-codex":
            ...
            print("  Switch with `/model` or `wenshu auth` first.")
            return
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by wenshu_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        * When opted out (``mcp.auto_reload_on_config_change: false``) it
        ...
        from wenshu_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.wenshu-hermes/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_wenshu_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "wenshu_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "wenshu_voice",
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        try:
            ...
            def run_agent():
                ...
                # Mirrors ``the desktop chat server`` and ``gateway/run.py`` which
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _wenshu_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard WENSHU_HOME, so no -p needed.
            try:
            ...
            print(f"  wenshu --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  wenshu -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from wenshu_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from wenshu_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from wenshu_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to 文枢! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to 文枢! Type your message or /help for commands."
        ...
        try:
            from wenshu_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("WENSHU_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(WENSHU_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→文枢 migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from wenshu_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in wenshu_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from wenshu_cli.model_switch import resolve_persist_behavior
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_wenshu_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting 文枢 binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from wenshu_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "文枢")
        ...
        # Dynamic prompt: shows 文枢 symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("文枢 needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', '文枢 needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("文枢 needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = WenshuCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_wenshu_osd_patched", False):
                ...
                _pt_renderer._wenshu_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``WENSHU_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("WENSHU_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the wenshu process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: wenshu setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `wenshu sessions list`
...
def _run_kanban_goal_loop_q(cli: "WenshuCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``WENSHU_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("WENSHU_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from wenshu_cli import kanban_db as _kb
    from wenshu_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    文枢 CLI - Interactive AI Assistant
    ...
    os.environ["WENSHU_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting 文枢 Gateway (messaging platforms)...")
    ...
    # Default to wenshu-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Wenshu): with no explicit --toolsets, collapse
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `wenshu skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    # mode registers its own inside WenshuCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  WENSHU_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("WENSHU_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``wenshu chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("WENSHU_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from wenshu_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Wenshu" box, no tool-gen
                        ...
                        if os.environ.get("WENSHU_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("WENSHU_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from wenshu_cli.kanban_db import (
                ...
                # Single-query mode (`wenshu chat -q "…"`): skip the welcome
